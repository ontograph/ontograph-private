...
from .client import OntocodeClient
...
class _AsyncLoginOwner(Protocol):
    """Subset of AsyncOntocode needed by async login handles."""

    _client: AsyncOntocodeClient

    async def _ensure_initialized(self) -> None:
        """Ensure the owning SDK client has a live Ontocode connection."""
        ...


def start_chatgpt_login(client: OntocodeClient) -> ChatgptLoginHandle:
...
def start_device_code_login(client: OntocodeClient) -> DeviceCodeLoginHandle:
...
class ChatgptLoginHandle:
    """Live browser-login attempt returned by `Ontocode.login_chatgpt()`."""
...
class DeviceCodeLoginHandle:
    """Live device-code login attempt returned by `Ontocode.login_chatgpt_device_code()`."""
...
class AsyncChatgptLoginHandle:
    """Live browser-login attempt returned by `AsyncOntocode.login_chatgpt()`."""

    _ontocode: _AsyncLoginOwner
    login_id: str
    auth_url: str
    ...
    async def wait(self) -> AccountLoginCompletedNotification:
        ...
        return await self._ontocode._client.wait_for_login_completed(self.login_id)

    async def cancel(self) -> CancelLoginAccountResponse:
        """Cancel this browser login attempt."""
        await self._ontocode._ensure_initialized()
        return await self._ontocode._client.account_login_cancel(self.login_id)


@dataclass(slots=True)
class AsyncDeviceCodeLoginHandle:
    """Live device-code attempt returned by `AsyncOntocode.login_chatgpt_device_code()`."""
