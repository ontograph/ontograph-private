...
/// neither is set, this prefers `~/.ontocode` when present, falls back to an
/// existing `~/.codex`, and otherwise defaults to `~/.ontocode`.
///
...
fn find_codex_home_with_overrides(
    ...
    match resolve_home_override(ontocode_home_env, codex_home_env) {
        ...
        None => {
            ...
            let ontocode_home = p.join(".ontocode");
            if ontocode_home.exists() {
                return AbsolutePathBuf::from_absolute_path(ontocode_home);
            }

            let codex_home = p.join(".codex");
            if codex_home.exists() {
                return AbsolutePathBuf::from_absolute_path(codex_home);
            }
...
mod tests {
    ...
    #[test]
    fn find_codex_home_without_env_prefers_existing_ontocode_home() {
        let temp_home = TempDir::new().expect("temp home");
    ...
    #[test]
    fn find_codex_home_without_env_falls_back_to_existing_codex_home() {
        let temp_home = TempDir::new().expect("temp home");
        let codex_home = temp_home.path().join(".codex");
        fs::create_dir_all(&codex_home).expect("create codex home");

        let resolved = find_codex_home_for_test(None, None, temp_home.path())
            .expect("default should use existing codex home");
        let expected = AbsolutePathBuf::from_absolute_path(codex_home).expect("absolute home");
        assert_eq!(resolved, expected);
    }
