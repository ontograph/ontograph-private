...
fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
    ...
    let tracer = provider.tracer("ontocode-exec-tests");
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = ontocode_protocol::protocol::W3cTraceContext {
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: ontocode_app_server_protocol::SessionSource::Exec,
        thread_source: None,
        agent_nickname: None,
        agent_role: None,
        git_info: None,
        name: None,
        turns: vec![
            ontocode_app_server_protocol::Turn {
                id: "turn-1".to_string(),
...
async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
    ...
    assert_eq!(
        ...
        Some(ontocode_app_server_protocol::ApprovalsReviewer::User)
    );
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(ontocode_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
async fn thread_start_params_include_user_thread_source() {
    ...
    assert_eq!(
        ...
        Some(ontocode_app_server_protocol::ThreadSource::User)
    );
}
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(ontocode_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        thread: ontocode_app_server_protocol::Thread {
            ...
            thread_source: Some(ontocode_app_server_protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            git_info: None,
        ...
        approval_policy: ontocode_app_server_protocol::AskForApproval::OnRequest,
        approvals_reviewer: ontocode_app_server_protocol::ApprovalsReviewer::AutoReview,
        sandbox: ontocode_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
