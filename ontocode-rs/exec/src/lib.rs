...
use ontocode_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use ontocode_app_server_client::EnvironmentManager;
use ontocode_app_server_client::ExecServerRuntimePaths;
use ontocode_app_server_client::InProcessAppServerClient;
use ontocode_app_server_client::InProcessClientStartArgs;
use ontocode_app_server_client::InProcessServerEvent;
...
use ontocode_app_server_protocol::Thread as AppServerThread;
use ontocode_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use ontocode_cloud_config::cloud_config_bundle_loader_for_storage;
use ontocode_config::CloudConfigBundleLoader;
use ontocode_config::ConfigLoadError;
use ontocode_config::ConfigLoadOptions;
use ontocode_config::LoaderOverrides;
use ontocode_config::format_config_error_with_source;
use ontocode_core::StateDbHandle;
use ontocode_core::check_execpolicy_for_warnings;
...
use ontocode_core::config::load_config_as_toml_with_cli_and_load_options;
use ontocode_core::config::resolve_oss_provider;
use ontocode_core::config::resolve_profile_v2_config_path;
use ontocode_core::find_thread_meta_by_name_str;
use ontocode_core::format_exec_policy_error_with_source;
use ontocode_core::path_utils;
use ontocode_feedback::CodexFeedback;
use ontocode_git_utils::get_git_repo_root;
use ontocode_login::AuthConfig;
use ontocode_login::default_client::set_default_client_residency_requirement;
use ontocode_login::default_client::set_default_originator;
use ontocode_login::enforce_login_restrictions;
use ontocode_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use ontocode_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use ontocode_otel::set_parent_from_context;
use ontocode_otel::traceparent_context_from_env;
...
use ontocode_protocol::protocol::ReviewTarget;
use ontocode_protocol::protocol::RolloutItem;
use ontocode_protocol::protocol::RolloutLine;
...
use ontocode_utils_absolute_path::canonicalize_existing_preserving_symlinks;
use ontocode_utils_cli::SharedCliOptions;
use ontocode_utils_oss::ensure_oss_provider_ready;
use ontocode_utils_oss::get_default_model_for_oss_provider;
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ontocode_core::otel_init::build_provider(
            &config,
    ...
    ontocode_core::otel_init::record_process_start(otel.as_ref(), "codex_exec");
    ontocode_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "codex_exec");
    ...
    let state_db = ontocode_core::init_state_db(&config).await;
...
async fn load_config_toml_or_exit(
    ...
    cli_kv_overrides: Vec<(String, ontocode_config::TomlValue)>,
    loader_overrides: LoaderOverrides,
    strict_config: bool,
    cloud_config_bundle: CloudConfigBundleLoader,
) -> ontocode_config::config_toml::ConfigToml {
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = ontocode_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    if !json_mode
        && let Some(message) =
            ontocode_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        ontocode_app_server_protocol::TurnStatus::Failed
                            | ontocode_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<ontocode_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<ontocode_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
