# Bazel in ontocode-rs

This repository uses Bazel to build the Rust workspace under `ontocode-rs`.
...
- `rules_rs` imports third-party crates from `ontocode-rs/Cargo.toml` and
  `ontocode-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `ontocode-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
...
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/codex \
  ...
  build --config=ci-linux //ontocode-rs/cli:ontocode
```
