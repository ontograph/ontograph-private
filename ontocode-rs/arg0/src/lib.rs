...
use ontocode_apply_patch::CODEX_CORE_APPLY_PATCH_ARG1;
use ontocode_exec_server::CODEX_FS_HELPER_ARG1;
use ontocode_install_context::InstallContext;
use ontocode_sandboxing::landlock::CODEX_LINUX_SANDBOX_ARG0;
use ontocode_sandboxing::landlock::ONTOCODE_LINUX_SANDBOX_ARG0;
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "ontocode-execve-wrapper";
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime
            .block_on(ontocode_shell_escalation::run_shell_escalation_execve_wrapper(file, argv));
        match exit_code {
    ...
    if exe_name == CODEX_LINUX_SANDBOX_ARG0 || exe_name == ONTOCODE_LINUX_SANDBOX_ARG0 {
        ...
        ontocode_linux_sandbox::run_main();
    ...
    if argv1 == CODEX_CORE_APPLY_PATCH_ARG1 {
        ...
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                ...
                let cwd = match ontocode_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                match runtime.block_on(ontocode_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    ontocode_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
...
/// [`ontocode_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
