...
use ontocode_app_server_protocol::RemoteControlConnectionStatus;
use ontocode_app_server_protocol::RemoteControlPairingStartParams;
use ontocode_app_server_protocol::RemoteControlStatusChangedNotification;
...
use ontocode_core::test_support::auth_manager_from_auth_with_home;
use ontocode_login::AuthDotJson;
use ontocode_login::AuthManager;
use ontocode_login::CodexAuth;
use ontocode_login::save_auth;
use ontocode_login::token_data::TokenData;
use ontocode_login::token_data::parse_chatgpt_jwt_claims;
...
async fn remote_control_transport_manages_virtual_clients_and_routes_messages() {
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Notification(
                    ontocode_app_server_protocol::JSONRPCNotification {
    ...
    let initialize_message =
        JSONRPCMessage::Request(ontocode_app_server_protocol::JSONRPCRequest {
            id: ontocode_app_server_protocol::RequestId::Integer(1),
    ...
    let followup_message =
        JSONRPCMessage::Notification(ontocode_app_server_protocol::JSONRPCNotification {
            method: "initialized".to_string(),
            params: None,
        });
    send_client_event(
...
async fn remote_control_transport_reconnects_after_disconnect() {
    ...
    send_client_event(
        ...
        ClientEnvelope {
            event: ClientEvent::ClientMessage {
                message: JSONRPCMessage::Request(ontocode_app_server_protocol::JSONRPCRequest {
                    id: ontocode_app_server_protocol::RequestId::Integer(2),
                    method: "initialize".to_string(),
                    params: Some(json!({
                        "clientInfo": {
                            "name": "remote-test-client",
...
async fn remote_control_http_mode_enrolls_before_connecting() {
    ...
    assert_eq!(
        handshake_request.headers.get("x-ontocode-protocol-version"),
    ...
    let writer = {
        let initialize_message =
            JSONRPCMessage::Request(ontocode_app_server_protocol::JSONRPCRequest {
                id: ontocode_app_server_protocol::RequestId::Integer(11),
                method: "initialize".to_string(),
                params: Some(json!({
                    "clientInfo": {
