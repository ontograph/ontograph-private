...
use ontocode_code_mode::ToolDefinition as CodeModeToolDefinition;
            ...
            for tool in &mut namespace.tools {
                match tool {
                    ResponsesApiNamespaceTool::Function(tool) => {
                        ...
                        tool.description =
                            ontocode_code_mode::augment_tool_definition(definition).description;
                    }
                }
            }
...
pub fn tool_spec_to_code_mode_tool_definition(spec: &ToolSpec) -> Option<CodeModeToolDefinition> {
    ...
    ontocode_code_mode::is_code_mode_nested_tool(&definition.name)
        .then(|| ontocode_code_mode::augment_tool_definition(definition))
}
...
pub fn collect_code_mode_tool_definitions<'a>(
    ...
    let mut tool_definitions = specs
        ...
        .filter(|definition| ontocode_code_mode::is_code_mode_nested_tool(&definition.name))
        .map(ontocode_code_mode::augment_tool_definition)
        .collect::<Vec<_>>();
