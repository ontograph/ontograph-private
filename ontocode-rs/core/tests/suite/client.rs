...
use ontocode_core::ModelClient;
use ontocode_core::NewThread;
use ontocode_core::Prompt;
use ontocode_core::ResponseEvent;
...
use ontocode_protocol::models::LocalShellAction;
use ontocode_protocol::models::LocalShellExecAction;
use ontocode_protocol::models::LocalShellStatus;
use ontocode_protocol::models::MessagePhase;
use ontocode_protocol::models::ReasoningItemContent;
use ontocode_protocol::models::ReasoningItemReasoningSummary;
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: match ontocode_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
            {
                Ok(cwd) => cwd,
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = ontocode_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = ontocode_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![ontocode_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = ontocode_protocol::models::ResponseItem::Message {
        id: None,
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let model = ontocode_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        ontocode_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let mut stream = client_session
        .stream(
            ...
            &ontocode_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    .await
    {
        Ok(Some(auth)) => ontocode_core::test_support::auth_manager_from_auth(auth),
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let model_info =
        ontocode_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let thread_id = ThreadId::new();
    let auth_manager = ontocode_core::test_support::auth_manager_from_auth(
