...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    codex
        .submit(Op::UserInput {
            environments: None,
            items: vec![ontocode_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    codex
        .submit(Op::UserInput {
            environments: None,
            items: vec![ontocode_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    assert_eq!(
        ...
        serde_json::json!(ontocode_login::default_client::originator().value)
    );
