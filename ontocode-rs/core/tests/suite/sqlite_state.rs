...
async fn new_thread_is_recorded_in_state_db() -> Result<()> {
    ...
    let db_path = ontocode_state::state_db_path(test.config.sqlite_home.as_path());
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let log_db_layer = ontocode_state::log_db::start(db.clone());
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = ontocode_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
