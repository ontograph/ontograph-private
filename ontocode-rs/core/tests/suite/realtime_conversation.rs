...
use ontocode_core::test_support::auth_manager_from_auth;
...
use ontocode_protocol::protocol::RealtimeNoopRequested;
...
const REALTIME_BACKEND_PROMPT: &str = ontocode_prompts::BACKEND_PROMPT;
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in ontocode_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let mut metadata_builder = ontocode_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("ontocode-rs/core"))?;
    ...
    assert!(startup_context.contains("ontocode-rs/"));
