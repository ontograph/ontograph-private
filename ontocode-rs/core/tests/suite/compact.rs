...
use ontocode_core::compact::SUMMARIZATION_PROMPT;
use ontocode_core::compact::SUMMARY_PREFIX;
...
async fn assert_compaction_uses_turn_lifecycle_id(
    codex: &std::sync::Arc<ontocode_core::CodexThread>,
) {
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        ontocode_protocol::models::ResponseItem::Message {
            id: None,
        ...
        ontocode_protocol::models::ResponseItem::Compaction {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn pre_sampling_compact_does_not_run_for_same_model() {
    skip_if_no_network!();
    ...
    let _models_mock = mount_models_once(
        &server,
        ModelsResponse {
            models: vec![model_info_with_context_window(
                model, /*context_window*/ 273_000,
            )],
        },
    )
    .await;
    ...
    let request_log = mount_sse_sequence(
        ...
        vec![
            sse(vec![
                ev_assistant_message("m1", "before same-model turn"),
                ev_completed_with_tokens("r1", /*total_tokens*/ 120_000),
            ]),
            sse(vec![
                ev_assistant_message("m2", "after same-model turn"),
    ...
    assert_eq!(
        ...
        2,
        "same-model turns should not emit an extra pre-sampling compact request"
    );
}
...
async fn snapshot_request_shape_pre_turn_compaction_including_incoming_user_message() {
    ...
    core_test_support::submit_thread_settings(
        &codex,
        ontocode_protocol::protocol::ThreadSettingsOverrides {
