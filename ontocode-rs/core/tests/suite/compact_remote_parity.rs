...
async fn build_harness_inner(
    ...
    TestCodexHarness::with_builder(builder.with_config(move |config| {
        config.cwd = ontocode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
async fn capture_from_requests(
    mode: Mode,
    codex: &ontocode_core::CodexThread,
    rollout_path: &Path,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        let start = windows_appdata_temp_start
            .or_else(|| prefix.rfind("/private/var/folders/"))
            ...
            .or_else(|| {
                prefix
                    .rfind("/.tmp")
                    .or_else(|| prefix.rfind("\\.tmp"))
                    .map(|temp_index| path_token_start_before(prefix, temp_index))
            });
...
fn path_token_start_before(prefix: &str, before: usize) -> usize {
    prefix[..before]
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    let text = normalize_string(
        "file: /tmp/.tmp5YYdK3/skills/.system/imagegen/SKILL.md and \
         ...
         /opt/demodb/_workfolder/ontocode-test-tmp/.tmpabc123/skills/local/SKILL.md",
    );
    ...
    assert_eq!(
        ...
        "file: <CODEX_HOME>/skills/.system/imagegen/SKILL.md and \
         ...
         <CODEX_HOME>/skills/local/SKILL.md"
    );
}
