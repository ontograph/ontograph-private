...
use ontocode_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
fn assert_repeated_char_output(output: &str, expected_char: char, expected_len: usize) {
    assert_eq!(output.len(), expected_len, "output length mismatch");
    assert!(
        output.chars().all(|ch| ch == expected_char),
        "output contains non-{expected_char:?} content"
    );
}

fn assert_shortened_output(output: &str, expected_char: char, original_len: usize) {
    assert!(
        output.len() < original_len,
        "nested exec output should be bounded by the process transcript cap"
    );
    assert!(
        output.contains(expected_char),
        "nested exec output should still contain command output"
    );
}
...
#[cfg_attr(windows, ignore = "no exec_command on Windows")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn code_mode_nested_exec_explicit_max_uses_process_transcript_cap() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
"#,
    ...
    assert_shortened_output(output, 'x', 50_000);
...
#[cfg_attr(windows, ignore = "no exec_command on Windows")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn code_mode_nested_exec_large_output_uses_process_transcript_cap() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
"#,
    ...
    let items = custom_tool_output_items(&second_mock.single_request(), "call-1");
    assert_shortened_output(text_item(&items, /*index*/ 1), 'A', 90_000);
...
#[cfg_attr(windows, ignore = "no exec_command on Windows")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn code_mode_nested_exec_explicit_max_bypasses_outer_truncation_policy() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
#[cfg_attr(windows, ignore = "no exec_command on Windows")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn code_mode_nested_exec_without_max_uses_nested_default() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
"#,
    ...
    assert!(
        output.len() < 50_000,
        "nested exec output should be truncated unless the nested call sets max_output_tokens"
    );
    assert!(
        output.contains('x'),
        "nested exec output should still contain command output"
...
#[cfg_attr(windows, ignore = "no exec_command on Windows")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn code_mode_nested_exec_without_max_ignores_outer_truncation_policy() -> Result<()> {
    skip_if_no_network!(Ok(()));
...
"#,
    ...
    let items = custom_tool_output_items(&second_mock.single_request(), "call-1");
    assert_shortened_output(text_item(&items, /*index*/ 1), 'x', 50_000);
