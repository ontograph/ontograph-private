...
use ontocode_protocol::items::parse_hook_prompt_fragment;
...
fn trust_plugin_hooks(config: &mut Config, plugin_hook_sources: Vec<PluginHookSource>) {
    ...
    let listed = ontocode_hooks::list_hooks(ontocode_hooks::HooksConfig {
...
"#,
  ...
  "hooks": {
    ...
    let plugin_hook_sources = vec![PluginHookSource {
        ...
        hooks: serde_json::from_str::<ontocode_config::HooksFile>(plugin_hooks_json)
