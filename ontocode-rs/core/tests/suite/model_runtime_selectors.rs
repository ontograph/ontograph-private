...
async fn remote_tool_mode_selector_overrides_feature_flags() -> Result<()> {
    ...
    assert!(
        direct_tools
            .iter()
            .all(|name| name != ontocode_code_mode::PUBLIC_TOOL_NAME
                && name != ontocode_code_mode::WAIT_TOOL_NAME),
    ...
    assert_eq!(
        ...
        vec![
            ...
            ontocode_code_mode::PUBLIC_TOOL_NAME.to_string(),
            ontocode_code_mode::WAIT_TOOL_NAME.to_string(),
