...
use ontocode_core::config::AgentRoleConfig;
...
use ontocode_provider_auth::derive_provider_route;
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    let spawned_id = wait_for_spawned_thread_id_with_timeout(&test, Duration::from_secs(6)).await?;
...
fn install_spawn_role(config: &mut Config, role_name: &str, provider_id: &str) {
    let role_path = config.codex_home.join(format!("{role_name}.toml"));
    if let Err(err) = std::fs::write(
        &role_path,
        format!("model = \"{ROLE_MODEL}\"\nmodel_provider = \"{provider_id}\"\n"),
    ) {
        panic!("failed to write role config: {err}");
    }
...
fn route_for_snapshot(config: &Config, snapshot: &ThreadConfigSnapshot) -> ProviderRoute {
    let provider_info = config
        .model_providers
        .get(snapshot.model_provider_id.as_str())
        .unwrap_or_else(|| panic!("missing child provider {}", snapshot.model_provider_id));
    derive_provider_route(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn spawn_agent_role_provider_override_keeps_parent_settings_and_auth_unchanged() -> Result<()>
{
    skip_if_no_network!(Ok(()));
    ...
    assert_eq!(child_snapshot.model_provider_id, "ollama");
    assert_eq!(child_snapshot.reasoning_effort, Some(ROLE_REASONING_EFFORT));

    let parent_thread = test
        .thread_manager
        .get_thread(test.session_configured.thread_id)
        .await?;
    let parent_snapshot = parent_thread.config_snapshot().await;
    assert_eq!(parent_snapshot.model, INHERITED_MODEL);
    assert_eq!(parent_snapshot.model_provider_id, "openai");
    assert_eq!(
        test.thread_manager.auth_manager().auth_cached(),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn spawn_agent_role_provider_override_routes_api_key_child_without_changing_parent_route()
-> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let (test, spawned_id, child_request_log) = setup_turn_one_with_custom_spawned_child(
        ...
        |builder| {
            builder.with_config(|config| {
                install_spawn_role(config, "api-key", "gemini");
            })
        },
    )
    .await?;
    ...
    let child_snapshot = test
        ...
        .config_snapshot()
        .await;
    assert_eq!(child_snapshot.model_provider_id, "gemini");
    assert_eq!(child_snapshot.model, ROLE_MODEL);
    assert_eq!(
        route_for_snapshot(&test.config, &child_snapshot),
        ProviderRoute::new("gemini", ROLE_MODEL, ProviderAuthKind::ApiKey,)
    );

    let parent_route = derive_provider_route(
    ...
    assert_eq!(
        parent_route,
        ProviderRoute::new("openai", INHERITED_MODEL, ProviderAuthKind::OAuth)
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn spawn_agent_role_provider_override_routes_oauth_child_without_changing_parent_route()
-> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let (test, spawned_id, child_request_log) = setup_turn_one_with_custom_spawned_child(
        ...
        |builder| {
            builder
                ...
                .with_config(|config| {
                    install_spawn_role(config, "oauth", "openai");
                })
        },
    )
    .await?;
    ...
    assert_eq!(
        route_for_snapshot(&test.config, &child_snapshot),
        ProviderRoute::new("openai", ROLE_MODEL, ProviderAuthKind::OAuth)
    );
