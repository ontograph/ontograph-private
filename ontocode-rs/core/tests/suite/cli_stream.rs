...
fn repo_root() -> std::path::PathBuf {
    ...
    ontocode_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = ontocode_utils_cargo_bin::cargo_bin("ontocode").unwrap();
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = ontocode_utils_cargo_bin::cargo_bin("ontocode").unwrap();
