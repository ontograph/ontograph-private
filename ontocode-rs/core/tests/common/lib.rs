...
use anyhow::ensure;
use ctor::ctor;
use ontocode_arg0::Arg0PathEntryGuard;
use ontocode_utils_cargo_bin::CargoBinError;
...
pub use ontocode_utils_absolute_path::test_support::PathBufExt;
pub use ontocode_utils_absolute_path::test_support::PathExt;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    ontocode_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    ontocode_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(ontocode_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = ontocode_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("ontocode-rs"));
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        codex_linux_sandbox_exe: Some(
            find_codex_linux_sandbox_exe().expect("should find binary for ontocode-linux-sandbox"),
        ),
...
pub fn find_codex_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    ontocode_utils_cargo_bin::cargo_bin("ontocode-linux-sandbox")
}

pub async fn wait_for_event<F>(
    codex: &CodexThread,
    predicate: F,
) -> ontocode_protocol::protocol::EventMsg
where
    F: FnMut(&ontocode_protocol::protocol::EventMsg) -> bool,
{
...
where
    F: Fn(&ontocode_protocol::protocol::EventMsg) -> Option<T>,
{
...
pub fn sandbox_env_var() -> &'static str {
    ontocode_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    ontocode_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    ontocode_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    ontocode_utils_cargo_bin::cargo_bin("test_stdio_server")
    ...
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_codex_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("ontocode-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
