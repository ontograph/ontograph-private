...
use ontocode_config::merge_toml_values;
...
use ontocode_config::permissions_toml::overlay_network_domain_permissions;
use ontocode_exec_server::LOCAL_FS;
use ontocode_network_proxy::ConfigReloader;
use ontocode_network_proxy::ConfigState;
use ontocode_network_proxy::NetworkMode;
use ontocode_network_proxy::NetworkProxyConfig;
use ontocode_network_proxy::NetworkProxyConstraintError;
use ontocode_network_proxy::NetworkProxyConstraints;
use ontocode_network_proxy::NetworkProxyState;
use ontocode_network_proxy::build_config_state;
use ontocode_network_proxy::normalize_host;
use ontocode_network_proxy::validate_policy_against_constraints;
...
async fn build_config_state_with_mtimes() -> Result<(ConfigState, Vec<LayerMtime>)> {
    ...
    let (exec_policy, warning) = match load_exec_policy(&config_layer_stack).await {
        ...
        Err(err @ ExecPolicyError::ParsePolicy { .. }) => {
            (ontocode_execpolicy::Policy::empty(), Some(err))
        }
...
fn apply_exec_policy_network_rules(
    ...
    for host in allowed_domains {
        upsert_network_domain(
            config,
            host,
            ontocode_network_proxy::NetworkDomainPermission::Allow,
        );
    }
    for host in denied_domains {
        upsert_network_domain(
            config,
            host,
            ontocode_network_proxy::NetworkDomainPermission::Deny,
        );
    }
}
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: ontocode_network_proxy::NetworkDomainPermission,
) {
    config
        .network
