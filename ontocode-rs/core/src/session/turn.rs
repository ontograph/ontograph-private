...
use ontocode_analytics::CompactionPhase;
use ontocode_analytics::CompactionReason;
use ontocode_analytics::InvocationType;
use ontocode_analytics::TurnResolvedConfigFact;
use ontocode_analytics::build_track_events_context;
use ontocode_async_utils::OrCancelExt;
...
use ontocode_git_utils::get_git_repo_root_with_fs;
...
use ontocode_protocol::items::PlanItem;
...
use ontocode_protocol::protocol::AgentMessageContentDeltaEvent;
use ontocode_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use ontocode_protocol::protocol::PlanDeltaEvent;
use ontocode_protocol::protocol::ReasoningContentDeltaEvent;
use ontocode_protocol::protocol::ReasoningRawContentDeltaEvent;
...
use ontocode_tools::filter_request_plugin_install_discoverable_tools_for_client;
use ontocode_utils_stream_parser::AssistantTextChunk;
use ontocode_utils_stream_parser::AssistantTextStreamParser;
use ontocode_utils_stream_parser::ProposedPlanSegment;
use ontocode_utils_stream_parser::extract_proposed_plan_text;
use ontocode_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<ontocode_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        let error = err.to_ontocode_protocol_error();
    ...
    loop {
        ...
        .await
        {
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_ontocode_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                if error == CodexErrorInfo::UsageLimitExceeded
                    && let Err(err) = sess
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = ontocode_connectors::merge::merge_plugin_connectors_with_accessible(
            loaded_plugins
                .effective_apps()
                .into_iter()
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = ontocode_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
    turn_store: Arc<ontocode_extension_api::ExtensionData>,
...
fn agent_message_text(item: &ontocode_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            ontocode_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: ontocode_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(ontocode_protocol::items::AgentMessageItem {
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(ontocode_async_utils::CancelErr::Cancelled) => break Err(CodexErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![ontocode_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
