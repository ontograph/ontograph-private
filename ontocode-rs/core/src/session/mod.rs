...
use ontocode_analytics::SubAgentThreadStartedInput;
use ontocode_analytics::TurnCodexErrorFact;
...
use ontocode_exec_server::Environment;
use ontocode_exec_server::EnvironmentManager;
use ontocode_exec_server::FileSystemSandboxContext;
use ontocode_extension_api::PromptSlot;
use ontocode_features::FEATURES;
use ontocode_features::Feature;
use ontocode_features::unstable_features_warning_event;
use ontocode_hooks::Hooks;
use ontocode_hooks::HooksConfig;
use ontocode_login::AuthManager;
use ontocode_login::CodexAuth;
use ontocode_login::auth_env_telemetry::collect_auth_env_telemetry;
use ontocode_login::default_client::originator;
use ontocode_mcp::McpConnectionManager;
use ontocode_mcp::McpRuntimeContext;
use ontocode_mcp::codex_apps_tools_cache_key;
use ontocode_models_manager::manager::RefreshStrategy;
use ontocode_models_manager::manager::SharedModelsManager;
use ontocode_network_proxy::NetworkProxy;
use ontocode_network_proxy::NetworkProxyAuditMetadata;
use ontocode_network_proxy::normalize_host;
use ontocode_otel::current_span_trace_id;
use ontocode_otel::current_span_w3c_trace_context;
use ontocode_otel::set_parent_from_w3c_trace_context;
use ontocode_protocol::SessionId;
use ontocode_protocol::ThreadId;
use ontocode_protocol::approvals::ElicitationRequestEvent;
use ontocode_protocol::approvals::ExecPolicyAmendment;
use ontocode_protocol::approvals::NetworkPolicyAmendment;
use ontocode_protocol::approvals::NetworkPolicyRuleAction;
use ontocode_protocol::config_types::ApprovalsReviewer;
...
use ontocode_protocol::models::format_allow_prefixes;
...
use ontocode_protocol::protocol::AdditionalContextEntry;
use ontocode_protocol::protocol::FileChange;
use ontocode_protocol::protocol::HasLegacyEvent;
use ontocode_protocol::protocol::InterAgentCommunication;
use ontocode_protocol::protocol::ItemCompletedEvent;
use ontocode_protocol::protocol::ItemStartedEvent;
use ontocode_protocol::protocol::MultiAgentVersion;
use ontocode_protocol::protocol::RawResponseItemEvent;
...
use ontocode_protocol::protocol::TurnAbortReason;
use ontocode_protocol::protocol::TurnContextItem;
use ontocode_protocol::protocol::TurnContextNetworkItem;
...
use ontocode_protocol::request_permissions::RequestPermissionsArgs;
use ontocode_protocol::request_permissions::RequestPermissionsEvent;
use ontocode_protocol::request_permissions::RequestPermissionsResponse;
...
use ontocode_rollout_trace::AgentResultTracePayload;
use ontocode_rollout_trace::ThreadStartedTraceMetadata;
use ontocode_rollout_trace::ThreadTraceContext;
use ontocode_sandboxing::policy_transforms::intersect_permission_profiles;
use ontocode_shell_command::parse_command::parse_command;
use ontocode_terminal_detection::user_agent;
use ontocode_thread_store::CreateThreadParams;
use ontocode_thread_store::LiveThread;
use ontocode_thread_store::LiveThreadInitGuard;
use ontocode_thread_store::LocalThreadStore;
use ontocode_thread_store::ReadThreadParams;
use ontocode_thread_store::ResumeThreadParams;
use ontocode_thread_store::ThreadPersistenceMetadata;
...
use ontocode_config::CONFIG_TOML_FILE;
...
use ontocode_mcp::compute_auth_statuses;
use ontocode_mcp::effective_mcp_servers_from_configured;
use ontocode_mcp::host_owned_codex_apps_enabled;
use ontocode_otel::SessionTelemetry;
use ontocode_otel::THREAD_STARTED_METRIC;
...
use ontocode_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
...
use ontocode_protocol::protocol::ApplyPatchApprovalRequestEvent;
...
use ontocode_protocol::protocol::DeprecationNoticeEvent;
use ontocode_protocol::protocol::ErrorEvent;
...
use ontocode_protocol::protocol::McpServerRefreshConfig;
use ontocode_protocol::protocol::ModelRerouteEvent;
use ontocode_protocol::protocol::ModelRerouteReason;
use ontocode_protocol::protocol::ModelVerification;
use ontocode_protocol::protocol::ModelVerificationEvent;
use ontocode_protocol::protocol::NetworkApprovalContext;
...
use ontocode_protocol::protocol::RequestUserInputEvent;
...
use ontocode_protocol::protocol::SessionNetworkProxyRuntime;
use ontocode_protocol::protocol::StreamErrorEvent;
use ontocode_protocol::protocol::Submission;
use ontocode_protocol::protocol::ThreadMemoryMode;
...
#[cfg(test)]
use ontocode_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct CodexSpawnArgs {
    ...
    pub(crate) extensions: Arc<ontocode_extension_api::ExtensionRegistry<crate::config::Config>>,
...
impl Codex {
    ...
    pub(crate) async fn spawn(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if ontocode_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        let refresh_strategy = if session_source.is_non_root_agent() {
            ontocode_models_manager::manager::RefreshStrategy::Offline
        } else {
            ontocode_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
    ...
    pub async fn set_thread_memory_mode(
        &self,
        mode: ontocode_protocol::protocol::ThreadMemoryMode,
    ) -> anyhow::Result<()> {
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &ontocode_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn ontocode_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn ontocode_network_proxy::BlockedRequestObserver>>,
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &ontocode_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| ontocode_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                ontocode_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(ontocode_hooks::SessionStartSource::Compact);
        }
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<ontocode_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
