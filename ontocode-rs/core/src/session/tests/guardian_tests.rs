...
use ontocode_protocol::models::AdditionalPermissionProfile as PermissionProfile;
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if matches!(
                event.msg,
                ontocode_protocol::protocol::EventMsg::GuardianAssessment(_)
            ) {
                break;
            }
        }
...
async fn guardian_allows_shell_command_additional_permissions_requests_past_policy_validation() {
    ...
    turn_context_raw.permission_profile = ontocode_protocol::models::PermissionProfile::Disabled;
    let mut config = (*turn_context_raw.config).clone();
    config.model_provider.base_url = Some(format!("{}/v1", server.uri()));
    let config = Arc::new(config);
    let models_manager = models_manager_with_provider(
    ...
    let handler = crate::tools::handlers::ShellCommandHandler::from(
        ontocode_tools::ShellCommandBackendConfig::Classic,
    );
    #[allow(deprecated)]
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: ontocode_tools::ToolName::plain("shell_command"),
...
async fn guardian_subagent_does_not_inherit_parent_exec_policy_rules() {
    ...
    let thread_store = Arc::new(ontocode_thread_store::LocalThreadStore::new(
        ontocode_thread_store::LocalThreadStoreConfig::from_config(&config),
        /*state_db*/ None,
    ));
    ...
    let CodexSpawnOk { codex, .. } = Codex::spawn(CodexSpawnArgs {
        ...
        plugins_manager,
        mcp_manager,
        extensions: ontocode_extension_api::empty_extension_registry(),
        ...
        parent_rollout_thread_trace: ontocode_rollout_trace::ThreadTraceContext::disabled(),
