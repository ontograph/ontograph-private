...
async fn apply_role_preserves_unspecified_keys() {
    ...
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/ontocode-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(
        ...
        Some(PathBuf::from("/tmp/ontocode-execve-wrapper"))
    );
}
...
"#,
    ...
    let outcome = skills_manager
        .skills_for_config(
            &skills_input,
            Some(Arc::clone(&ontocode_exec_server::LOCAL_FS)),
        )
        .await;
    let skill = outcome
        .skills
