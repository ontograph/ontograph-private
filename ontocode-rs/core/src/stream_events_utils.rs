...
use ontocode_protocol::items::ImageGenerationItem;
...
use futures::Future;
use ontocode_memories_read::citations::parse_memory_citation;
use ontocode_memories_read::citations::thread_ids_from_memory_citation;
use ontocode_protocol::error::CodexErr;
use ontocode_protocol::error::Result;
use ontocode_protocol::memory_citation::MemoryCitation;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<ontocode_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                ontocode_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
            ...
            TurnItem::AgentMessage(agent_message) => {
                let combined = agent_message
                    ...
                    .map(|entry| match entry {
                        ontocode_protocol::items::AgentMessageContent::Text { text } => {
                            text.as_str()
                        }
                    })
                    .collect::<String>();
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(error) => match error.reason_code() {
            ontocode_tools::FunctionCallErrorReasonCode::RespondToModel => {
                let FunctionCallError::RespondToModel(message) = error else {
                    unreachable!("respond_to_model reason code must carry a model-facing message")
                };
            ...
            ontocode_tools::FunctionCallErrorReasonCode::Fatal => {
                let FunctionCallError::Fatal(message) = error else {
                    unreachable!("fatal reason code must carry a fatal message")
                };
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![ontocode_protocol::items::AgentMessageContent::Text { text: stripped }];
