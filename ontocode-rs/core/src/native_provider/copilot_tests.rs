...
#[test]
fn sets_versioned_copilot_headers() {
    ...
    assert_eq!(
        ...
        Some(
            &http::HeaderValue::from_str(&format!("codex/{version}"))
                .expect("valid version header")
        )
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(&http::HeaderValue::from_static("vscode-chat"))
    );
}
...
#[test]
fn builds_canonical_copilot_source_oauth_credential_from_env_key() {
    let provider_info = ModelProviderInfo {
        name: "GitHub Copilot".to_string(),
        env_key: Some("GITHUB_TOKEN_FOR_TEST".to_string()),
        ..Default::default()
    };
    unsafe {
        std::env::set_var("GITHUB_TOKEN_FOR_TEST", "github-token");
    }

    let credential =
        copilot_source_oauth_credential(&provider_info).expect("source credential should build");

    assert_eq!(credential.provider_name, "GitHub Copilot");
    assert_eq!(credential.credential_id, "GITHUB_TOKEN_FOR_TEST");
    assert_eq!(credential.access_token, "github-token");
    assert_eq!(credential.refresh_token, None);
    assert_eq!(credential.provenance.as_deref(), Some("github_env_token"));

    unsafe {
        std::env::remove_var("GITHUB_TOKEN_FOR_TEST");
    }
}
