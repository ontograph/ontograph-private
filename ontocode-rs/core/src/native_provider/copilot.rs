...
const COPILOT_USER_AGENT: &str = concat!("Codex/", env!("CARGO_PKG_VERSION"));
const COPILOT_EDITOR_VERSION: &str = concat!("codex/", env!("CARGO_PKG_VERSION"));
const COPILOT_EDITOR_PLUGIN_VERSION: &str = concat!("codex/", env!("CARGO_PKG_VERSION"));
...
pub(crate) async fn stream_chat_completions(
    ...
    let source_credential = copilot_source_oauth_credential(provider_info)?;
    let copilot_token = exchange_github_token(
        client.clone(),
...
fn copilot_source_oauth_credential(
    provider_info: &ModelProviderInfo,
) -> Result<ProviderOAuthCredential> {
    let access_token = copilot_github_token(provider_info)?;
    let credential_id = provider_info
        .env_key
        .clone()
        .unwrap_or_else(|| "github-access-token".to_string());
    let mut credential = ProviderOAuthCredential::new(
        provider_info.name.clone(),
        ontocode_protocol::credential_routing::ProviderCredentialSourceKind::ExternalImport,
        credential_id,
        access_token,
    );
    credential.provenance = Some("github_env_token".to_string());
    Ok(credential)
}
