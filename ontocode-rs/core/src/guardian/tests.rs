...
use ontocode_analytics::GuardianApprovalRequestSource;
...
use ontocode_model_provider_info::AMAZON_BEDROCK_GPT_5_4_MODEL_ID;
use ontocode_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use ontocode_protocol::protocol::GuardianRiskLevel;
use ontocode_protocol::protocol::GuardianUserAuthorization;
        ...
        .record_conversation_items(
            ...
            &[
                ...
                ResponseItem::FunctionCallOutput {
                    call_id: "call-1".to_string(),
                    output: ontocode_protocol::models::FunctionCallOutputPayload::from_text(
...
fn normalize_guardian_snapshot_paths(text: String) -> String {
    let mut text = text;
    for canonical_path in ["/repo/ontocode-rs/core", "/repo"] {
...
fn guardian_prompt_text(items: &[ontocode_protocol::user_input::UserInput]) -> String {
    items
        .iter()
        .map(|item| match item {
            ontocode_protocol::user_input::UserInput::Text { text, .. } => text.as_str(),
            _ => "",
        })
        .collect::<String>()
}
...
async fn build_guardian_prompt_full_mode_preserves_initial_review_format() -> anyhow::Result<()> {
    ...
    let prompt = build_guardian_prompt_items(
        ...
        GuardianApprovalRequest::Shell {
            ...
            cwd: test_path_buf("/repo/ontocode-rs/core").abs(),
...
-> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(ontocode_analytics::GuardianReviewSessionKind::TrunkNew)
    ));
...
async fn build_guardian_prompt_items_includes_parent_session_id() -> anyhow::Result<()> {
    ...
    let prompt_text = prompt
        ...
        .into_iter()
        .map(|item| match item {
            ontocode_protocol::user_input::UserInput::Text { text, .. } => text,
            ontocode_protocol::user_input::UserInput::Image { .. } => String::new(),
            _ => String::new(),
        })
        .collect::<String>();
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    assert!(matches!(
        ...
        Some(ontocode_analytics::GuardianReviewSessionKind::TrunkReused)
    ));
    assert!(matches!(
...
async fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        "parent-active-model",
        Some(ontocode_protocol::openai_models::ReasoningEffort::Low),
    )
