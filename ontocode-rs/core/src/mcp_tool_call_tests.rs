...
use crate::mcp_tool_approval_templates::build_mcp_tool_approval_elicitation_meta;
...
use ontocode_config::types::AppConfig;
use ontocode_config::types::AppToolConfig;
use ontocode_config::types::AppToolsConfig;
use ontocode_config::types::ApprovalsReviewer;
use ontocode_config::types::AppsConfigToml;
...
use ontocode_rollout_trace::ToolDispatchInvocation;
use ontocode_rollout_trace::ToolDispatchPayload;
use ontocode_rollout_trace::ToolDispatchRequester;
...
"#,
    ...
    let hook_list = ontocode_hooks::list_hooks(HooksConfig {
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        ontocode_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
...
async fn install_host_owned_codex_apps_manager(session: &Session, turn_context: &TurnContext) {
    ...
    let (manager, _cancel_token) = ontocode_mcp::McpConnectionManager::new(
        &HashMap::new(),
        ...
        ontocode_mcp::McpRuntimeContext::new(Arc::clone(&session.services.environment_manager), {
            #[allow(deprecated)]
            turn_context.cwd.to_path_buf()
        }),
        turn_context.config.codex_home.to_path_buf(),
        ontocode_mcp::codex_apps_tools_cache_key(auth.as_ref()),
        /*host_owned_codex_apps_enabled*/ true,
        turn_context.config.prefix_mcp_tool_names(),
        rmcp::model::ElicitationCapability::default(),
        ontocode_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
...
async fn codex_apps_auth_elicitation_feature_enabled_requests_elicitation() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        request.id,
        ontocode_protocol::mcp::RequestId::String("codex_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: ontocode_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    ConfigEditsBuilder::new(&codex_home)
        .set_project_trust_level(
            project_dir.path(),
            ontocode_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            ontocode_login::CodexAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        turn_context
            .approval_policy
            .set(approval_policy)
