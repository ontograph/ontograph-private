...
use ontocode_config::types::OtelExporterKind as Kind;
use ontocode_config::types::OtelHttpProtocol as Protocol;
use ontocode_features::Feature;
use ontocode_login::default_client::originator;
use ontocode_otel::OtelExporter;
use ontocode_otel::OtelHttpProtocol;
use ontocode_otel::OtelProvider;
use ontocode_otel::OtelSettings;
use ontocode_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Keeps events that originated from ontocode_otel module
pub fn codex_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("ontocode_otel")
}
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = ontocode_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = ontocode_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
    let _ = ontocode_state::install_process_db_telemetry(telemetry);
}
