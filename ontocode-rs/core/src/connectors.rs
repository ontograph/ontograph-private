...
pub use ontocode_app_server_protocol::AppBranding;
pub use ontocode_app_server_protocol::AppInfo;
pub use ontocode_app_server_protocol::AppMetadata;
use ontocode_connectors::ConnectorDirectoryCacheContext;
use ontocode_connectors::ConnectorDirectoryCacheKey;
...
use ontocode_config::AppsRequirementsToml;
...
use ontocode_mcp::with_codex_apps_mcp;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = ontocode_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        ontocode_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    read_cached_accessible_connectors(&cache_key).map(|connectors| {
        ontocode_connectors::filter::filter_disallowed_connectors(
            connectors,
            originator().value.as_str(),
        )
    })
...
pub(crate) fn refresh_accessible_connectors_cache_from_mcp_tools(
    ...
    let accessible_connectors = ontocode_connectors::filter::filter_disallowed_connectors(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_environment_manager(
    ...
    {
        let cached_connectors = ontocode_connectors::filter::filter_disallowed_connectors(
            cached_connectors,
            originator().value.as_str(),
        );
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + ontocode_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    ontocode_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
pub(crate) fn accessible_connectors_from_mcp_tools(mcp_tools: &[ToolInfo]) -> Vec<AppInfo> {
    ...
    let tools = mcp_tools.iter().filter_map(|tool| {
        ...
        Some(ontocode_connectors::accessible::AccessibleConnectorTool {
    ...
    ontocode_connectors::accessible::collect_accessible_connectors(tools)
}
