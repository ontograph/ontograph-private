...
fn exec_server_params_use_env_policy_overlay_contract() {
    let cwd: ontocode_utils_absolute_path::AbsolutePathBuf = std::env::current_dir()
        .expect("current dir")
        .try_into()
        .expect("absolute path");
    let file_system_sandbox_policy =
        ontocode_protocol::permissions::FileSystemSandboxPolicy::unrestricted();
    let network_sandbox_policy = ontocode_protocol::permissions::NetworkSandboxPolicy::Restricted;
    let permission_profile = ontocode_protocol::models::PermissionProfile::Disabled;
    let request = ExecRequest {
        ...
        exec_server_env_config: Some(ExecServerEnvConfig {
            policy: ontocode_exec_server::ExecEnvPolicy {
                inherit: ontocode_protocol::config_types::ShellEnvironmentPolicyInherit::Core,
        ...
        sandbox: ontocode_sandboxing::SandboxType::None,
...
async fn failed_initial_end_for_unstored_process_uses_fallback_output() {
    ...
    let request = ExecCommandRequest {
        ...
        shell_mode: ontocode_tools::UnifiedExecShellMode::Direct,
        network: None,
        tty: true,
    ...
    let ontocode_protocol::protocol::EventMsg::ExecCommandEnd(end_event) = event.msg else {
        panic!("expected ExecCommandEnd event");
    };
    assert_eq!(end_event.call_id, "call-unified-denied");
    assert_eq!(
        end_event.status,
        ontocode_protocol::protocol::ExecCommandStatus::Failed
    );
