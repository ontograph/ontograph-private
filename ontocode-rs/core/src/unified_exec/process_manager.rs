...
) -> ontocode_exec_server::ExecEnvPolicy {
    ontocode_exec_server::ExecEnvPolicy {
...
fn exec_server_env_for_request(
    ...
    Option<ontocode_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> ontocode_exec_server::ExecParams {
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        if request.sandbox == ontocode_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let spawned = match request.windows_sandbox_level {
                ontocode_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    ontocode_windows_sandbox::spawn_windows_sandbox_session_elevated_for_permission_profile(
                ...
                ontocode_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | ontocode_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    ontocode_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        &request.permission_profile,
                        request.windows_sandbox_workspace_roots.as_slice(),
                        codex_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            ontocode_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
                &request.arg0,
                ontocode_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            ontocode_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                request.cwd.as_path(),
                &request.env,
