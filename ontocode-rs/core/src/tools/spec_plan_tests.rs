...
use ontocode_protocol::openai_models::ApplyPatchToolType;
...
use ontocode_tools::DiscoverablePluginInfo;
...
impl ToolExecutor<ExtensionToolCall> for WebRunExtensionTool {
    ...
    async fn handle(
        ...
        Ok(Box::new(ontocode_tools::JsonToolOutput::new(json!({}))))
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: ontocode_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    ontocode_tools::JsonSchema::string(/*description*/ None),
                )]),
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if ontocode_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            ontocode_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
    ...
    let plan = probe(|turn| {
        ...
        turn.environments
            ...
            .push(crate::session::turn_context::TurnEnvironment {
                ...
                environment: Arc::new(
                    ontocode_exec_server::Environment::create_for_tests(Some(
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    plain.assert_visible_lacks(&[
        ontocode_code_mode::PUBLIC_TOOL_NAME,
        ontocode_code_mode::WAIT_TOOL_NAME,
    ]);
