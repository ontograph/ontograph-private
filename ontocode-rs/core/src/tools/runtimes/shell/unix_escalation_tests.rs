...
use ontocode_execpolicy::PolicyParser;
...
use ontocode_protocol::protocol::GuardianCommandSource;
use ontocode_sandboxing::SandboxType;
use ontocode_shell_escalation::EscalationExecution;
use ontocode_shell_escalation::EscalationPermissions;
use ontocode_shell_escalation::ExecResult;
use ontocode_shell_escalation::ResolvedPermissionProfile;
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    let config_toml_path = turn_context
        .config
        .codex_home
        .join(ontocode_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command = ontocode_shell_command::parse_command::shlex_join(&[
        "/usr/bin/touch".to_string(),
        target_str,
    ]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(ontocode_execpolicy::Policy::empty())),
        ...
        stopwatch: ontocode_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = tokio::time::timeout(
        Duration::from_secs(5),
        ontocode_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        ontocode_shell_escalation::EscalationDecision::Escalate(
            ontocode_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    let action = ontocode_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    assert_eq!(action, ontocode_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        ontocode_shell_escalation::EscalationDecision::Deny {
