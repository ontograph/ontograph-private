...
use ontocode_code_mode::CellId;
use ontocode_code_mode::CodeModeNestedToolCall;
use ontocode_code_mode::CodeModeSession;
use ontocode_code_mode::CodeModeToolKind;
use ontocode_code_mode::RuntimeResponse;
...
use ontocode_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use ontocode_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = ontocode_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = ontocode_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = ontocode_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    pub(crate) fn new() -> Self {
        ...
        Self {
            session: Some(Arc::new(
                ontocode_code_mode::CodeModeService::with_delegate(dispatch_broker.clone()),
            )),
            dispatch_broker,
        }
    }
    ...
    pub(crate) async fn execute(
        &self,
        request: ontocode_code_mode::ExecuteRequest,
    ) -> Result<ontocode_code_mode::StartedCell, String> {
        self.session()?.execute(request).await
    }

    pub(crate) async fn wait(
        &self,
        request: ontocode_code_mode::WaitRequest,
    ) -> Result<ontocode_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &ontocode_code_mode::CellId) {
