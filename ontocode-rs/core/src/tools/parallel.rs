    ...
    fn failure_response(call: ToolCall, err: FunctionCallError) -> ResponseInputItem {
        ...
        match call.payload {
            ...
            ToolPayload::Custom { .. } => ResponseInputItem::CustomToolCallOutput {
                call_id: call.call_id,
                name: None,
                output: ontocode_protocol::models::FunctionCallOutputPayload {
                    body: ontocode_protocol::models::FunctionCallOutputBody::Text(message),
                    success: Some(false),
                },
            },
...
mod tests {
    ...
    impl ontocode_extension_api::ToolLifecycleContributor for FinishRecorder {
        fn on_tool_finish<'a>(
            &'a self,
    ...
    impl ontocode_extension_api::ToolLifecycleContributor for BlockingFinishContributor {
        fn on_tool_finish<'a>(
            &'a self,
            input: ontocode_extension_api::ToolFinishInput<'a>,
        ) -> ontocode_extension_api::ToolLifecycleFuture<'a> {
    ...
    -> anyhow::Result<()> {
        ...
        let tool_name = ontocode_tools::ToolName::plain("test_tool");
    ...
    -> anyhow::Result<()> {
        ...
        let session = Arc::new(session);
        let turn_context = Arc::new(turn_context);
        let tool_name = ontocode_tools::ToolName::plain("cleanup_tool");
