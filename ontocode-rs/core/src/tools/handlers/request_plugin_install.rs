...
use ontocode_tools::DiscoverableToolAction;
use ontocode_tools::DiscoverableToolType;
use ontocode_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use ontocode_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use ontocode_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use ontocode_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use ontocode_tools::RequestPluginInstallArgs;
use ontocode_tools::RequestPluginInstallResult;
use ontocode_tools::ToolName;
use ontocode_tools::ToolSpec;
use ontocode_tools::all_requested_connectors_picked_up;
use ontocode_tools::build_request_plugin_install_elicitation_request;
use ontocode_tools::filter_request_plugin_install_discoverable_tools_for_client;
use ontocode_tools::verified_connector_install_completed;
...
impl ToolExecutor<ToolInvocation> for RequestPluginInstallHandler {
    ...
    async fn handle(
        ...
        if args.tool_type == DiscoverableToolType::Plugin
            && turn.app_server_client_name.as_deref() == Some("ontocode-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in ontocode-tui yet".to_string(),
            ));
        }
...
async fn persist_disabled_install_request(
    codex_home: &ontocode_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    ...
    turn: &crate::session::turn_context::TurnContext,
    tool: &DiscoverableTool,
    auth: Option<&ontocode_login::CodexAuth>,
) -> bool {
    match tool {
