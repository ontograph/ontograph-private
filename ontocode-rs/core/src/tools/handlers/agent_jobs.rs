...
const AGENT_JOB_RESULT_JSON_PREVIEW_CHARS: usize = 4096;
...
fn required_state_db(
    session: &Arc<Session>,
) -> Result<Arc<ontocode_state::StateRuntime>, FunctionCallError> {
...
fn build_worker_prompt(
    job: &ontocode_state::AgentJob,
    item: &ontocode_state::AgentJobItem,
...
fn job_runtime_timeout(job: &ontocode_state::AgentJob) -> Duration {
    job.max_runtime_seconds
        .map(Duration::from_secs)
        .unwrap_or(DEFAULT_AGENT_JOB_ITEM_TIMEOUT)
}

fn started_at_from_item(item: &ontocode_state::AgentJobItem) -> Instant {
...
fn is_item_stale(item: &ontocode_state::AgentJobItem, runtime_timeout: Duration) -> bool {
    let now = chrono::Utc::now();
...
fn render_job_csv(
    headers: &[String],
    items: &[ontocode_state::AgentJobItem],
    ...
    output_headers.extend([
        ...
        "row_index".to_string(),
        "source_id".to_string(),
        "assigned_thread_id".to_string(),
        "status".to_string(),
        "attempt_count".to_string(),
        "last_error".to_string(),
        "result_json".to_string(),
        "result_json_truncated".to_string(),
    ...
    for item in items {
        ...
        row_values.push(csv_escape(
            item.assigned_thread_id.clone().unwrap_or_default().as_str(),
        ));
        ...
        let (result_json, result_json_truncated) = capped_result_json(item.result_json.as_ref());
        row_values.push(csv_escape(result_json.as_str()));
        row_values.push(csv_escape(result_json_truncated.to_string().as_str()));
        row_values.push(csv_escape(
            item.reported_at
...
fn capped_result_json(value: Option<&Value>) -> (String, bool) {
    let Some(value) = value else {
        return (String::new(), false);
    };
    let rendered = value.to_string();
    if rendered.chars().count() <= AGENT_JOB_RESULT_JSON_PREVIEW_CHARS {
        return (rendered, false);
    }
    let capped = rendered
        .chars()
        .take(AGENT_JOB_RESULT_JSON_PREVIEW_CHARS)
        .collect::<String>();
