...
use crate::tools::handlers::mcp_support::build_search_info;
use crate::tools::handlers::mcp_support::create_tool_spec;
use crate::tools::handlers::mcp_support::ensure_mcp_prefix;
use crate::tools::handlers::mcp_support::join_tool_name;
use crate::tools::handlers::mcp_support::mcp_hook_tool_input;
...
impl ToolExecutor<ToolInvocation> for McpHandler {
    ...
    fn search_info(&self) -> Option<ToolSearchInfo> {
        build_search_info(&self.tool_info, self.spec())
    }
...
mod tests {
    ...
    async fn mcp_pre_tool_use_payload_uses_prefixed_tool_name_and_raw_args() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: ontocode_tools::ToolName::namespaced("memory", "create_entities"),
    ...
    async fn mcp_pre_tool_use_payload_keeps_builtin_like_tool_names_namespaced() {
        ...
        assert_eq!(
            handler.pre_tool_use_payload(&ToolInvocation {
                ...
                tool_name: ontocode_tools::ToolName::namespaced("mcp__foo", "exec_command"),
                source: ToolCallSource::Direct,
                payload,
            }),
            Some(PreToolUsePayload {
    ...
    async fn mcp_post_tool_use_payload_uses_prefixed_tool_name_args_and_result() {
        ...
        let output = McpToolOutput {
            result: ontocode_protocol::mcp::CallToolResult {
                content: vec![json!({
                    "type": "text",
                    "text": "notes"
                })],
            ...
            truncation_policy: ontocode_utils_output_truncation::TruncationPolicy::Bytes(1024),
        };
        let (session, turn) = make_session_and_context().await;
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: ontocode_tools::ToolName::namespaced("filesystem", "read_file"),
            source: ToolCallSource::Direct,
            payload,
        };
        assert_eq!(
