...
use ontocode_models_manager::model_info::model_info_from_slug;
...
use ontocode_protocol::protocol::FileSystemAccessMode;
use ontocode_protocol::protocol::FileSystemPath;
use ontocode_protocol::protocol::FileSystemSandboxEntry;
...
use ontocode_protocol::protocol::NetworkSandboxPolicy;
...
use ontocode_state::DirectionalThreadSpawnEdgeStatus;
        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    ontocode_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
#[tokio::test]
async fn spawn_agent_rejects_unknown_model_id() {
    let (session, turn) = make_session_and_context().await;
    let available_models = session
        ...
        .await;
    let unknown_model = "definitely-not-a-model";
    ...
    let err = SpawnAgentHandler::default()
        ...
        .await
        .err()
        .expect("unknown model should be rejected");
    ...
    assert_eq!(
        ...
        FunctionCallError::RespondToModel(format!(
            "Unknown model `{unknown_model}` for spawn_agent. Available models: {available}"
        ))
    );
}
...
async fn multi_agent_v2_spawn_fork_turns_all_rejects_agent_type_override() {
    ...
    let turn = TurnContext {
        ...
        multi_agent_version: ontocode_protocol::protocol::MultiAgentVersion::V2,
        ..turn
    };
...
#[tokio::test]
async fn spawn_agent_accepts_custom_model_id_from_catalog() {
    #[derive(Debug, Deserialize)]
    struct SpawnAgentResult {
        agent_id: String,
    }
    ...
    let custom_model = "gpt-5-custom-spawn";
    let mut model_info = model_info_from_slug(custom_model);
    model_info.visibility = ModelVisibility::None;
    session.services.models_manager = Arc::new(StaticModelsManager::new(
        None,
        ModelsResponse {
    ...
    let output = SpawnAgentHandler::default()
        ...
        .await
        .expect("spawn_agent should accept an exact custom model id");
    let (content, _) = expect_text_output(output);
    let result: SpawnAgentResult =
        serde_json::from_str(&content).expect("spawn_agent result should be json");
    let snapshot = manager
    ...
    assert_eq!(snapshot.model, custom_model);
}
