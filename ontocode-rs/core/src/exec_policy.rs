...
use ontocode_execpolicy::AmendError;
use ontocode_execpolicy::Decision;
use ontocode_execpolicy::Error as ExecPolicyRuleError;
use ontocode_execpolicy::Evaluation;
use ontocode_execpolicy::MatchOptions;
...
use ontocode_execpolicy::blocking_append_allow_prefix_rule;
use ontocode_execpolicy::blocking_append_network_rule;
...
use ontocode_shell_command::is_dangerous_command::command_might_be_dangerous;
use ontocode_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
use ontocode_shell_command::bash::parse_shell_lc_plain_commands;
use ontocode_shell_command::bash::parse_shell_lc_single_command_prefix;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(ontocode_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: ontocode_execpolicy::Error,
    },
}
...
fn exec_policy_message_for_display(source: &ontocode_execpolicy::Error) -> String {
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            ontocode_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
    ...
    let command_is_dangerous = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            ontocode_shell_command::is_dangerous_command::is_dangerous_powershell_words(command)
        }
    };
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            ontocode_shell_command::powershell::parse_powershell_command_into_plain_commands(
                command,
            )
            && !commands.is_empty()
        {
