...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let execve_wrapper = allowed_arg0_dir.join("ontocode-execve-wrapper");
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        ...
        Some(ontocode_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        ...
        Some(ontocode_network_proxy::NetworkDomainPermissions {
            entries: vec![ontocode_network_proxy::NetworkDomainPermissionEntry {
                pattern: "openai.com".to_string(),
                permission: ontocode_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}
