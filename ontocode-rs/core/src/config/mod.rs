...
use ontocode_config::CloudConfigBundleLoader;
use ontocode_config::ConfigLayerSource;
use ontocode_config::ConfigLayerStack;
use ontocode_config::ConfigLayerStackOrdering;
use ontocode_config::ConfigRequirements;
use ontocode_config::ConfigRequirementsToml;
use ontocode_config::ConstrainedWithSource;
use ontocode_config::FeatureRequirementsToml;
use ontocode_config::McpServerIdentity;
use ontocode_config::McpServerRequirement;
use ontocode_config::PluginRequirementsToml;
use ontocode_config::ProfileV2Name;
use ontocode_config::ResidencyRequirement;
use ontocode_config::SandboxModeRequirement;
use ontocode_config::Sourced;
use ontocode_config::ThreadConfigLoader;
use ontocode_config::config_toml::ConfigLockfileToml;
use ontocode_config::config_toml::ConfigToml;
use ontocode_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use ontocode_config::config_toml::ProjectConfig;
use ontocode_config::config_toml::RealtimeAudioConfig;
use ontocode_config::config_toml::RealtimeConfig;
use ontocode_config::config_toml::ThreadStoreToml;
use ontocode_config::config_toml::validate_model_providers;
use ontocode_config::loader::load_config_layers_state;
use ontocode_config::loader::project_trust_key;
use ontocode_config::permissions_toml::PermissionsToml;
use ontocode_config::sandbox_mode_requirement_for_permission_profile;
use ontocode_config::types::ApprovalsReviewer;
use ontocode_config::types::AuthCredentialsStoreMode;
use ontocode_config::types::History;
use ontocode_config::types::McpServerConfig;
use ontocode_config::types::McpServerDisabledReason;
use ontocode_config::types::McpServerTransportConfig;
use ontocode_config::types::MemoriesConfig;
use ontocode_config::types::ModelAvailabilityNuxConfig;
use ontocode_config::types::Notice;
use ontocode_config::types::OAuthCredentialsStoreMode;
use ontocode_config::types::SessionPickerViewMode;
use ontocode_config::types::ToolSuggestConfig;
use ontocode_config::types::ToolSuggestDisabledTool;
use ontocode_config::types::ToolSuggestDiscoverable;
use ontocode_config::types::TuiKeymap;
use ontocode_config::types::TuiNotificationSettings;
use ontocode_config::types::TuiPetAnchor;
use ontocode_config::types::UriBasedFileOpener;
use ontocode_config::types::WindowsSandboxModeToml;
use ontocode_core_plugins::PluginsConfigInput;
use ontocode_exec_server::ExecutorFileSystem;
use ontocode_exec_server::LOCAL_FS;
use ontocode_features::AppsMcpPathOverrideConfigToml;
use ontocode_features::Feature;
use ontocode_features::FeatureConfigSource;
use ontocode_features::FeatureOverrides;
use ontocode_features::FeatureToml;
use ontocode_features::Features;
use ontocode_features::FeaturesToml;
use ontocode_features::MultiAgentV2ConfigToml;
use ontocode_features::NetworkProxyConfigToml;
use ontocode_git_utils::resolve_root_git_project_for_trust;
use ontocode_install_context::InstallContext;
use ontocode_login::AuthManagerConfig;
use ontocode_mcp::McpConfig;
use ontocode_memories_read::memory_root;
use ontocode_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use ontocode_model_provider_info::ModelProviderInfo;
use ontocode_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use ontocode_model_provider_info::built_in_model_providers;
use ontocode_model_provider_info::merge_configured_model_providers;
use ontocode_models_manager::ModelsManagerConfig;
use ontocode_protocol::config_types::AltScreenMode;
use ontocode_protocol::config_types::AutoCompactTokenLimitScope;
...
use ontocode_protocol::config_types::SandboxMode;
...
use ontocode_protocol::config_types::WebSearchConfig;
...
use ontocode_utils_absolute_path::AbsolutePathBufGuard;
...
pub use ontocode_config::ConfigLoadOptions;
pub use ontocode_config::Constrained;
pub use ontocode_config::ConstraintError;
pub use ontocode_config::ConstraintResult;
pub use ontocode_config::LoaderOverrides;
pub use ontocode_network_proxy::NetworkProxyAuditMetadata;
use ontocode_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use ontocode_sandboxing::system_bwrap_warning;
...
You can spawn direct sub-agents to handle subtasks. Coding sub-agents cannot spawn their own sub-agents.
All agents in the team, including the agents that you can assign tasks to, are equally intelligent and capable, while tool access may vary by role and mode.

You can use `spawn_agent` to create a new agent, `followup_task` to give an existing agent a new task and trigger a turn, and `send_message` to pass a message to a running agent without triggering a turn.
Coding child agents cannot spawn their own sub-agents.
...
If `spawn_agent` is available, you can spawn direct sub-agents to handle subtasks. Coding sub-agents cannot spawn their own sub-agents. All agents in the team, including the agents that you can assign tasks to, are equally intelligent and capable, while tool access may vary by role and mode.

When these tools are available, you can use `spawn_agent` to create a new agent, `followup_task` to give an existing agent a new task and trigger a turn, and `send_message` to pass a message to a running agent.
Coding child agents cannot spawn their own sub-agents.
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(ontocode_state::SQLITE_HOME_ENV).ok()?;
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<ontocode_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    ///          https://github.com/openai/codex/blob/main/ontocode-rs/rmcp-client/src/oauth.rs#L2
    ...
    /// [`ontocode_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    ...
    /// Path to the `ontocode-execve-wrapper` executable used for shell
    ...
    pub otel: ontocode_config::types::OtelConfig,
}
...
impl ConfigBuilder {
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        if loader_overrides.user_config_path.is_none() {
            migrate_legacy_config_to_ontocode_home(codex_home.as_ref())?;
        }
        let config_load_home =
            config_read_home_with_legacy_fallback(&codex_home, &mut loader_overrides);
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&ontocode_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = ontocode_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    ontocode_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(ontocode_config::io_error_from_config_error(
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &ontocode_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub async fn load_default_with_cli_overrides_for_codex_home(
        ...
        let cli_layer = ontocode_config::build_cli_overrides_layer(&cli_overrides);
        ontocode_config::merge_toml_values(&mut merged, &cli_layer);
...
fn config_read_home_with_legacy_fallback(
    codex_home: &AbsolutePathBuf,
    loader_overrides: &mut LoaderOverrides,
) -> PathBuf {
    if loader_overrides.user_config_path.is_some() {
        return codex_home.to_path_buf();
    }

    let Some(legacy_home) = sibling_legacy_codex_home(codex_home.as_ref()) else {
        return codex_home.to_path_buf();
    };
    let active_base_config = codex_home.join(CONFIG_TOML_FILE);
    let legacy_base_config = legacy_home.join(CONFIG_TOML_FILE);

    if let Some(profile_name) = loader_overrides.user_config_profile.as_ref() {
        let active_profile_config = resolve_profile_v2_config_path(codex_home, profile_name);
        if active_profile_config.exists() || active_base_config.exists() {
            return codex_home.to_path_buf();
        }

        let legacy_profile_config = resolve_profile_v2_config_path(&legacy_home, profile_name);
        if legacy_profile_config.exists() || legacy_base_config.exists() {
            loader_overrides.user_config_path = legacy_profile_config
                .exists()
                .then_some(legacy_profile_config);
            return legacy_home;
        }
    ...
    if !active_base_config.exists() && legacy_base_config.exists() {
        return legacy_home;
    }
...
fn sibling_legacy_codex_home(codex_home: &Path) -> Option<PathBuf> {
    if codex_home.file_name()? != ".ontocode" {
        return None;
    }
...
pub fn migrate_legacy_config_to_ontocode_home(codex_home: &Path) -> std::io::Result<bool> {
    if codex_home.file_name() != Some(std::ffi::OsStr::new(".ontocode")) {
        return Ok(false);
    }
    let Some(legacy_home) = sibling_legacy_codex_home(codex_home) else {
        return Ok(false);
    };
    let active_config = codex_home.join(CONFIG_TOML_FILE);
    let legacy_config = legacy_home.join(CONFIG_TOML_FILE);
    if active_config.exists() || !legacy_config.exists() {
        return Ok(false);
    }
    std::fs::create_dir_all(codex_home)?;
    std::fs::copy(legacy_config, active_config)?;
    Ok(true)
}
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    if options.loader_overrides.user_config_path.is_none() {
        migrate_legacy_config_to_ontocode_home(codex_home)?;
    }
    let config_load_home = AbsolutePathBuf::from_absolute_path(codex_home)
        .map(|codex_home| {
            config_read_home_with_legacy_fallback(&codex_home, &mut options.loader_overrides)
        })
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &cli_overrides,
        options,
        &ontocode_config::NoopThreadConfigLoader,
    )
    .await?;
...
pub fn set_default_oss_provider(codex_home: &Path, provider: &str) -> std::io::Result<()> {
    ontocode_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &ontocode_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            ontocode_protocol::permissions::FileSystemSandboxEntry {
                path: ontocode_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: ontocode_protocol::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            ontocode_protocol::permissions::FileSystemSandboxEntry {
                path: ontocode_protocol::permissions::FileSystemPath::Path { path },
                access: ontocode_protocol::permissions::FileSystemAccessMode::Deny,
            }
        };
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(ontocode_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
...
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    ontocode_utils_home_dir::find_codex_home()
}
