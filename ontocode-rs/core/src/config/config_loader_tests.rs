...
use ontocode_config::ConfigError;
...
use ontocode_config::ConfigRequirementsWithSources;
use ontocode_config::FilesystemDenyReadPattern;
use ontocode_config::LoaderOverrides;
use ontocode_config::RequirementSource;
use ontocode_config::RequirementsLayerEntry;
use ontocode_config::SessionThreadConfig;
use ontocode_config::StaticThreadConfigLoader;
use ontocode_config::ThreadConfigSource;
use ontocode_config::compose_requirements;
use ontocode_config::config_error_from_ignored_toml_fields;
use ontocode_config::config_error_from_toml;
...
use ontocode_config::loader::load_requirements_toml;
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = ontocode_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        ontocode_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = ontocode_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = ontocode_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
#[tokio::test]
async fn default_ontocode_home_falls_back_to_legacy_codex_base_config() -> std::io::Result<()> {
    let home_root = tempdir().expect("tempdir");
    let ontocode_home = home_root.path().join(".ontocode");
    let legacy_home = home_root.path().join(".codex");
    std::fs::create_dir_all(&ontocode_home).expect("create ontocode home");
    std::fs::create_dir_all(&legacy_home).expect("create legacy home");
    std::fs::write(
        legacy_home.join(CONFIG_TOML_FILE),
        r#"model = "gpt-legacy-base""#,
    )
    .expect("write legacy base config");

    let config = ConfigBuilder::without_managed_config_for_tests()
        .codex_home(ontocode_home.clone())
        .fallback_cwd(Some(ontocode_home.clone()))
        .build()
        .await?;

    assert_eq!(config.codex_home.as_ref(), ontocode_home.as_path());
    assert_eq!(config.model.as_deref(), Some("gpt-legacy-base"));
    assert_eq!(
        std::fs::read_to_string(ontocode_home.join(CONFIG_TOML_FILE))?,
        r#"model = "gpt-legacy-base""#
    );
    Ok(())
}
...
#[tokio::test]
async fn default_ontocode_home_keeps_active_base_config_precedence_over_legacy_codex()
-> std::io::Result<()> {
    let home_root = tempdir().expect("tempdir");
    let ontocode_home = home_root.path().join(".ontocode");
    let legacy_home = home_root.path().join(".codex");
    ...
    std::fs::create_dir_all(&legacy_home).expect("create legacy home");
    std::fs::write(
        ontocode_home.join(CONFIG_TOML_FILE),
        r#"model = "gpt-ontocode-base""#,
    )
    .expect("write active base config");
    std::fs::write(
        legacy_home.join(CONFIG_TOML_FILE),
        r#"model = "gpt-legacy-base""#,
    )
    ...
    assert_eq!(config.model.as_deref(), Some("gpt-ontocode-base"));
    Ok(())
}

#[tokio::test]
async fn default_ontocode_home_profile_falls_back_to_legacy_codex_home() -> std::io::Result<()> {
    let home_root = tempdir().expect("tempdir");
    let ontocode_home = home_root.path().join(".ontocode");
    let legacy_home = home_root.path().join(".codex");
    std::fs::create_dir_all(&ontocode_home).expect("create ontocode home");
    ...
    std::fs::write(
        legacy_home.join(CONFIG_TOML_FILE),
        r#"log_dir = "legacy-logs""#,
    )
    .expect("write legacy base config");
    std::fs::write(
        legacy_home.join("work.config.toml"),
        r#"model = "gpt-legacy-work""#,
    )
    ...
    assert_eq!(config.model.as_deref(), Some("gpt-legacy-work"));
    assert_eq!(config.log_dir, legacy_home.join("legacy-logs"));
    Ok(())
}
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![ontocode_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(ontocode_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(ontocode_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            ontocode_config::SandboxModeRequirement::ReadOnly,
            ontocode_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    ontocode_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
mod requirements_exec_policy_tests {
    ...
    use ontocode_config::ConfigRequirementsWithSources;
    use ontocode_config::RequirementSource;
    use ontocode_config::RequirementsExecPolicyDecisionToml;
    use ontocode_config::RequirementsExecPolicyParseError;
    use ontocode_config::RequirementsExecPolicyPatternTokenToml;
    use ontocode_config::RequirementsExecPolicyPrefixRuleToml;
    use ontocode_config::RequirementsExecPolicyToml;
    use ontocode_execpolicy::Decision;
    use ontocode_execpolicy::Evaluation;
    use ontocode_execpolicy::RuleMatch;
    use ontocode_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
