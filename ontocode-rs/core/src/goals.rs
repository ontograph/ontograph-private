...
use ontocode_otel::GOAL_BLOCKED_METRIC;
use ontocode_otel::GOAL_BUDGET_LIMITED_METRIC;
use ontocode_otel::GOAL_COMPLETED_METRIC;
use ontocode_otel::GOAL_CREATED_METRIC;
use ontocode_otel::GOAL_DURATION_SECONDS_METRIC;
use ontocode_otel::GOAL_RESUMED_METRIC;
use ontocode_otel::GOAL_TOKEN_COUNT_METRIC;
use ontocode_otel::GOAL_USAGE_LIMITED_METRIC;
use ontocode_prompts::budget_limit_prompt;
use ontocode_prompts::continuation_prompt;
use ontocode_prompts::objective_updated_prompt;
...
use ontocode_protocol::protocol::validate_thread_goal_objective;
...
pub struct ExternalGoalPreviousGoal {
    goal_id: String,
    status: ontocode_state::ThreadGoalStatus,
    objective: String,
}

impl From<&ontocode_state::ThreadGoal> for ExternalGoalPreviousStatus {
    fn from(goal: &ontocode_state::ThreadGoal) -> Self {
        Self::Existing(ExternalGoalPreviousGoal::from(goal))
    }
}

impl From<&ontocode_state::ThreadGoal> for ExternalGoalPreviousGoal {
    fn from(goal: &ontocode_state::ThreadGoal) -> Self {
        Self {
...
pub struct ExternalGoalSet {
    pub goal: ontocode_state::ThreadGoal,
...
impl Session {
    ...
    pub(crate) async fn set_thread_goal(
        ...
        self.account_thread_goal_wall_clock_usage(
            &state_db,
            ontocode_state::GoalAccountingMode::ActiveOnly,
            TerminalMetricEmission::Emit,
        )
        .await?;
                ...
                state_db
                    ...
                    .replace_thread_goal(
                        ...
                        status
                            ...
                            .unwrap_or(ontocode_state::ThreadGoalStatus::Active),
        ...
        *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        let newly_active_goal = goal_status == ontocode_state::ThreadGoalStatus::Active
            && (replacing_goal
                || previous_status
                    .is_some_and(|status| status != ontocode_state::ThreadGoalStatus::Active));
        if newly_active_goal {
            ...
            .await;
        } else if goal_status != ontocode_state::ThreadGoalStatus::Active {
    ...
    async fn apply_external_thread_goal_status(self: &Arc<Self>, external_set: ExternalGoalSet) {
        ...
        match status {
            ...
            ontocode_state::ThreadGoalStatus::BudgetLimited => {
                if self.active_turn_context().await.is_none() {
                    self.clear_stopped_thread_goal_runtime_state().await;
                }
            }
            ontocode_state::ThreadGoalStatus::Paused
            | ontocode_state::ThreadGoalStatus::Blocked
            | ontocode_state::ThreadGoalStatus::UsageLimited
    ...
    fn emit_goal_resumed_metric_if_status_changed(
        &self,
        previous_status: Option<ontocode_state::ThreadGoalStatus>,
        goal_status: ontocode_state::ThreadGoalStatus,
    ) {
        if goal_status == ontocode_state::ThreadGoalStatus::Active
            && matches!(
                previous_status,
                Some(
                    ontocode_state::ThreadGoalStatus::Paused
    ...
    fn emit_goal_terminal_metrics_if_status_changed(
        &self,
        previous_status: Option<ontocode_state::ThreadGoalStatus>,
        goal: &ontocode_state::ThreadGoal,
    ) {
        ...
        let counter = match goal.status {
            ontocode_state::ThreadGoalStatus::Blocked => GOAL_BLOCKED_METRIC,
            ontocode_state::ThreadGoalStatus::UsageLimited => GOAL_USAGE_LIMITED_METRIC,
            ontocode_state::ThreadGoalStatus::BudgetLimited => GOAL_BUDGET_LIMITED_METRIC,
            ontocode_state::ThreadGoalStatus::Complete => GOAL_COMPLETED_METRIC,
            ontocode_state::ThreadGoalStatus::Active | ontocode_state::ThreadGoalStatus::Paused => {
                return;
            }
        };
    ...
    ) -> anyhow::Result<Option<ontocode_state::ThreadGoalStatus>> {
        let goal = state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
            .await?;
    ...
    async fn account_thread_goal_progress(
        ...
        let goal = match outcome {
            ontocode_state::GoalAccountingOutcome::Updated(goal) => {
                let clear_active_goal = match goal.status {
                    ontocode_state::ThreadGoalStatus::Active => false,
                    ontocode_state::ThreadGoalStatus::BudgetLimited => {
                    ...
                    | ontocode_state::ThreadGoalStatus::Blocked
                    | ontocode_state::ThreadGoalStatus::UsageLimited
                    | ontocode_state::ThreadGoalStatus::Complete => true,
                };
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
            ...
            ontocode_state::GoalAccountingOutcome::Unchanged(_) => return Ok(()),
        };
        let should_steer_budget_limit =
            matches!(budget_limit_steering, BudgetLimitSteering::Allowed)
                && goal.status == ontocode_state::ThreadGoalStatus::BudgetLimited
                && !budget_limit_was_already_reported;
        let goal_status = goal.status;
        let goal_id = goal.goal_id.clone();
        if goal_status != ontocode_state::ThreadGoalStatus::BudgetLimited {
            *self.goal_runtime.budget_limit_reported_goal_id.lock().await = None;
        }
        let goal = protocol_goal_from_state(goal);
        self.send_event(
    ...
    async fn account_thread_goal_wall_clock_usage(
        &self,
        state_db: &StateDbHandle,
        mode: ontocode_state::GoalAccountingMode,
        terminal_metric_emission: TerminalMetricEmission,
            ...
            .await?
        {
            ...
            ontocode_state::GoalAccountingOutcome::Unchanged(goal) => {
                {
                    let mut accounting = self.goal_runtime.accounting.lock().await;
                ...
                .await
            {
                Ok(Some(goal))
                    if goal.goal_id == candidate.goal_id
                        && goal.status == ontocode_state::ThreadGoalStatus::Active =>
                {
                    true
                }
                Ok(Some(_)) | Ok(None) => {
    ...
    async fn goal_continuation_candidate_if_active(
        ...
        if goal.status != ontocode_state::ThreadGoalStatus::Active {
...
pub(crate) fn protocol_goal_from_state(goal: ontocode_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
pub(crate) fn protocol_goal_status_from_state(
    ...
    match status {
        ontocode_state::ThreadGoalStatus::Active => ThreadGoalStatus::Active,
        ontocode_state::ThreadGoalStatus::Paused => ThreadGoalStatus::Paused,
        ontocode_state::ThreadGoalStatus::Blocked => ThreadGoalStatus::Blocked,
        ontocode_state::ThreadGoalStatus::UsageLimited => ThreadGoalStatus::UsageLimited,
        ontocode_state::ThreadGoalStatus::BudgetLimited => ThreadGoalStatus::BudgetLimited,
        ontocode_state::ThreadGoalStatus::Complete => ThreadGoalStatus::Complete,
    }
}

pub(crate) fn state_goal_status_from_protocol(
    status: ThreadGoalStatus,
) -> ontocode_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => ontocode_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => ontocode_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => ontocode_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => ontocode_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => ontocode_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => ontocode_state::ThreadGoalStatus::Complete,
    }
}
