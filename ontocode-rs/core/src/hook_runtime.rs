...
use ontocode_analytics::CompactionTrigger;
use ontocode_analytics::HookRunFact;
use ontocode_analytics::build_track_events_context;
use ontocode_hooks::PermissionRequestDecision;
use ontocode_hooks::PermissionRequestOutcome;
use ontocode_hooks::PermissionRequestRequest;
use ontocode_hooks::PostToolUseOutcome;
use ontocode_hooks::PostToolUseRequest;
use ontocode_hooks::PreToolUseOutcome;
use ontocode_hooks::PreToolUseRequest;
use ontocode_hooks::SessionStartOutcome;
use ontocode_hooks::StartHookTarget;
use ontocode_hooks::StopHookTarget;
use ontocode_hooks::StopOutcome;
use ontocode_hooks::SubagentHookContext;
use ontocode_hooks::UserPromptSubmitOutcome;
use ontocode_hooks::UserPromptSubmitRequest;
use ontocode_otel::HOOK_RUN_DURATION_METRIC;
use ontocode_otel::HOOK_RUN_METRIC;
...
use ontocode_protocol::protocol::HookStartedEvent;
        ...
        let target = match &turn_context.session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { agent_role, .. })
                if matches!(
                    ...
                    ontocode_hooks::SessionStartSource::Startup
                ) =>
            {
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = ontocode_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = ontocode_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = ontocode_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(ontocode_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: ontocode_hooks::HookEvent::AfterAgent {
                event: ontocode_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            ontocode_hooks::HookResult::Success => continue,
            ontocode_hooks::HookResult::Action(ontocode_hooks::HookAction::Allow) => continue,
            ontocode_hooks::HookResult::Action(ontocode_hooks::HookAction::Warn { message }) => {
                tracing::warn!(turn_id = %turn_context.sub_id, "hook {hook_name} issued warning: {message}");
                continue;
            }
            ontocode_hooks::HookResult::Action(ontocode_hooks::HookAction::SystemMessage {
                ..
            }) => {
                continue;
            }
            ontocode_hooks::HookResult::Action(ontocode_hooks::HookAction::Block { message }) => (
                anyhow::anyhow!("hook {hook_name} blocked operation: {message}").into(),
                true,
            ),
            ontocode_hooks::HookResult::FailedContinue(error) => (error, false),
            ontocode_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(ontocode_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (ontocode_analytics::TrackEventsContext, HookRunFact) {
    (
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    ontocode_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
