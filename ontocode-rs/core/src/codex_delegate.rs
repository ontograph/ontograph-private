    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                ontocode_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                ontocode_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                ontocode_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            ontocode_protocol::request_user_input::RequestUserInputAnswer {
...
) -> ontocode_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = ontocode_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, ontocode_protocol::protocol::ReviewDecision::Abort)
                .await;
            ontocode_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
