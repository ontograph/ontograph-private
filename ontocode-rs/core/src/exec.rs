...
use ontocode_protocol::protocol::ExecCommandOutputDeltaEvent;
use ontocode_protocol::protocol::ExecOutputStream;
use ontocode_sandboxing::SandboxCommand;
use ontocode_sandboxing::SandboxManager;
use ontocode_sandboxing::SandboxTransformRequest;
use ontocode_sandboxing::SandboxType;
use ontocode_sandboxing::SandboxablePreference;
...
use ontocode_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: ontocode_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: ontocode_protocol::config_types::WindowsSandboxLevel,
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        ontocode_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = ontocode_otel::global() {
...
async fn exec_windows_sandbox(
    ...
    use ontocode_windows_sandbox::run_windows_sandbox_capture_for_permission_profile_elevated;
    use ontocode_windows_sandbox::run_windows_sandbox_capture_with_filesystem_overrides;
    ...
    let (cancellation, timeout_ms) = if capture_policy.uses_expiration() {
        let cancellation = expiration.cancellation_token().map(|token| {
            ontocode_windows_sandbox::WindowsSandboxCancellationToken::new(move || {
                token.is_cancelled()
            })
        });
    ...
    let spawn_res = tokio::task::spawn_blocking(move || {
        if use_elevated {
            run_windows_sandbox_capture_for_permission_profile_elevated(
                ontocode_windows_sandbox::ElevatedSandboxProfileCaptureRequest {
...
pub(crate) fn resolve_windows_restricted_token_filesystem_overrides(
    ...
    let additional_deny_read_paths = ontocode_windows_sandbox::resolve_windows_deny_read_paths(
        &file_system_sandbox_policy,
        sandbox_policy_cwd,
    )?;
...
pub(crate) fn resolve_windows_elevated_filesystem_overrides(
    ...
    let split_readable_roots: Vec<PathBuf> = file_system_sandbox_policy
        ...
        .into_iter()
        .map(ontocode_utils_absolute_path::AbsolutePathBuf::into_path_buf)
        .map(&normalize_path)
        .collect();
...
fn has_reopened_writable_descendant(
    writable_roots: &[ontocode_protocol::protocol::WritableRoot],
) -> bool {
            ...
            match outcome {
                ...
                Some(ExecExpirationOutcome::Cancelled) => {
                    ...
                    let should_escalate = if let Some(process_group_id) = process_group_id {
                        ontocode_utils_pty::process_group::terminate_process_group(process_group_id)?
                    } else {
                        false
                    };
                    ...
                    .await
                    {
                        Ok(status) => {
                            ...
                            {
                                ontocode_utils_pty::process_group::kill_process_group(
                                    process_group_id,
                                )?;
                            }
                        }
