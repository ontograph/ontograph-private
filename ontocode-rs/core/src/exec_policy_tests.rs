...
use ontocode_config::RequirementsExecPolicy;
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        ontocode_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        /*justification*/ None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(ontocode_config::Sourced::new(
            ontocode_config::RequirementsExecPolicy::new(requirements_exec_policy),
            ontocode_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
...
async fn ignore_user_project_rules_keeps_system_policy_files() {
    ...
    .expect("write policy file");
    let config_file =
        AbsolutePathBuf::from_absolute_path(config_dir.join(ontocode_config::CONFIG_TOML_FILE))
