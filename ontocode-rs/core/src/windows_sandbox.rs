...
pub fn sandbox_setup_is_complete(codex_home: &Path) -> bool {
    ontocode_windows_sandbox::sandbox_setup_is_complete(codex_home)
}
...
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = ontocode_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = ontocode_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if ontocode_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            ontocode_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
...
pub fn run_elevated_setup(
    ...
    let permissions =
        ontocode_windows_sandbox::ResolvedWindowsSandboxPermissions::try_from_permission_profile_for_workspace_roots(
            permission_profile,
            workspace_roots,
        )?;
    ontocode_windows_sandbox::run_elevated_setup(
        ontocode_windows_sandbox::SandboxSetupRequest {
        ...
        ontocode_windows_sandbox::SetupRootOverrides::default(),
    )
}

#[cfg(target_os = "windows")]
pub fn run_elevated_provisioning_setup(codex_home: &Path, real_user: &str) -> anyhow::Result<()> {
    ontocode_windows_sandbox::run_elevated_provisioning_setup(codex_home, real_user)
}
...
pub fn run_legacy_setup_preflight(
    ...
    ontocode_windows_sandbox::run_windows_sandbox_legacy_preflight(
        permission_profile,
        workspace_roots,
        codex_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    ontocode_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        permission_profile,
        workspace_roots,
        command_cwd,
        env_map,
