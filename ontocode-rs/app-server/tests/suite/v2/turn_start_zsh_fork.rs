...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(ontocode_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(ontocode_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(ontocode_app_server_protocol::AskForApproval::UnlessTrusted),
...
fn create_test_package_app_server(codex_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    ...
    let app_server = bin_dir.join("ontocode-app-server");
    copy_with_permissions(
        &ontocode_utils_cargo_bin::cargo_bin("ontocode-app-server")?,
        &app_server,
    )?;
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = ontocode_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("ontocode-rs/app-server/tests/suite/zsh");
