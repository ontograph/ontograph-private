...
use ontocode_app_server_protocol::ChatgptAuthTokensRefreshReason;
use ontocode_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ontocode_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<ontocode_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(ontocode_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            client_user_message_id: None,
            input: vec![ontocode_app_server_protocol::UserInput::Text {
...
async fn login_account_gemini_start_can_be_cancelled() -> Result<()> {
    ...
    assert!(auth_url.contains("redirect_uri=http%3A%2F%2Flocalhost"));
    ...
    assert!(
        maybe_updated.is_err(),
        "account/updated should not be emitted when Gemini login is cancelled"
    );
    Ok(())
}
