...
#[tokio::test]
async fn thread_settings_update_emits_model_provider() -> Result<()> {
    let server = create_mock_responses_server_sequence_unchecked(Vec::new()).await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    append_alternate_provider_config(codex_home.path(), &server.uri())?;
    ...
    assert_eq!(updated.thread_id, thread.id);
    assert_eq!(
        updated.thread_settings.model_provider,
        "alternate_provider".to_string()
    );
    Ok(())
}
...
fn append_alternate_provider_config(
...
name = "Alternate provider for test"
