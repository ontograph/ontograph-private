...
use ontocode_protocol::protocol::SessionSource as CoreSessionSource;
...
async fn list_threads_with_sort(
    ...
    let request_id = mcp
        .send_thread_list_request(ontocode_app_server_protocol::ThreadListParams {
            cursor,
            limit,
            sort_key,
            sort_direction: None,
...
"#,
    ...
    let state_db =
        ontocode_state::StateRuntime::init(codex_home.path().to_path_buf(), "mock_provider".into())
            .await?;
    state_db
        .mark_backfill_complete(/*last_watermark*/ None)
        .await?;
    let rollout_config = ontocode_rollout::RolloutConfig {
    ...
    let repaired_page = ontocode_core::RolloutRecorder::list_threads(
        Some(state_db.clone()),
        &rollout_config,
        /*page_size*/ 10,
        /*cursor*/ None,
        ontocode_core::ThreadSortKey::CreatedAt,
        ontocode_core::SortDirection::Desc,
        &[],
...
async fn thread_search_returns_content_matches() -> Result<()> {
    ...
    let mut mcp = init_mcp(codex_home.path()).await?;
    let request_id = mcp
        .send_thread_search_request(ontocode_app_server_protocol::ThreadSearchParams {
            cursor: None,
            limit: Some(10),
            sort_key: None,
            sort_direction: None,
