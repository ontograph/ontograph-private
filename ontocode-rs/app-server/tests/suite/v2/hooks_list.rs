...
use ontocode_app_server_protocol::HookEventName;
use ontocode_app_server_protocol::HookHandlerType;
use ontocode_app_server_protocol::HookMetadata;
use ontocode_app_server_protocol::HookSource;
use ontocode_app_server_protocol::HookTrustStatus;
...
fn command_hook_hash(
    ...
    let Ok(value) = ontocode_config::TomlValue::try_from(identity) else {
        unreachable!("normalized hook identity should serialize to TOML");
    };
    ontocode_config::version_for_toml(&value)
}
...
async fn hooks_list_uses_root_repo_hooks_for_linked_worktrees() -> Result<()> {
    ...
    .await??;
    let _: ontocode_app_server_protocol::ConfigWriteResponse = to_response(response)?;
