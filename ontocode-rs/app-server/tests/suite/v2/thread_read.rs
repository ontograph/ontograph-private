...
use ontocode_app_server::in_process;
...
use ontocode_core::ARCHIVED_SESSIONS_SUBDIR;
...
use ontocode_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use ontocode_thread_store::AppendThreadItemsParams;
use ontocode_thread_store::CreateThreadParams;
use ontocode_thread_store::InMemoryThreadStore;
use ontocode_thread_store::ThreadMetadataPatch;
use ontocode_thread_store::ThreadPersistenceMetadata;
use ontocode_thread_store::ThreadStore;
use ontocode_thread_store::UpdateThreadMetadataParams;
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let codex_home = TempDir::new()?;
    let thread_id =
        ontocode_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(ontocode_config::NoopThreadConfigLoader),
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "ontocode-app-server-tests".to_string(),
                title: None,
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = ontocode_protocol::ThreadId::from_string(&thread.id)?;
    store
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let codex_home = TempDir::new()?;
    let thread_id =
        ontocode_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(codex_home.path(), &store_id)?;
    let store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(ontocode_app_server_protocol::TurnsPage::from(expected_page))
    );
...
) -> anyhow::Result<ontocode_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
fn turn_user_texts(turns: &[ontocode_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[ontocode_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
async fn seed_pathless_store_thread(
    ...
    thread_id: ontocode_protocol::ThreadId,
) -> Result<()> {
    store
