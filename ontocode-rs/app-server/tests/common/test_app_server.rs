...
use ontocode_app_server_protocol::ClientNotification;
...
use ontocode_app_server_protocol::ConfigValueWriteParams;
use ontocode_app_server_protocol::ExperimentalFeatureListParams;
use ontocode_app_server_protocol::FeedbackUploadParams;
use ontocode_app_server_protocol::FsCopyParams;
use ontocode_app_server_protocol::FsCreateDirectoryParams;
use ontocode_app_server_protocol::FsGetMetadataParams;
use ontocode_app_server_protocol::FsReadDirectoryParams;
use ontocode_app_server_protocol::FsReadFileParams;
use ontocode_app_server_protocol::FsRemoveParams;
use ontocode_app_server_protocol::FsUnwatchParams;
use ontocode_app_server_protocol::FsWatchParams;
use ontocode_app_server_protocol::FsWriteFileParams;
...
use ontocode_app_server_protocol::InitializeCapabilities;
use ontocode_app_server_protocol::InitializeParams;
use ontocode_app_server_protocol::JSONRPCError;
use ontocode_app_server_protocol::JSONRPCErrorError;
use ontocode_app_server_protocol::JSONRPCMessage;
use ontocode_app_server_protocol::JSONRPCNotification;
use ontocode_app_server_protocol::JSONRPCRequest;
use ontocode_app_server_protocol::JSONRPCResponse;
...
use ontocode_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use ontocode_app_server_protocol::ProcessKillParams;
use ontocode_app_server_protocol::ProcessResizePtyParams;
use ontocode_app_server_protocol::ProcessSpawnParams;
use ontocode_app_server_protocol::ProcessWriteStdinParams;
use ontocode_app_server_protocol::RemoteControlClientsListParams;
use ontocode_app_server_protocol::RemoteControlClientsRevokeParams;
use ontocode_app_server_protocol::RemoteControlPairingStartParams;
...
use ontocode_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use ontocode_login::default_client::CODEX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
pub const DEFAULT_CLIENT_NAME: &str = "ontocode-app-server-tests";
...
impl TestAppServer {
    ...
    async fn new_with_env_and_args(
        ...
        let program = ontocode_utils_cargo_bin::cargo_bin("ontocode-app-server")
            .context("should find binary for ontocode-app-server")?;
    ...
    async fn new_with_program_env_and_args(
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("ontocode-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: ontocode_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
    ...
    /// Send an `account/login/start` JSON-RPC request for Gemini login.
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `ontocode-app-server` child process.
        //
