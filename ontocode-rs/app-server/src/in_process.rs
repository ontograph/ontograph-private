...
//!
//! # Relationship to `ontocode-app-server-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `ontocode-app-server-client`,
...
pub use ontocode_rollout::StateDbHandle;
pub use ontocode_state::log_db::LogDbLayer;
use tokio::sync::mpsc;
...
/// through `ontocode-app-server-client`, which adds worker-task buffering,
...
mod tests {
    ...
    async fn start_test_client_with_capacity(
        ...
        let state_db = ontocode_rollout::state_db::try_init(config.as_ref())
            .await
