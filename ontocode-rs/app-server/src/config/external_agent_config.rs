use ontocode_config::types::PluginConfig;
...
use ontocode_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use ontocode_core_plugins::marketplace::find_marketplace_manifest_path;
use ontocode_core_plugins::marketplace_add::MarketplaceAddRequest;
use ontocode_core_plugins::marketplace_add::add_marketplace;
use ontocode_core_plugins::marketplace_add::is_local_marketplace_source;
use ontocode_external_agent_migration::build_mcp_config_from_external;
use ontocode_external_agent_migration::count_missing_commands;
use ontocode_external_agent_migration::count_missing_subagents;
use ontocode_external_agent_migration::hook_migration_event_names;
use ontocode_external_agent_migration::import_commands;
use ontocode_external_agent_migration::import_hooks;
use ontocode_external_agent_migration::import_subagents;
use ontocode_external_agent_migration::missing_command_names;
use ontocode_external_agent_migration::missing_subagent_names;
use ontocode_external_agent_sessions::ExternalAgentSessionMigration;
use ontocode_external_agent_sessions::detect_recent_sessions;
use ontocode_plugin::PluginId;
use ontocode_protocol::protocol::Product;
...
impl ExternalAgentConfigService {
    ...
    fn import_mcp_oauth_credentials(&self, cwd: Option<&Path>) -> io::Result<()> {
        ...
        use ontocode_external_agent_migration::claude_oauth_import::parse_claude_oauth_import_sample;
        use ontocode_rmcp_client::oauth::OAuthBearerTokenParts;
        use ontocode_rmcp_client::oauth::StoredOAuthTokens;
        use ontocode_rmcp_client::oauth::save_oauth_tokens;
    ...
    async fn detect_migrations(
        ...
        if target_claude_mcp_json.exists()
            && let Ok(raw) = std::fs::read_to_string(&target_claude_mcp_json)
                && let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&raw)
                    && !ontocode_external_agent_migration::claude_oauth_import::parse_claude_oauth_import_sample(&parsed, false, true).credentials.is_empty() {
                        items.push(ExternalAgentConfigMigrationItem {
...
fn emit_migration_metric(
    ...
    let Some(metrics) = ontocode_otel::global() else {
        return;
    };
