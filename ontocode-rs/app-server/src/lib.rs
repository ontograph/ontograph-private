...
use ontocode_analytics::AppServerRpcTransport;
...
use ontocode_app_server_protocol::TextPosition as AppTextPosition;
use ontocode_app_server_protocol::TextRange as AppTextRange;
use ontocode_config::ConfigLoadError;
use ontocode_config::TextRange as CoreTextRange;
use ontocode_core::ExecPolicyError;
...
use ontocode_rollout::state_db as rollout_state_db;
...
const OTEL_SERVICE_NAME: &str = "ontocode-app-server";
...
pub async fn run_main_with_transport_options(
    ...
    let otel = ontocode_core::otel_init::build_provider(
        &config,
    ...
    ontocode_core::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    ontocode_core::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
    ...
    if should_run_personality_migration {
        ...
        match effective_toml.try_into() {
            Ok(config_toml) => {
                match ontocode_core::personality_migration::maybe_migrate_personality(
                    &config.codex_home,
                    &config_toml,
                    state_db.clone(),
                )
                .await
                {
                    Ok(ontocode_core::personality_migration::PersonalityMigrationStatus::Applied) => {
                    ...
                    Ok(
                        ontocode_core::personality_migration::PersonalityMigrationStatus::SkippedMarker
                        | ontocode_core::personality_migration::PersonalityMigrationStatus::SkippedExplicitPersonality
                        | ontocode_core::personality_migration::PersonalityMigrationStatus::SkippedNoSessions,
                    ) => {}
                    Err(err) => {
