...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: ontocode_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                ontocode_protocol::protocol::SandboxPolicy::ReadOnly { .. }
            ) | (
                SandboxMode::WorkspaceWrite,
                ontocode_protocol::protocol::SandboxPolicy::WorkspaceWrite { .. }
            ) | (
                SandboxMode::DangerFullAccess,
                ontocode_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
                ontocode_protocol::protocol::SandboxPolicy::ExternalSandbox { .. }
            )
        );
        if !sandbox_matches {
            mismatch_details.push(format!(
...
fn validate_dynamic_tools(tools: &[ApiDynamicToolSpec]) -> Result<(), String> {
    ...
    for tool in tools {
        ...
        if let Err(err) = ontocode_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    async fn instruction_sources_from_config(config: &Config) -> Vec<AbsolutePathBuf> {
        ontocode_core::AgentsMdManager::new(config)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<ontocode_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<ontocode_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                ontocode_core::config::set_project_trust_level(
        ...
        let NewThread {
            ...
            .start_thread_with_options(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(ontocode_app_server_protocol::ThreadStartSource::Startup)
                {
                    ontocode_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    ontocode_app_server_protocol::ThreadStartSource::Clear => {
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<ontocode_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<ontocode_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(ontocode_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(ontocode_app_server_protocol::ApprovalsReviewer::to_core),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = ontocode_core::util::normalize_thread_name(&name) else {
    ...
    async fn thread_turns_list_response_inner(
        ...
        let items = self
            ...
            .map_err(thread_read_view_error)?;
        let mut turns = self
            .reconstruct_thread_turns_from_history_items(thread_uuid, &items)
            .await;
        apply_thread_turns_items_view(&mut turns, items_view.unwrap_or(TurnItemsView::Summary));
    ...
    async fn thread_turns_items_list_response_inner(
        ...
        let items = self
            .load_thread_turns_items_list_history(thread_uuid)
            .await
            .map_err(thread_read_view_error)?;
        let turns = self
            .reconstruct_thread_turns_from_history_items(thread_uuid, &items)
        ...
        let turn = turns
            ...
            .ok_or_else(|| {
                invalid_request(format!("turn not found in thread {thread_id}: {turn_id}"))
            })?;
        let page = paginate_thread_turn_items(
            turn.items,
            cursor.as_deref(),
            limit,
    ...
    async fn load_thread_turns_items_list_history(
        &self,
        thread_id: ThreadId,
    ) -> Result<Vec<RolloutItem>, ThreadReadViewError> {
        match self
        ...
        if config_snapshot.ephemeral {
            return Err(ThreadReadViewError::InvalidRequest(
                "ephemeral threads do not support thread/turns/items/list".to_string(),
            ));
        }
        ...
        thread
            ...
            .await
            .map(|history| history.items)
            .map_err(|err| thread_turns_items_list_history_load_error(thread_id, err))
    }

    async fn reconstruct_thread_turns_from_history_items(
        &self,
        thread_id: ThreadId,
        items: &[RolloutItem],
    ) -> Vec<Turn> {
        // Rollback and compaction can still rewrite earlier turns, so rebuild the
        // turn list from rollout history on every read.
        let loaded_thread = self.thread_manager.get_thread(thread_id).await.ok();
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(ontocode_core::util::normalize_thread_name);
        let history_items = source_thread
            .history
            .as_ref()
            .map(|history| history.items.clone())
...
const THREAD_TURN_ITEMS_DEFAULT_LIMIT: usize = 100;
const THREAD_TURN_ITEMS_MAX_LIMIT: usize = 100;
...
fn paginate_thread_turn_items(
    items: Vec<ThreadItem>,
    cursor: Option<&str>,
    limit: Option<u32>,
    sort_direction: SortDirection,
) -> Result<ThreadTurnItemsPage, JSONRPCErrorError> {
    if items.is_empty() {
        return Ok(ThreadTurnItemsPage {
            items: Vec::new(),
            next_cursor: None,
            backwards_cursor: None,
        });
    ...
    let anchor = cursor.map(parse_thread_turn_items_cursor).transpose()?;
    let page_size = limit
        .map(|value| value as usize)
        .unwrap_or(THREAD_TURN_ITEMS_DEFAULT_LIMIT)
        .clamp(1, THREAD_TURN_ITEMS_MAX_LIMIT);
    ...
    let backwards_cursor = keyed_items
        .first()
        .map(|(_, item)| {
            serialize_thread_turn_items_cursor(item.id(), /*include_anchor*/ true)
        })
        .transpose()?;
    let next_cursor = if more_items_available {
        keyed_items
            .last()
            .map(|(_, item)| {
                serialize_thread_turn_items_cursor(item.id(), /*include_anchor*/ false)
            })
            .transpose()?
    } else {
        None
...
fn serialize_thread_turn_items_cursor(
    item_id: &str,
    include_anchor: bool,
) -> Result<String, JSONRPCErrorError> {
    serde_json::to_string(&ThreadTurnsItemsCursor {
...
fn parse_thread_turn_items_cursor(
    cursor: &str,
) -> Result<ThreadTurnsItemsCursor, JSONRPCErrorError> {
    serde_json::from_str(cursor).map_err(|_| invalid_request(format!("invalid cursor: {cursor}")))
}
...
) -> Result<ontocode_app_server_protocol::TurnsPage, JSONRPCErrorError> {
...
fn thread_turns_items_list_history_load_error(
    thread_id: ThreadId,
    err: ThreadStoreError,
) -> ThreadReadViewError {
    match err {
        ...
        {
            ThreadReadViewError::InvalidRequest(format!(
                "thread {thread_id} is not materialized yet; thread/turns/items/list is unavailable before first user message"
            ))
        }
...
) -> (Thread, Option<ontocode_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<ontocode_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(ontocode_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        ...
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match ontocode_core::parse_turn_item(item) {
                Some(ontocode_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            ontocode_protocol::config_types::SandboxMode::WorkspaceWrite
                | ontocode_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &ontocode_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        ontocode_protocol::models::PermissionProfile::Disabled
        | ontocode_protocol::models::PermissionProfile::External { .. } => true,
        ontocode_protocol::models::PermissionProfile::Managed { .. } => profile
