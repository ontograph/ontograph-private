...
use ontocode_app_server_protocol::PluginAvailability;
use ontocode_app_server_protocol::PluginInstallPolicy;
use ontocode_app_server_protocol::PluginSharePrincipalRole;
use ontocode_app_server_protocol::PluginShareTargetRole;
use ontocode_config::types::McpServerConfig;
use ontocode_core_plugins::OPENAI_CURATED_MARKETPLACE_NAME;
use ontocode_core_plugins::remote::REMOTE_GLOBAL_MARKETPLACE_NAME;
use ontocode_core_plugins::remote::RemotePluginScope;
use ontocode_core_plugins::remote::is_valid_remote_plugin_id;
use ontocode_core_plugins::remote::validate_remote_plugin_id;
use ontocode_mcp::McpOAuthLoginSupport;
use ontocode_mcp::oauth_login_support;
use ontocode_mcp::should_retry_without_scopes;
use ontocode_rmcp_client::perform_oauth_login_silent;
...
fn plugin_skills_to_info(
    skills: &[ontocode_core::skills::SkillMetadata],
    ...
    skills
        ...
        .map(|skill| SkillSummary {
            ...
            interface: skill.interface.clone().map(|interface| {
                ontocode_app_server_protocol::SkillInterface {
...
fn load_shared_plugin_ids_by_local_path(
    ...
    ontocode_core_plugins::remote::load_plugin_share_remote_ids_by_local_path(
...
fn convert_configured_marketplace_plugin_to_plugin_summary(
    plugin: ontocode_core_plugins::ConfiguredMarketplacePlugin,
    shared_plugin_ids_by_local_path: &std::collections::BTreeMap<AbsolutePathBuf, String>,
) -> PluginSummary {
...
) -> ontocode_core_plugins::remote::RemotePluginShareDiscoverability {
    match discoverability {
        PluginShareDiscoverability::Listed => {
            ontocode_core_plugins::remote::RemotePluginShareDiscoverability::Listed
        }
        PluginShareDiscoverability::Unlisted => {
            ontocode_core_plugins::remote::RemotePluginShareDiscoverability::Unlisted
        }
        PluginShareDiscoverability::Private => {
            ontocode_core_plugins::remote::RemotePluginShareDiscoverability::Private
        }
    }
}
...
) -> ontocode_core_plugins::remote::RemotePluginShareUpdateDiscoverability {
    match discoverability {
        PluginShareUpdateDiscoverability::Unlisted => {
            ontocode_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Unlisted
        }
        PluginShareUpdateDiscoverability::Private => {
            ontocode_core_plugins::remote::RemotePluginShareUpdateDiscoverability::Private
        }
    }
}
...
) -> ontocode_core_plugins::remote::RemotePluginShareTargetRole {
    match role {
        PluginShareTargetRole::Reader => {
            ontocode_core_plugins::remote::RemotePluginShareTargetRole::Reader
        }
        PluginShareTargetRole::Editor => {
            ontocode_core_plugins::remote::RemotePluginShareTargetRole::Editor
        }
    }
}

fn plugin_share_principal_role_from_remote(
    role: ontocode_core_plugins::remote::RemotePluginSharePrincipalRole,
) -> PluginSharePrincipalRole {
    match role {
        ontocode_core_plugins::remote::RemotePluginSharePrincipalRole::Reader => {
            PluginSharePrincipalRole::Reader
        }
        ontocode_core_plugins::remote::RemotePluginSharePrincipalRole::Editor => {
            PluginSharePrincipalRole::Editor
        }
        ontocode_core_plugins::remote::RemotePluginSharePrincipalRole::Owner => {
...
) -> Vec<ontocode_core_plugins::remote::RemotePluginShareTarget> {
    targets
        .into_iter()
        .map(
            |target| ontocode_core_plugins::remote::RemotePluginShareTarget {
                principal_type: match target.principal_type {
                    PluginSharePrincipalType::User => {
                        ontocode_core_plugins::remote::RemotePluginSharePrincipalType::User
                    }
                    PluginSharePrincipalType::Group => {
                        ontocode_core_plugins::remote::RemotePluginSharePrincipalType::Group
                    }
                    PluginSharePrincipalType::Workspace => {
                        ontocode_core_plugins::remote::RemotePluginSharePrincipalType::Workspace
                    }
                },
...
fn plugin_share_principal_from_remote(
    principal: ontocode_core_plugins::remote::RemotePluginSharePrincipal,
...
impl PluginRequestProcessor {
    ...
    async fn plugin_list_response(
            ...
            match tokio::task::spawn_blocking(move || {
                ...
                Ok::<
                    (
                        ...
                        Vec<ontocode_app_server_protocol::MarketplaceLoadErrorInfo>,
                    ),
                    MarketplaceError,
                >((
                    outcome
                    ...
                    outcome
                        ...
                        .into_iter()
                        .map(
                            |err| ontocode_app_server_protocol::MarketplaceLoadErrorInfo {
        ...
        if include_vertical && !config.features.enabled(Feature::RemotePlugin) {
            ...
            match ontocode_core_plugins::remote::fetch_openai_curated_remote_collection_marketplace(
        ...
        if !remote_sources.is_empty() {
            ...
            match ontocode_core_plugins::remote::fetch_remote_marketplaces(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_sources,
    ...
    async fn load_local_installed_and_suggested_plugins(
        &self,
        plugins_manager: Arc<ontocode_core_plugins::PluginsManager>,
        config: &Config,
        plugins_input: &ontocode_core_plugins::PluginsConfigInput,
    ...
    async fn plugin_read_response(
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                let share_context = match share_context {
                    Some(context) => {
                        ...
                        match ontocode_core_plugins::remote::fetch_remote_plugin_share_context(
                            &remote_plugin_service_config,
                            auth.as_ref(),
                ...
                PluginDetail {
                    ...
                    hooks: outcome
                        ...
                        .hooks
                        .into_iter()
                        .map(|hook| ontocode_app_server_protocol::PluginHookSummary {
                            key: hook.key,
            ...
            Err(remote_marketplace_name) => {
                ...
                let remote_detail = ontocode_core_plugins::remote::fetch_remote_plugin_detail(
                    &remote_plugin_service_config,
                    auth.as_ref(),
                    &remote_marketplace_name,
                    &plugin_name,
                ...
                let plugin_apps = remote_detail
                    ...
                    .iter()
                    .cloned()
                    .map(ontocode_plugin::AppConnectorId)
                    .collect::<Vec<_>>();
                let environment_manager = self.thread_manager.environment_manager();
                let app_summaries = load_plugin_app_summaries(
                    &config,
    ...
    async fn plugin_skill_read_response(
        ...
        let remote_skill_detail = ontocode_core_plugins::remote::fetch_remote_plugin_skill_detail(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_marketplace_name,
            &remote_plugin_id,
    ...
    async fn plugin_share_save_response(
        ...
        let access_policy = ontocode_core_plugins::remote::RemotePluginShareAccessPolicy {
        ...
        let result = ontocode_core_plugins::remote::save_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codex_home.as_path(),
            &plugin_path,
    ...
    async fn plugin_share_update_targets_response(
        ...
        let result = ontocode_core_plugins::remote::update_remote_plugin_share_targets(
            &remote_plugin_service_config,
            auth.as_ref(),
            &remote_plugin_id,
    ...
    async fn plugin_share_list_response(
        ...
        let data = ontocode_core_plugins::remote::list_remote_plugin_shares(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codex_home.as_path(),
        )
    ...
    async fn plugin_share_checkout_response(
        ...
        let result = ontocode_core_plugins::remote::checkout_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codex_home.as_path(),
            &remote_plugin_id,
    ...
    async fn plugin_share_delete_response(
        ...
        ontocode_core_plugins::remote::delete_remote_plugin_share(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codex_home.as_path(),
            &remote_plugin_id,
    ...
    async fn remote_plugin_install_response(
        ...
        let remote_detail =
            ontocode_core_plugins::remote::fetch_remote_plugin_detail_with_download_urls(
                &remote_plugin_service_config,
                auth.as_ref(),
                &remote_marketplace_name,
                &remote_plugin_id,
        ...
        let _remote_plugin_cache_mutation =
            ontocode_core_plugins::remote::mark_remote_plugin_cache_mutation_in_flight(
                config.codex_home.as_path(),
                &actual_remote_marketplace_name,
                &remote_detail.summary.name,
            );
        let validated_bundle = ontocode_core_plugins::remote_bundle::validate_remote_plugin_bundle(
            &remote_plugin_id,
            &actual_remote_marketplace_name,
            &remote_detail.summary.name,
        ...
        let result =
            ontocode_core_plugins::remote_bundle::download_and_install_remote_plugin_bundle(
        ...
        ontocode_core_plugins::remote::install_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            &actual_remote_marketplace_name,
            &remote_plugin_id,
    ...
    async fn plugin_apps_needing_auth_for_install(
        ...
        plugin_id: &str,
        plugin_apps: &[ontocode_plugin::AppConnectorId],
    ) -> Vec<AppSummary> {
    ...
    async fn plugin_uninstall_response(
        ...
        if ontocode_plugin::PluginId::parse(&plugin_id).is_err()
    ...
    async fn remote_plugin_uninstall_response(
        ...
        let uninstall_result = ontocode_core_plugins::remote::uninstall_remote_plugin(
            &remote_plugin_service_config,
            auth.as_ref(),
            config.codex_home.to_path_buf(),
            &plugin_id,
...
fn remote_plugin_share_discoverability_to_info(
    discoverability: ontocode_core_plugins::remote::RemotePluginShareDiscoverability,
...
fn remote_plugin_bundle_install_error_to_jsonrpc(
    err: ontocode_core_plugins::remote_bundle::RemotePluginBundleInstallError,
) -> JSONRPCErrorError {
