...
use ontocode_feedback::WINDOWS_SANDBOX_LOG_ATTACHMENT_FILENAME;
                    ...
                    .await
                {
                    ...
                    Err(err) => {
                        ...
                        if let Some(state_db_ctx) = state_db_ctx.as_ref() {
                            for status in [
                                ontocode_state::DirectionalThreadSpawnEdgeStatus::Open,
                                ontocode_state::DirectionalThreadSpawnEdgeStatus::Closed,
                            ] {
                                match state_db_ctx
...
fn windows_sandbox_log_attachment(codex_home: &Path) -> Option<FeedbackAttachmentPath> {
    let sandbox_log_path =
        ontocode_windows_sandbox::current_log_file_path_for_codex_home(codex_home);
    sandbox_log_path
        .is_file()
...
mod tests {
    ...
    fn windows_sandbox_log_attachment_uses_current_log() {
        ...
        let sandbox_dir = ontocode_windows_sandbox::sandbox_dir(codex_home.path());
        std::fs::create_dir_all(&sandbox_dir).expect("create sandbox dir");
        let sandbox_log_path =
            ontocode_windows_sandbox::current_log_file_path_for_codex_home(codex_home.path());
