...
) -> Vec<ontocode_app_server_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            ontocode_app_server_protocol::SkillMetadata {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    ontocode_app_server_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| ontocode_app_server_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
fn hooks_to_info(hooks: &[ontocode_hooks::HookListEntry]) -> Vec<HookMetadata> {
    hooks
        .iter()
...
fn errors_to_info(
    errors: &[ontocode_core::skills::SkillError],
) -> Vec<ontocode_app_server_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| ontocode_app_server_protocol::SkillErrorInfo {
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                ontocode_app_server_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![ontocode_app_server_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
                    ...
                    let skills_input = ontocode_core::skills::SkillsLoadInput::new(
                        cwd_abs.clone(),
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(ontocode_app_server_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
                        errors: vec![ontocode_app_server_protocol::HookErrorInfo {
                            path: error_path,
            ...
            let hooks = ontocode_hooks::list_hooks(ontocode_hooks::HooksConfig {
