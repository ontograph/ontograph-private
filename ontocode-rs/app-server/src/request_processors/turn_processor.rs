use super::*;
use ontocode_protocol::protocol::AdditionalContextEntry as CoreAdditionalContextEntry;
use ontocode_protocol::protocol::AdditionalContextKind as CoreAdditionalContextKind;
...
struct ThreadSettingsBuildParams {
    ...
    sandbox_policy: Option<ontocode_app_server_protocol::SandboxPolicy>,
...
impl TurnRequestProcessor {
    ...
    fn review_request_from_target(
        ...
        let hint = ontocode_core::review_prompts::user_facing_hint(&core_target);
    ...
    async fn turn_start_inner(
        ...
        if turn_has_input {
            ...
            ontocode_memories_write::start_memories_startup_task(
    ...
    async fn build_thread_settings_overrides(
    ...
    ) -> Result<ontocode_protocol::protocol::ThreadSettingsOverrides, JSONRPCErrorError> {
        ...
        let approval_policy =
            approval_policy.map(ontocode_app_server_protocol::AskForApproval::to_core);
        let approvals_reviewer =
            approvals_reviewer.map(ontocode_app_server_protocol::ApprovalsReviewer::to_core);
        ...
        Ok(ontocode_protocol::protocol::ThreadSettingsOverrides {
            cwd,
    ...
    async fn thread_settings_update_inner(
        ...
        if thread_settings != ontocode_protocol::protocol::ThreadSettingsOverrides::default() {
            self.submit_core_op(
                request_id,
                thread.as_ref(),
                ...
                let (message, data, error_type) = match err {
                    ...
                    SteerInputError::ActiveTurnNotSteerable { turn_kind } => {
                        let (message, turn_steer_error) = match turn_kind {
                            ontocode_protocol::protocol::NonSteerableTurnKind::Review => (
                                "cannot steer a review turn".to_string(),
                                TurnSteerRequestError::NonSteerableReview,
                            ),
                            ontocode_protocol::protocol::NonSteerableTurnKind::Compact => (
