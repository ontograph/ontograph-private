...
fn extract_conversation_summary(
    ...
    let preview = head
        ...
        .find_map(|item| match ontocode_core::parse_turn_item(&item) {
...
pub(super) fn with_thread_spawn_agent_metadata(
    source: ontocode_protocol::protocol::SessionSource,
    agent_nickname: Option<String>,
    agent_role: Option<String>,
) -> ontocode_protocol::protocol::SessionSource {
    if agent_nickname.is_none() && agent_role.is_none() {
        return source;
    }

    match source {
        ontocode_protocol::protocol::SessionSource::SubAgent(
            ontocode_protocol::protocol::SubAgentSource::ThreadSpawn {
                parent_thread_id,
                depth,
                agent_path,
...
) -> Option<ontocode_app_server_protocol::ActivePermissionProfile> {
    active_permission_profile.map(Into::into)
}

pub(crate) fn thread_response_sandbox_policy(
    permission_profile: &ontocode_protocol::models::PermissionProfile,
    cwd: &Path,
) -> ontocode_app_server_protocol::SandboxPolicy {
    let sandbox_policy = ontocode_sandboxing::compatibility_sandbox_policy_for_permission_profile(
        permission_profile,
        cwd,
    );
    sandbox_policy.into()
...
pub(crate) fn thread_settings_from_core_snapshot(
    snapshot: ontocode_protocol::protocol::ThreadSettingsSnapshot,
) -> ThreadSettings {
    ThreadSettings {
        sandbox_policy: thread_response_sandbox_policy(
