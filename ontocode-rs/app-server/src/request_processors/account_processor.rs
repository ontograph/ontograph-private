...
impl AccountRequestProcessor {
    ...
    async fn login_gemini_v2(&self, request_id: ConnectionRequestId) {
        let result = self.login_gemini_response().await;
        self.outgoing.send_result(request_id, result).await;
    }

    async fn login_gemini_response(&self) -> Result<LoginAccountResponse, JSONRPCErrorError> {
        let opts = GeminiLoginServerOptions {
            codex_home: self.config.codex_home.to_path_buf(),
            client_id: GEMINI_OAUTH_CLIENT_ID.to_string(),
            credential_id: "gemini".to_string(),
            port: 0,
            open_browser: false,
            cli_auth_credentials_store_mode: self.config.cli_auth_credentials_store_mode,
            ..Default::default()
        };
        let server = run_gemini_login_server(opts)
        ...
        Ok(LoginAccountResponse::Gemini {
