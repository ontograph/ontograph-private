...
use ontocode_app_server_protocol::CommandMigration;
...
use ontocode_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
...
use ontocode_app_server_protocol::ExternalAgentConfigMigrationItemType;
use ontocode_app_server_protocol::HookMigration;
use ontocode_app_server_protocol::JSONRPCErrorError;
use ontocode_app_server_protocol::McpServerMigration;
use ontocode_app_server_protocol::MigrationDetails;
use ontocode_app_server_protocol::PluginsMigration;
...
use ontocode_external_agent_sessions::ExternalAgentSessionMigration as CoreSessionMigration;
use ontocode_external_agent_sessions::ImportedExternalAgentSession;
use ontocode_external_agent_sessions::PendingSessionImport;
use ontocode_external_agent_sessions::prepare_validated_session_imports;
use ontocode_external_agent_sessions::record_imported_session;
...
impl ExternalAgentConfigRequestProcessor {
                ...
                .map(|migration_item| ExternalAgentConfigMigrationItem {
                    ...
                    details: migration_item.details.map(|details| MigrationDetails {
                        ...
                        sessions: details
                            .sessions
                            .into_iter()
                            .map(|session| ontocode_app_server_protocol::SessionMigration {
                                path: session.path,
                                cwd: session.cwd,
                                title: session.title,
                            })
                        ...
                        subagents: details
                            .subagents
                            .into_iter()
                            .map(|subagent| ontocode_app_server_protocol::SubagentMigration {
                                name: subagent.name,
                            })
                            .collect(),
                        commands: details
    ...
    async fn import_external_agent_session(
        ...
        if let Some(title) = title
            && let Some(name) = ontocode_core::util::normalize_thread_name(&title)
        {
            imported_thread
                .thread
