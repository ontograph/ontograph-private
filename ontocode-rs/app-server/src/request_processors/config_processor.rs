...
use ontocode_app_server_protocol::ComputerUseRequirements;
...
use ontocode_app_server_protocol::ConfigRequirements;
use ontocode_app_server_protocol::ConfigRequirementsReadResponse;
use ontocode_app_server_protocol::ConfigValueWriteParams;
use ontocode_app_server_protocol::ConfigWriteErrorCode;
use ontocode_app_server_protocol::ConfigWriteResponse;
use ontocode_app_server_protocol::ConfiguredHookHandler;
use ontocode_app_server_protocol::ConfiguredHookMatcherGroup;
use ontocode_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use ontocode_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use ontocode_app_server_protocol::JSONRPCErrorError;
use ontocode_app_server_protocol::ManagedHooksRequirements;
use ontocode_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use ontocode_app_server_protocol::NetworkDomainPermission;
use ontocode_app_server_protocol::NetworkRequirements;
use ontocode_app_server_protocol::NetworkUnixSocketPermission;
...
use ontocode_config::HookEventsToml;
use ontocode_config::HookHandlerConfig as CoreHookHandlerConfig;
use ontocode_config::ManagedHooksRequirementsToml;
use ontocode_config::MatcherGroup as CoreMatcherGroup;
use ontocode_config::ResidencyRequirement as CoreResidencyRequirement;
use ontocode_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use ontocode_core::ThreadManager;
use ontocode_features::canonical_feature_for_key;
use ontocode_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    async fn refresh_apps_list_after_experimental_feature_enablement_set(&self) {
        ...
        if !config.features.apps_enabled_for_auth(
            auth.as_ref()
                .is_some_and(ontocode_login::CodexAuth::uses_codex_backend),
        ) {
            return;
        }
    ...
    ) -> Result<ontocode_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = ontocode_core_plugins::toggles::collect_plugin_enabled_candidates(
    ...
    async fn emit_plugin_toggle_events(
        ...
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata = ontocode_core_plugins::loader::installed_plugin_telemetry_metadata(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(ontocode_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(ontocode_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            ontocode_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            ontocode_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: ontocode_config::ComputerUseRequirementsToml,
...
) -> ontocode_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => ontocode_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: ontocode_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(ontocode_config::NetworkDomainPermissionsToml::allowed_domains);
    ...
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(ontocode_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: ontocode_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        ontocode_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        ontocode_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: ontocode_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        ontocode_config::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        ontocode_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use ontocode_app_server_protocol::WindowsSandboxSetupMode;
    use ontocode_config::ComputerUseRequirementsToml;
    use ontocode_config::ConfigRequirementsToml;
    use ontocode_config::WindowsRequirementsToml;
