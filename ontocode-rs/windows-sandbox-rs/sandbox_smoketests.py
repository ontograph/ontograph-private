...
def _resolve_codex_cmd() -> List[str]:
    ...
    candidates = [
        ws_root / "target" / "debug" / "ontocode.exe",
        ws_root / "target" / "release" / "ontocode.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "ontocode.exe",
            cargo_base / "release" / "ontocode.exe",
        ])
    ...
    if shutil.which("ontocode"):
        return ["ontocode"]

    raise FileNotFoundError(
        "Ontocode CLI not found. Build it first, e.g.\n"
        "  cargo build -p ontocode-cli --bin ontocode --release\n"
        "or for debug:\n"
