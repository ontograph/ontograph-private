...
const SETUP_EXE_FILENAME: &str = "ontocode-windows-sandbox-setup.exe";
...
struct ElevationPayload {
    ...
    otel: Option<ontocode_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
pub fn run_elevated_setup(
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: ontocode_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
...
mod tests {
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let setup_exe = resources_dir.join("ontocode-windows-sandbox-setup.exe");
