...
async fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let child_task = tokio::task::spawn_blocking(move || -> anyhow::Result<ChildOutput> {
        ...
        let mut child = Command::new(ontocode_utils_cargo_bin::cargo_bin(
            "ontocode-stdio-to-uds",
        )?)
        .arg(&socket_path)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .context("failed to spawn ontocode-stdio-to-uds")?;
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                ...
                anyhow::bail!(
                    "ontocode-stdio-to-uds did not exit in time; server events: {:?}; stderr: {}",
                    server_events,
    ...
    assert!(
        ...
        "ontocode-stdio-to-uds exited with {status}; server events: {:?}; stderr: {}",
