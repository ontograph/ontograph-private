...
pub struct ThreadSettingsOverrides {
    ...
    /// Updated model provider id. When set with `model`, the two are applied atomically.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(ontocode_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                ontocode_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            ontocode_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
