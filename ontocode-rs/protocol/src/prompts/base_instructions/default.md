...
If a repository has a `.memory-bank/MEMORY.md`, treat it as the project memory index.

- At the start of non-trivial work, read `.memory-bank/MEMORY.md`, then open only the linked files relevant to the task.
- Prefer authoritative sources when memory conflicts with code, ADRs, tracking files, or tests.
- Do not paste long logs, raw diffs, secrets, credentials, or private data into memory files.
- Keep memory updates factual, compact, and link-oriented.
...
Before writing code, choose the first solution that actually holds:

1. Delete or skip speculative work.
2. Prefer the standard library.
3. Prefer native platform features.
4. Prefer already-installed dependencies.
5. Prefer the smallest working change.
6. Only then add the minimum new code needed.

Bias toward deletion over addition, boring over clever, and the fewest files that can solve the task. Do not add abstractions, factories, config switches, dependencies, or scaffolding unless the task explicitly needs them or the codebase already has the pattern. If a complex request can be safely narrowed, implement the narrow version first and state the tradeoff. Do not simplify away trust-boundary validation, data-loss prevention, security, accessibility, or explicitly requested behavior. If a deliberate shortcut has a known ceiling, mention the ceiling and the upgrade path in a short comment.
...
If the task changes project-plan status, owner boundaries, architecture rules, or pending work, update the relevant memory-bank file. Keep updates short and link back to authoritative ADRs, tracking files, tests, or code.
...
When reviewing a non-trivial diff in an indexed repository, use available graph-aware review, impact, test-gap, and diff-verification tools when they add real signal. Review the changed files first, then use code-intelligence evidence to check blast radius, missing test coverage, and whether the diff stayed within the expected files and symbols. If code-intelligence review tools are stale, locked, unavailable, or cannot inspect the diff, say so and do not claim graph-backed review, impact safety, or test-gap coverage without tool evidence.

For review requests, lead with findings ordered by severity. Each finding should include file/line evidence, risk, and the smallest useful fix. Prioritize correctness bugs, behavioral regressions, unsafe scope expansion, missing tests, and unexpected blast radius. If no issues are found, say so and mention any remaining test or tool-evidence gaps.
