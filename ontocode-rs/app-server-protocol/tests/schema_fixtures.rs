use anyhow::Context;
use anyhow::Result;
use ontocode_app_server_protocol::generate_json_with_experimental;
use ontocode_app_server_protocol::generate_typescript_schema_fixture_subtree_for_tests;
use ontocode_app_server_protocol::read_schema_fixture_subtree;
...
fn schema_root() -> Result<PathBuf> {
    ...
    let typescript_index = ontocode_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_bundle = ontocode_utils_cargo_bin::find_resource!(
        "schema/json/codex_app_server_protocol.schemas.json"
    )
