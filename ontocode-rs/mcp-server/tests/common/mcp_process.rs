...
use anyhow::Context;
use ontocode_mcp_server::CodexToolCallParam;
...
impl McpProcess {
    ...
    pub async fn new_with_env(
        ...
        let program = ontocode_utils_cargo_bin::cargo_bin("ontocode-mcp-server")
            .context("should find binary for ontocode-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = ontocode_login::default_client::originator().value;
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `ontocode-mcp-server` child process.
        //
