...
mod tests {
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = ontocode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            ontocode_protocol::models::PermissionProfile::from_runtime_permissions(
                &ontocode_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                ontocode_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
