...
use ontocode_test_binary_support::TestBinaryDispatchGuard;
use ontocode_test_binary_support::TestBinaryDispatchMode;
use ontocode_test_binary_support::configure_test_binary_dispatch;
...
pub static TEST_BINARY_DISPATCH_GUARD: Option<TestBinaryDispatchGuard> = {
    let guard = configure_test_binary_dispatch("ontocode-exec-server-tests", |exe_name, argv1| {
...
fn maybe_run_exec_server_from_test_binary(guard: Option<&TestBinaryDispatchGuard>) {
    ...
    let exit_code =
        match runtime.block_on(ontocode_exec_server::run_main(&listen_url, runtime_paths)) {
            Ok(()) => 0,
            Err(error) => {
