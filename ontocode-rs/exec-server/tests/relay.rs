...
use ontocode_exec_server::InitializeParams;
use ontocode_exec_server::InitializeResponse;
use ontocode_exec_server::RemoteEnvironmentConfig;
...
fn static_registry_auth_provider() -> ontocode_api::SharedAuthProvider {
...
async fn multiplexed_remote_environment_routes_independent_virtual_streams() -> Result<()> {
    ...
    let remote_environment = tokio::spawn(ontocode_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));
