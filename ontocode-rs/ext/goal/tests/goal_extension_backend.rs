...
use ontocode_extension_api::ThreadResumeInput;
use ontocode_extension_api::ThreadStartInput;
use ontocode_extension_api::ThreadStopInput;
use ontocode_extension_api::ToolCall;
use ontocode_extension_api::ToolCallOutcome;
use ontocode_extension_api::ToolCallSource;
use ontocode_extension_api::ToolExecutor;
use ontocode_extension_api::ToolFinishInput;
use ontocode_extension_api::ToolPayload;
use ontocode_extension_api::TurnErrorInput;
use ontocode_extension_api::TurnStartInput;
use ontocode_extension_api::TurnStopInput;
use ontocode_goal_extension::GoalObjectiveUpdate;
use ontocode_goal_extension::GoalRuntimeHandle;
use ontocode_goal_extension::GoalService;
use ontocode_goal_extension::GoalSetRequest;
use ontocode_goal_extension::GoalTokenBudgetUpdate;
use ontocode_goal_extension::install_with_backend;
...
use ontocode_protocol::protocol::TruncationPolicy;
...
async fn update_goal_can_block_and_accounts_final_progress() -> anyhow::Result<()> {
    ...
    let goal = runtime
        ...
        .ok_or_else(|| anyhow::anyhow!("goal should exist"))?;
    assert_eq!(23, goal.tokens_used);
    assert_eq!(ontocode_state::ThreadGoalStatus::Blocked, goal.status);
...
async fn installed_tools(
    runtime: Arc<ontocode_state::StateRuntime>,
    thread_id: ThreadId,
...
struct GoalExtensionHarness {
    registry: ontocode_extension_api::ExtensionRegistry<()>,
...
impl GoalExtensionHarness {
    ...
    async fn notify_tool_finish(&self, turn_id: &str, call_id: &str, tool_name: &str) {
        let turn_store = ExtensionData::new(turn_id);
        let tool_name = ontocode_extension_api::ToolName::plain(tool_name);
...
fn tool_call(tool_name: &str, call_id: &str, arguments: serde_json::Value) -> ToolCall {
    ToolCall {
        ...
        tool_name: ontocode_extension_api::ToolName::plain(tool_name),
        model: "gpt-test".to_string(),
        truncation_policy: TruncationPolicy::Bytes(1024),
        conversation_history: ontocode_extension_api::ConversationHistory::default(),
...
async fn test_runtime() -> anyhow::Result<Arc<ontocode_state::StateRuntime>> {
    let tempdir = TempDir::new()?;
    ontocode_state::StateRuntime::init(tempdir.keep(), "test-provider".to_string()).await
}
...
async fn seed_thread_metadata(
    runtime: &ontocode_state::StateRuntime,
    thread_id: ThreadId,
) -> anyhow::Result<()> {
    let builder = ontocode_state::ThreadMetadataBuilder::new(
        thread_id,
        runtime
            .codex_home()
...
fn protocol_status(status: ontocode_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
