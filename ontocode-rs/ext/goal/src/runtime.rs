...
struct GoalRuntimeInner {
    thread_id: ThreadId,
    state_dbs: Arc<ontocode_state::StateRuntime>,
...
pub struct PreviousGoalSnapshot {
    pub goal_id: String,
    pub status: ontocode_state::ThreadGoalStatus,
    pub objective: String,
}

impl From<&ontocode_state::ThreadGoal> for PreviousGoalSnapshot {
...
impl GoalRuntimeHandle {
    ...
    pub async fn restore_after_resume(&self) -> Result<(), String> {
        ...
        let goal = self
            ...
            .map_err(|err| err.to_string())?;
        match goal {
            Some(goal) if goal.status == ontocode_state::ThreadGoalStatus::Active => {
                self.inner
                    .accounting_state
    ...
    async fn account_idle_goal_progress(
        ...
        Ok(match outcome {
            ...
            ontocode_state::GoalAccountingOutcome::Unchanged(_) => {
    ...
    ) -> Result<Option<ontocode_state::ThreadGoalStatus>, String> {
        let goal = self
            .inner
            .state_dbs
            .thread_goals()
