...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<ontocode_state::StateRuntime>,
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => ontocode_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => ontocode_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        ...
        Ok(match outcome {
            ...
            ontocode_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<ontocode_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &ontocode_state::StateRuntime,
    thread_id: ThreadId,
...
fn protocol_status_from_state(status: ontocode_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
