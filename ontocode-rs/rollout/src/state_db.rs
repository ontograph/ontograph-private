...
pub use ontocode_state::LogEntry;
use ontocode_state::ThreadMetadataBuilder;
use ontocode_utils_path::normalize_for_path_comparison;
...
pub type StateDbHandle = Arc<ontocode_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        ontocode_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .map_err(|err| {
                anyhow::anyhow!(
    ...
    .await;
    ontocode_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == ontocode_state::BackfillStatus::Complete {
            return Ok(());
        }
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = ontocode_state::state_db_path(config.sqlite_home());
    if !tokio::fs::try_exists(&state_path).await.unwrap_or(false) {
        ontocode_state::record_fallback(
            "get_state_db",
            "db_unavailable",
    ...
    let runtime = match ontocode_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    metrics: ontocode_otel::MetricsClient,
    originator: &str,
) -> ontocode_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == ontocode_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<ontocode_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&ontocode_state::StateRuntime>,
    codex_home: &Path,
    page_size: usize,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => ontocode_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => ontocode_state::SortKey::UpdatedAt,
            },
...
) -> Option<ontocode_state::ThreadsPage> {
    let ctx = context?;
    if ctx.codex_home() != codex_home {
        warn!(
            "state db codex_home mismatch: expected {}, got {}",
        ...
        .list_threads(
            ...
            ontocode_state::ThreadFilterOptions {
                ...
                sort_direction: match sort_direction {
                    SortDirection::Asc => ontocode_state::SortDirection::Asc,
                    SortDirection::Desc => ontocode_state::SortDirection::Desc,
                },
                search_term,
            },
        )
