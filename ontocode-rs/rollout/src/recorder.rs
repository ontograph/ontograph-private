...
use ontocode_protocol::protocol::GitInfo as ProtocolGitInfo;
...
impl From<ontocode_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: ontocode_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
...
fn thread_item_from_state_metadata(item: ontocode_state::ThreadMetadata) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &ontocode_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
