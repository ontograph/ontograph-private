...
impl ImportableMcpOAuthCredential {
    pub fn to_provider_credential_routing_view(&self) -> ProviderCredentialRoutingView {
        let mut view = ProviderCredentialRoutingView::new(
            ...
            format!("{}:{}", self.connector_name, self.server_url),
        );
        view.endpoint = Some(self.server_url.clone());
    ...
    pub fn to_provider_oauth_credential(&self) -> ProviderOAuthCredential {
        ...
        credential.endpoint = Some(self.server_url.clone());
...
mod tests {
    ...
    #[test]
    fn importable_mcp_oauth_credential_produces_redacted_provider_routing_view() {
        let credential = ImportableMcpOAuthCredential {
            connector_name: "claude".to_string(),
        ...
        let view = credential.to_provider_credential_routing_view();

        assert_eq!(view.provider_name, "claude");
        assert_eq!(view.credential_id, "claude:https://mcp.example.test");
        assert_eq!(view.endpoint.as_deref(), Some("https://mcp.example.test"));
        assert_eq!(view.client_id.as_deref(), Some("client-123"));
        assert_eq!(
            view.scopes,
            vec!["scope-a".to_string(), "scope-b".to_string()]
        );
        assert_eq!(view.expires_at, Some(1234));
        assert_eq!(view.provenance.as_deref(), Some("claude-code"));
    }

    #[test]
    fn importable_mcp_oauth_credential_converts_to_provider_oauth_credential() {
        let credential = ImportableMcpOAuthCredential {
            connector_name: "claude".to_string(),
            server_url: "https://mcp.example.test".to_string(),
            client_id: "client-123".to_string(),
        ...
        assert_eq!(oauth.provider_name, "claude");
        assert_eq!(oauth.credential_id, "claude:https://mcp.example.test");
        assert_eq!(oauth.endpoint.as_deref(), Some("https://mcp.example.test"));
        assert_eq!(oauth.client_id.as_deref(), Some("client-123"));
        assert_eq!(oauth.access_token, "access-token");
        assert_eq!(oauth.refresh_token.as_deref(), Some("refresh-token"));
        assert_eq!(
            oauth.scopes,
            vec!["scope-a".to_string(), "scope-b".to_string()]
        );
        assert_eq!(oauth.expires_at, Some(1234));
        assert_eq!(oauth.provenance.as_deref(), Some("claude-code"));
    }
