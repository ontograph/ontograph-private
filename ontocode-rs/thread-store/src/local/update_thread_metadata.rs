...
use ontocode_protocol::protocol::GitInfo;
...
use ontocode_rollout::ARCHIVED_SESSIONS_SUBDIR;
...
pub(super) async fn update_thread_metadata(
    ...
    ontocode_rollout::state_db::reconcile_rollout(
...
async fn refresh_resolved_rollout_path(resolved: &mut ResolvedRolloutPath) {
    if let Some(path) = ontocode_rollout::existing_rollout_path(resolved.path.as_path()).await {
        resolved.path = path;
    }
}
...
fn normalize_cwd(cwd: PathBuf) -> PathBuf {
    ontocode_utils_path::normalize_for_path_comparison(cwd.as_path()).unwrap_or(cwd)
}
...
async fn apply_thread_git_info_to_rollout(
    ...
    session_meta.git = Some(GitInfo {
        commit_hash: sha.as_deref().map(ontocode_git_utils::GitSha::new),
        branch: branch.clone(),
...
mod tests {
    ...
    async fn update_thread_metadata_sets_name_on_active_rollout_and_indexes_name() {
        ...
        let latest_name = ontocode_rollout::find_thread_name_by_id(home.path(), &thread_id)
            .await
    ...
    async fn observed_metadata_normalizes_cwd_for_list_filters() {
        ...
        let normalized_cwd =
            ontocode_utils_path::normalize_for_path_comparison(workspace.as_path())
