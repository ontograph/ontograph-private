...
use ontocode_rollout::RolloutRecorder;
use ontocode_rollout::find_archived_thread_path_by_id_str;
use ontocode_rollout::find_thread_name_by_id;
use ontocode_rollout::find_thread_path_by_id_str;
use ontocode_rollout::read_session_meta_line;
use ontocode_rollout::read_thread_item_from_rollout;
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    ...
    if ontocode_rollout::existing_rollout_path(path)
        .await
        .is_none()
    {
        return false;
...
async fn resolve_requested_rollout_path(
    ...
    let Some(path) = ontocode_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && ontocode_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
...
async fn read_thread_from_rollout_path(
    ...
    thread.rollout_path = Some(ontocode_rollout::plain_rollout_path(path.as_path()));
...
async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<ontocode_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let rollout_path = ontocode_rollout::plain_rollout_path(metadata.rollout_path.as_path());
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = ontocode_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_by_rollout_path_prefers_sqlite_git_info() {
        ...
        let active_path =
            write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        let runtime = ontocode_state::StateRuntime::init(
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        ontocode_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
