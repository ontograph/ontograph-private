...
use ontocode_app_server_protocol::DynamicToolSpec;
...
use ontocode_otel::OtelProvider;
...
const OTEL_SERVICE_NAME: &str = "ontocode-app-server-test-client";
...
enum CliCommand {
    ...
    ///
    /// Logs are written to:
    ///   `/tmp/ontocode-app-server-test-client/`
    Serve {
...
fn serve(codex_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/ontocode-app-server-test-client");
...
fn live_elicitation_timeout_pause(
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve ontocode-app-server-test-client binary path")?;
...
impl CodexClient {
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `ontocode-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: ontocode_app_server_protocol::LoginAccountParams::Chatgpt {
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: ontocode_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = ontocode_core::otel_init::build_provider(
            &config,
