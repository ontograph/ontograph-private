...
`ontocode-rs/core/src/memories/`.

## Crates

- `ontocode-rs/memories/read` (`codex-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `ontocode-rs/memories/write` (`codex-memories-write`) owns the write path:
