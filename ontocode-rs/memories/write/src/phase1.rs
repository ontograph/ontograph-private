...
use ontocode_core::RolloutRecorder;
...
use ontocode_rollout::INTERACTIVE_SESSION_SOURCES;
use ontocode_rollout::should_persist_response_item_for_memories;
use ontocode_secrets::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<ontocode_state::Stage1JobClaim>> {
    ...
    match state_db
        ...
        .claim_stage1_jobs_for_startup(
            context.thread_id(),
            ontocode_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    ...
    config: Arc<Config>,
    claimed_candidates: Vec<ontocode_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(crate) async fn run(
        context: &MemoryStartupContext,
        config: &Config,
        claim: ontocode_state::Stage1JobClaim,
        stage_one_context: &StageOneRequestContext,
    ) -> JobResult {
    ...
    pub(super) fn serialize_filtered_rollout_response_items(
        items: &[RolloutItem],
    ) -> ontocode_protocol::error::Result<String> {
        let filtered = items
            .iter()
            .filter_map(|item| {
            ...
            job::serialize_filtered_rollout_response_items(&[RolloutItem::ResponseItem(
                ResponseItem::FunctionCallOutput {
                    ...
                    output: ontocode_protocol::models::FunctionCallOutputPayload {
                        body: ontocode_protocol::models::FunctionCallOutputBody::Text(
