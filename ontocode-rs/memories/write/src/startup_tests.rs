...
use ontocode_git_utils::diff_since_latest_init;
use ontocode_git_utils::reset_git_repository;
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<ontocode_state::StateRuntime>> {
    let db = ontocode_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into())
        .await?;
...
async fn seed_stage1_output(
    db: &ontocode_state::StateRuntime,
    codex_home: &Path,
...
) -> anyhow::Result<ontocode_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.codex.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        ontocode_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
