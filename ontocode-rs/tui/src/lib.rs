...
// The standalone `ontocode-tui` binary prints a short help message before the
...
use ontocode_app_server_client::RemoteAppServerClient;
use ontocode_app_server_client::RemoteAppServerConnectArgs;
pub use ontocode_app_server_client::RemoteAppServerEndpoint;
use ontocode_app_server_protocol::Account as AppServerAccount;
use ontocode_app_server_protocol::AskForApproval;
use ontocode_app_server_protocol::AuthMode as AppServerAuthMode;
...
use ontocode_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use ontocode_exec_server::ExecServerRuntimePaths;
...
use ontocode_utils_home_dir::find_codex_home;
...
pub(crate) use ontocode_app_server_client::legacy_core;
...
const TUI_LOG_FILE_NAME: &str = "ontocode-tui.log";
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            std::io::Error::other(LocalStateDbStartupError::new(
                ontocode_state::state_db_path(config.sqlite_home.as_path()),
                err.to_string(),
            ))
        }),
...
pub fn resolve_remote_addr(addr: &str) -> color_eyre::Result<RemoteAppServerEndpoint> {
    if let Some(socket_path) = addr.strip_prefix("unix://") {
        let socket_path = if socket_path.is_empty() {
            ...
            ontocode_app_server_client::app_server_control_socket_path(&codex_home)
...
async fn maybe_probe_default_daemon_socket(codex_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path =
        ontocode_app_server_client::app_server_control_socket_path(codex_home).ok()?;
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        ontocode_feedback::CodexFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = ontocode_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let feedback = ontocode_feedback::CodexFeedback::new();
...
async fn run_ratatui_app(
    ...
    let login_status = if provider_requires_openai_auth(&initial_config) {
...
async fn get_login_status(
    ...
    if !provider_requires_openai_auth(config) {
...
async fn load_config_toml_or_exit(
    ...
    match load_config_as_toml_with_cli_and_load_options(
        ...
        cwd,
        cli_kv_overrides,
        ontocode_config::ConfigLoadOptions {
            loader_overrides,
            strict_config,
            cloud_config_bundle,
        },
...
fn provider_requires_openai_auth(config: &Config) -> bool {
...
mod tests {
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: ontocode_app_server_client::app_server_control_socket_path(
                    &codex_home
                )?,
            }
        );
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let codex_home = TempDir::new()?;
        let socket_path =
            ontocode_app_server_client::app_server_control_socket_path(codex_home.path())?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        fn write_session_rollout(
            ...
            let session_meta = ontocode_protocol::protocol::SessionMeta {
                id: thread_id,
                timestamp: meta_rfc3339.to_string(),
                cwd: cwd.to_path_buf(),
                originator: "codex".to_string(),
                cli_version: "0.0.0".to_string(),
                source: ontocode_protocol::protocol::SessionSource::Cli,
                model_provider: Some(model_provider.to_string()),
                ..Default::default()
            };
            let session_meta =
                serde_json::to_value(ontocode_protocol::protocol::SessionMetaLine {
                    meta: session_meta,
                    git: None,
                })?;
            let lines = [
        ...
        let mut app_server = AppServerSession::new(
            ontocode_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = ontocode_state::StateRuntime::init(
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            ontocode_state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
