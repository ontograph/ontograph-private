...
impl SlashCommand {
    ...
    pub fn description(self) -> &'static str {
        match self {
            ...
            SlashCommand::Login => "choose a provider to sign in or import credentials",
            SlashCommand::Auth => "manage provider auth menus",
...
mod tests {
    ...
    fn certain_commands_are_available_during_task() {
        ...
        assert!(SlashCommand::Auth.available_during_task());
        assert!(SlashCommand::Status.supports_inline_args());
    ...
    #[test]
    fn login_and_auth_commands_are_exported() {
        let commands = super::built_in_slash_commands();
        assert!(
            commands
                .iter()
                .any(|(_, command)| *command == SlashCommand::Login)
        );
        assert!(
            commands
                .iter()
                .any(|(_, command)| *command == SlashCommand::Auth)
        );
    }
}
