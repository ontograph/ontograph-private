...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<ontocode_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: ontocode_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![ontocode_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, ontocode_protocol::mcp::Tool>,
    HashMap<String, Vec<ontocode_protocol::mcp::Resource>>,
    HashMap<String, Vec<ontocode_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                ontocode_app_server_protocol::McpAuthStatus::Unsupported => {
                    McpAuthStatus::Unsupported
                }
                ontocode_app_server_protocol::McpAuthStatus::NotLoggedIn => {
                    McpAuthStatus::NotLoggedIn
                }
                ontocode_app_server_protocol::McpAuthStatus::BearerToken => {
                    McpAuthStatus::BearerToken
                }
                ontocode_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    use ontocode_protocol::mcp::Tool;
    use ontocode_utils_absolute_path::AbsolutePathBuf;
