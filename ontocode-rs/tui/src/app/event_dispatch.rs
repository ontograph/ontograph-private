    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::UpdateModel(model) => {
                ...
                self.sync_active_thread_model_setting(app_server, model, None)
                    .await;
            ...
            AppEvent::OpenReasoningPopup {
                ...
                self.chat_widget
                    .open_reasoning_popup_for_provider(model, model_provider);
            }
            ...
            AppEvent::PersistModelSelection(selection) => {
                match crate::config_update::write_config_batch(
                    ...
                    crate::config_update::build_model_selection_edits(
                        crate::config_update::ModelSelectionConfigEdits {
                            model: selection.model.as_str(),
                            model_provider: selection.model_provider.as_deref(),
                ...
                {
                    Ok(_) => {
                        self.chat_widget.set_model(&selection.model);
                        ...
                        if let Some(model_provider) = selection.model_provider.as_deref() {
                            tracing::info!(
                        ...
                        if let Some(label) = Self::reasoning_label_for(model, selection.effort) {
                            message.push(' ');
