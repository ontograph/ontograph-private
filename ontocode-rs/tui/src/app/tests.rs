...
use ontocode_app_server_protocol::AdditionalFileSystemPermissions;
use ontocode_app_server_protocol::AdditionalNetworkPermissions;
use ontocode_app_server_protocol::AdditionalPermissionProfile;
use ontocode_app_server_protocol::AgentMessageDeltaNotification;
...
use ontocode_app_server_protocol::FileUpdateChange;
...
use ontocode_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use ontocode_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use ontocode_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use ontocode_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use ontocode_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use ontocode_app_server_protocol::PatchChangeKind;
use ontocode_app_server_protocol::PermissionsRequestApprovalParams;
use ontocode_app_server_protocol::RequestId as AppServerRequestId;
use ontocode_app_server_protocol::ServerNotification;
use ontocode_app_server_protocol::ServerRequest;
use ontocode_app_server_protocol::SessionSource;
...
use ontocode_app_server_protocol::TokenUsageBreakdown;
...
use ontocode_app_server_protocol::TurnError as AppServerTurnError;
...
use ontocode_app_server_protocol::UserInput as AppServerUserInput;
...
use ontocode_protocol::config_types::CollaborationModeMask;
...
use ontocode_protocol::models::NetworkPermissions;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let ontocode_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_respects_origin_thread() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: ontocode_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        feedback: ontocode_feedback::CodexFeedback::new(),
        is_first_run: false,
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: ontocode_app_server_protocol::CollabAgentTool::Wait,
                status: ontocode_app_server_protocol::CollabAgentToolCallStatus::InProgress,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(
            ontocode_app_server_protocol::ItemCompletedNotification {
                thread_id: ThreadId::new().to_string(),
                turn_id: "turn-1".to_string(),
                completed_at_ms: 0,
                item: ThreadItem::CollabAgentToolCall {
                    id: "send-1".to_string(),
                    tool: ontocode_app_server_protocol::CollabAgentTool::SendInput,
                    status: ontocode_app_server_protocol::CollabAgentToolCallStatus::Failed,
                    sender_thread_id: ThreadId::new().to_string(),
                    receiver_thread_ids: vec![receiver_thread_id.to_string()],
                    ...
                    agents_states: HashMap::from([(
                        ...
                        ontocode_app_server_protocol::CollabAgentState {
                            status: ontocode_app_server_protocol::CollabAgentStatus::NotFound,
                            message: None,
                        },
                    )]),
                },
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        available_decisions,
        vec![
            ontocode_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            ontocode_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            ontocode_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            ontocode_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: ontocode_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: ontocode_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: ontocode_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                created_at: 1,
                updated_at: 2,
                status: ontocode_app_server_protocol::ThreadStatus::Idle,
                path: Some(rollout_path.clone()),
                cwd: test_path_buf("/tmp/agent").abs(),
                cli_version: "0.0.0".to_string(),
                source: ontocode_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            ontocode_app_server_protocol::ServerRequestResolvedNotification {
...
async fn side_thread_ignores_global_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        ontocode_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            ontocode_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        ontocode_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
        ...
        ThreadEventSnapshot {
            ...
            events: vec![ThreadBufferedEvent::Notification(
                ServerNotification::ItemStarted(
                    ontocode_app_server_protocol::ItemStartedNotification {
                        ...
                        item: ThreadItem::CollabAgentToolCall {
                            ...
                            tool: ontocode_app_server_protocol::CollabAgentTool::Wait,
                            status:
                                ontocode_app_server_protocol::CollabAgentToolCallStatus::InProgress,
                            sender_thread_id: ThreadId::new().to_string(),
                            receiver_thread_ids: vec![receiver_thread_id.to_string()],
                            prompt: None,
                            model: None,
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                ontocode_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn thread_setting_update_params_sync_model_and_default_reasoning() {
    ...
    assert_eq!(params.model_provider, Some("openai".to_string()));
    assert_eq!(
        params
            .collaboration_mode
            .as_ref()
...
#[test]
fn thread_settings_update_detects_provider_only_changes() {
    ...
    assert!(thread_settings::thread_settings_update_has_changes(&params));
}
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: ontocode_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: ontocode_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                ontocode_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
            model_provider: "openai".to_string(),
            service_tier: None,
