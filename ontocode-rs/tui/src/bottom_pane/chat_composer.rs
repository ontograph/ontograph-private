...
//!
//! For the detailed burst state machine, see `ontocode-rs/tui/src/bottom_pane/paste_burst.rs`.
//!
...
#[cfg(test)]
use ontocode_core_skills::model::SkillInterface;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let display_name =
                    ontocode_connectors::metadata::connector_display_label(connector);
...
程序员 与 Unicode 同行";
    ...
    #[test]
    fn slash_popup_login_for_lo_logic() {
        use super::super::command_popup::CommandItem;
        let (tx, _rx) = unbounded_channel::<AppEvent>();
        let sender = AppEventSender::new(tx);
        let mut composer = ChatComposer::new(
        ...
        match &composer.popups.active {
            ActivePopup::Command(popup) => match popup.selected_item() {
                ...
                Some(CommandItem::ServiceTier(command)) => {
                    panic!("expected login command, got service tier {command:?}")
                }
                None => panic!("no selected command for '/lo'"),
            },
            _ => panic!("slash popup not active after typing '/lo'"),
        }
    }
    ...
    fn file_completion_preserves_large_paste_placeholder_elements() {
        ...
        composer.on_file_search_result(
            ...
            vec![FileMatch {
                ...
                match_type: ontocode_file_search::MatchType::File,
