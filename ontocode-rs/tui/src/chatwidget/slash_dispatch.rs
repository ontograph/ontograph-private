...
use ontocode_external_agent_migration::gemini_oauth_import::ANTIGRAVITY_PROVIDER_NAME;
use ontocode_login::load_auth_dot_json;
use ontocode_login::remove_provider_oauth_credential;
use ontocode_model_provider::ProviderAuthStatusState;
...
const KIMI_RUNTIME_UNAVAILABLE_REASON: &str = "Kimi runtime is not available yet.";
const DEFAULT_ROUTE_NOTE: &str = "Default route: global model/provider selection; sub-agent roles may use their own provider route.";
...
impl ChatWidget {
    ...
    fn auth_login_message() -> String {
        "Native login is Codex / OpenAI only. Configure Gemini, Kimi, Antigravity, Claude, and GitHub Copilot as external OpenAI-compatible providers."
            .to_string()
    }

    fn auth_actions_message() -> String {
        "Native auth is Codex / OpenAI only. External providers own their own API keys or OAuth."
            .to_string()
    }

    fn auth_import_usage_message() -> String {
        "Provider OAuth import is no longer native. Configure an external OpenAI-compatible provider instead.".to_string()
    }

    fn login_import_usage_message() -> String {
        "Provider OAuth import is no longer native. Configure an external OpenAI-compatible provider instead.".to_string()
    }

    fn parse_provider_login_args(args: &str, provider_id: &str) -> Option<Vec<String>> {
        let tokens = shlex::split(args)?;
        let provider = tokens.first()?;
        provider
    ...
    fn handle_login_provider_command(&mut self, args: &str) -> bool {
        if let Some(tokens) = Self::parse_provider_login_args(args, "gemini")
            && tokens.is_empty()
        {
            self.add_info_message(
                "Gemini OAuth is not a native login mode. Configure Gemini as an external OpenAI-compatible provider.".to_string(),
                /*hint*/ None,
            );
            return true;
        }
        if let Some(tokens) = Self::parse_provider_login_args(args, KIMI_PROVIDER_NAME)
            && tokens.is_empty()
        {
            self.add_info_message(
                "Kimi OAuth is not a native login mode. Configure Kimi as an external OpenAI-compatible provider.".to_string(),
                /*hint*/ None,
            );
            return true;
        }
        if let Some(tokens) = Self::parse_provider_login_args(args, ANTIGRAVITY_PROVIDER_NAME)
            && tokens.is_empty()
        {
            self.add_info_message(
                "Antigravity OAuth is not a native login mode. Configure Antigravity as an external OpenAI-compatible provider.".to_string(),
                /*hint*/ None,
            );
            return true;
        }
    ...
    fn provider_display_label(provider_id: &str) -> &'static str {
        if provider_id.eq_ignore_ascii_case(ANTIGRAVITY_PROVIDER_NAME) {
            "Antigravity"
        } else if provider_id.eq_ignore_ascii_case(KIMI_PROVIDER_NAME) {
            "Kimi"
        } else {
            "Gemini"
        }
    ...
    fn supported_oauth_import_provider(provider_id: &str) -> bool {
        provider_id.eq_ignore_ascii_case("gemini")
            || provider_id.eq_ignore_ascii_case(KIMI_PROVIDER_NAME)
            || provider_id.eq_ignore_ascii_case(ANTIGRAVITY_PROVIDER_NAME)
    }
    ...
    fn parse_import_args(
        ...
        let tokens = shlex::split(args)?;
        let provider = tokens.first()?;
        if !Self::supported_oauth_import_provider(provider) {
            return None;
        }

        let flag = tokens.get(1)?;
        if !flag.eq_ignore_ascii_case(import_flag) {
            return None;
        }
        ...
        if path.trim().is_empty() {
            if provider.eq_ignore_ascii_case("gemini") {
                default_path.map(|path| ("gemini".to_string(), path.to_string()))
            } else {
                None
            }
        } else {
            Some((provider.to_ascii_lowercase(), path.trim().to_string()))
        }
    }
    ...
    fn import_provider_oauth_credential(&mut self, provider_id: &str, path: &Path) -> bool {
        let _ = path;
        self.add_error_message(format!(
            "{} OAuth import is no longer native. Configure an external OpenAI-compatible provider instead.",
            Self::provider_display_label(provider_id)
        ));
        true
    }

    fn auth_provider_rows(&self) -> Vec<ontocode_model_provider::ProviderAuthStatusRow> {
        ...
        providers.sort_by(|(left_provider_id, _), (right_provider_id, _)| {
            match (
                left_provider_id == OPENAI_PROVIDER_ID,
                right_provider_id == OPENAI_PROVIDER_ID,
            ) {
                (true, true) => Ordering::Equal,
                (true, false) => Ordering::Less,
                (false, true) => Ordering::Greater,
                (false, false) => left_provider_id.cmp(right_provider_id),
            }
        });
        ...
        let mut rows = providers
            ...
            .filter_map(|(provider_id, provider_info)| {
                ...
                if provider_id == ANTIGRAVITY_PROVIDER_NAME {
                    return Some(
                        ontocode_model_provider::ProviderAuthStatusRow::builder(
                            provider_id,
                            provider_info.name,
                        )
                        .blocked("Antigravity runtime is not available yet.")
                        .build(),
                    );
                }
                if provider_id == KIMI_PROVIDER_NAME {
                    return Some(
                        ontocode_model_provider::ProviderAuthStatusRow::builder(
                            provider_id,
                            provider_info.name,
                        )
                        .blocked(KIMI_RUNTIME_UNAVAILABLE_REASON)
                        .build(),
                    );
                }
                if provider_id == CLAUDE_PROVIDER_NAME {
                    return Some(
                        ontocode_model_provider::ProviderAuthStatusRow::builder(
                            provider_id,
                            provider_info.name,
                        )
                        .blocked(CLAUDE_RUNTIME_UNAVAILABLE_REASON)
                        .build(),
                    );
                }
                Some(
                    ontocode_model_provider::ProviderAuthStatusRow::from_provider(
                        provider_id,
                        &provider_info,
                        None,
                    ),
            ...
            .collect::<Vec<_>>();
        if !seen_provider_ids.contains(ANTIGRAVITY_PROVIDER_NAME) {
            rows.push(
                ontocode_model_provider::ProviderAuthStatusRow::builder(
        ...
        if !seen_provider_ids.contains(KIMI_PROVIDER_NAME) {
            rows.push(
                ontocode_model_provider::ProviderAuthStatusRow::builder(KIMI_PROVIDER_NAME, "Kimi")
                    .blocked(KIMI_RUNTIME_UNAVAILABLE_REASON)
                    .build(),
            );
        }
        if !seen_provider_ids.contains(CLAUDE_PROVIDER_NAME) {
            rows.push(
                ontocode_model_provider::ProviderAuthStatusRow::builder(
    ...
    fn auth_provider_rows_for_query(
        &self,
        provider_query: &str,
    ) -> Vec<ontocode_model_provider::ProviderAuthStatusRow> {
        let rows = self.auth_provider_rows();
        if provider_query.is_empty() {
            return rows;
        }
        rows.into_iter()
            .filter(|row| Self::auth_provider_row_matches_query(row, provider_query))
            .collect()
    }

    fn auth_provider_row_matches_query(
        row: &ontocode_model_provider::ProviderAuthStatusRow,
        query: &str,
    ) -> bool {
        query.eq_ignore_ascii_case(row.provider_id.as_str())
            || query.eq_ignore_ascii_case(row.display_label.as_str())
    }

    fn format_auth_provider_status_row(
        row: &ontocode_model_provider::ProviderAuthStatusRow,
    ) -> String {
        let mut status = match row.state {
            ProviderAuthStatusState::Ready => "ready".to_string(),
            ProviderAuthStatusState::NotConfigured => {
                let reason = row.disabled_reason.as_deref().unwrap_or("not configured");
                format!("not configured: {reason}")
            }
            ProviderAuthStatusState::ApiKeyActive => "api key active".to_string(),
            ProviderAuthStatusState::Blocked => {
                let reason = row.disabled_reason.as_deref().unwrap_or("blocked");
                format!("blocked: {reason}")
            }
            ProviderAuthStatusState::ErrorRedacted => {
                let reason = row.disabled_reason.as_deref().unwrap_or("error");
                format!("error: {reason}")
            }
        };

        if let Some(source) = row.active_source_label.as_deref() {
            status.push_str(" (");
            status.push_str(source);
            status.push(')');
        }

        if let Some(account_label) = row.redacted_account_label.as_deref() {
            status.push_str(" (");
            status.push_str(account_label);
            status.push(')');
        }

        format!(
            "  - {} ({}) — {}",
            row.display_label, row.provider_id, status
        )
    }

    fn auth_provider_oauth_credential_id(
        &mut self,
        provider_id: &str,
        ...
        let credential_ids = auth
            .provider_oauth_credentials
            .into_iter()
            .filter_map(|credential| {
                (credential.provider_id() == provider_id).then_some(credential.credential_id)
            })
            .collect::<Vec<_>>();
        if credential_ids.len() > 1 {
            self.add_error_message(format!(
                "Multiple OAuth credentials configured for {provider_id}; account-specific removal is not implemented yet."
            ));
            return Ok(None);
        }
        Ok(credential_ids.into_iter().next())
    }

    fn parse_auth_remove_args(args: &str) -> Option<String> {
        let provider_tokens = args.split_whitespace().collect::<Vec<_>>();
        let (action, provider_tokens) = provider_tokens.split_last()?;
        if !action.eq_ignore_ascii_case("remove") {
            return None;
        }
        let provider_query = provider_tokens.join(" ");
        (!provider_query.is_empty()).then_some(provider_query)
    }

    fn handle_auth_remove_command(&mut self, args: &str) -> bool {
        let Some(provider_query) = Self::parse_auth_remove_args(args) else {
            self.add_error_message("Usage: /auth <provider> remove".to_string());
            return true;
        };
        let mut provider_rows = self.auth_provider_rows_for_query(&provider_query);
        if provider_rows.is_empty() {
            self.add_error_message(format!(
                "No provider matches {provider_query:?} for auth removal."
            ));
            return true;
        }
        if provider_rows.len() > 1 {
            self.add_error_message(format!(
                "Provider query {provider_query:?} matched multiple providers; please use a unique provider id."
            ));
            return true;
        }
        let row = provider_rows.remove(0);
        let provider_id = row.provider_id.clone();
        let credential_id = match self.auth_provider_oauth_credential_id(&provider_id) {
            Ok(Some(credential_id)) => credential_id,
            Ok(None) => {
                self.add_error_message(format!(
                    "No OAuth credential configured for {provider_query:?}."
                ));
                return true;
            }
            Err(err) => {
                self.add_error_message(format!(
                    "Failed to read provider auth for {provider_query:?}: {err}"
                ));
                return true;
            }
        };
        match remove_provider_oauth_credential(
            self.config.codex_home.as_path(),
            self.config.cli_auth_credentials_store_mode,
            &provider_id,
            &credential_id,
            ...
            Ok(true) => {
                self.add_info_message(
                    format!(
                        "Removed OAuth credential for provider {} ({}).",
                        row.display_label, row.provider_id
                    ),
                    /*hint*/ None,
                );
            }
            Ok(false) => {
                self.add_error_message(format!(
                    "No matching OAuth credential found for {provider_query:?}."
                ));
            }
            Err(err) => {
                self.add_error_message(format!(
                    "Failed to remove OAuth credential for {provider_query:?}: {err}"
                ));
            }
        }
        true
    ...
    fn handle_auth_import_command(&mut self, args: &str) -> bool {
        let Some((provider_id, path)) =
            Self::parse_import_args(args, "import", /*default_path*/ None)
        else {
            return false;
        };
        self.import_provider_oauth_credential(&provider_id, Path::new(&path))
    }

    fn handle_login_import_command(&mut self, args: &str) -> bool {
        let Some((provider_id, path)) =
            Self::parse_import_args(args, "--import", Some("~/.gemini/oauth_creds.json"))
        else {
            return false;
        };
        self.import_provider_oauth_credential(&provider_id, Self::expand_home_path(&path).as_path())
    }

    fn handle_auth_shell_command(&mut self, cmd: SlashCommand, args: &str) -> bool {
        match cmd {
            SlashCommand::Login => {
                if args.is_empty() {
                    self.add_info_message(Self::auth_login_message(), /*hint*/ None);
                    return true;
                }
                if self.handle_login_import_command(args) {
                    return true;
                }
                if self.handle_login_provider_command(args) {
                    return true;
                }
                self.add_error_message(Self::login_import_usage_message());
                return true;
            }
            SlashCommand::Auth => {
                if args.is_empty() {
                    self.add_info_message(Self::auth_actions_message(), /*hint*/ None);
                    return true;
                }
                if self.handle_auth_import_command(args) {
                    return true;
                }
                if self.handle_auth_remove_command(args) {
                    return true;
                }
                self.add_error_message(Self::auth_import_usage_message());
                self.add_error_message("Usage: /auth <provider> remove".to_string());
                return true;
            }
            SlashCommand::Status => {
                ...
                let provider_query = tokens.collect::<Vec<_>>().join(" ");
                let provider_rows = self.auth_provider_rows_for_query(&provider_query);
                if provider_rows.is_empty() {
                    self.add_error_message(format!(
                        "No provider matches {provider_query:?} for auth status."
                    ));
                } else {
                    let message = provider_rows
                        .into_iter()
                        .map(|row| Self::format_auth_provider_status_row(&row))
                        .collect::<Vec<_>>()
                        .join("\n");
                    self.add_info_message(
                        if provider_query.is_empty() {
                            format!("Provider auth status:\n{DEFAULT_ROUTE_NOTE}\n{message}")
                        } else {
                            format!(
                                "Provider auth status for {provider_query:?}:\n{DEFAULT_ROUTE_NOTE}\n{message}"
                            )
                        },
                        /*hint*/ None,
                    );
            ...
            SlashCommand::Logout => {
                self.add_error_message(format!(
                    "'/logout {args}' is read-only in this shell. Use '/auth {args} remove' to remove provider-scoped auth instead."
                ));
            }
            _ => return false,
        }
    ...
    pub(super) fn dispatch_command_with_args(
        ...
        if self.handle_auth_shell_command(cmd, trimmed) {
            self.bottom_pane.drain_pending_submission_state();
            return;
        }
    ...
    fn queued_command_drain_result(&self, cmd: SlashCommand) -> QueueDrain {
        ...
        match cmd {
            ...
            | SlashCommand::TestApproval
            | SlashCommand::Login
            | SlashCommand::Auth => QueueDrain::Continue,
            SlashCommand::Feedback
            | SlashCommand::New
            | SlashCommand::Archive
            | SlashCommand::Clear
...
mod tests {
    ...
    #[test]
    fn login_gemini_import_defaults_to_gemini_cli_oauth_file() {
        assert_eq!(
            ChatWidget::parse_import_args(
                "gemini --import",
                "--import",
                Some("~/.gemini/oauth_creds.json"),
            ),
            Some((
                "gemini".to_string(),
                "~/.gemini/oauth_creds.json".to_string()
            ))
        );
    }

    #[test]
    fn auth_gemini_import_still_requires_explicit_path() {
        assert_eq!(
            ChatWidget::parse_import_args("gemini import", "import", /*default_path*/ None),
            None
        );
    }

    #[test]
    fn login_antigravity_import_requires_explicit_path() {
        assert_eq!(
            ChatWidget::parse_import_args(
                "antigravity --import",
                "--import",
        ...
        assert_eq!(
            ChatWidget::parse_import_args("antigravity --import /tmp/oauth.json", "--import", None),
            Some(("antigravity".to_string(), "/tmp/oauth.json".to_string()))
        );
    }
}
