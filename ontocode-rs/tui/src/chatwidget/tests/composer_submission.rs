...
#[tokio::test]
async fn submission_blocks_gemini_model_with_openai_provider() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let command = chat.submit_user_message_with_shell_escape_policy(
        UserMessage {
    ...
    while let Ok(op) = op_rx.try_recv() {
        assert!(
            ...
            "expected Gemini/OpenAI mismatch to block user turn, got {op:?}"
        );
    }
