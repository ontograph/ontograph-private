...
fn token_usage_breakdown(usage: TokenUsage) -> ontocode_app_server_protocol::TokenUsageBreakdown {
    ontocode_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    ontocode_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: ontocode_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = ontocode_shell_command::parse_command::parse_command(&command)
        .into_iter()
    ...
    let item = AppServerThreadItem::CommandExecution {
        ...
        command: ontocode_shell_command::parse_command::shlex_join(&command),
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            ontocode_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(ontocode_app_server_protocol::HookEventName, usize)],
    apps: &[(&str, bool)],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    ontocode_app_server_protocol::PluginHookSummary {
...
pub(super) fn hook_run(
    run_id: &str,
    event_name: ontocode_app_server_protocol::HookEventName,
    status: ontocode_app_server_protocol::HookRunStatus,
    status_message: &str,
    entries: Vec<ontocode_app_server_protocol::HookOutputEntry>,
) -> ontocode_app_server_protocol::HookRunSummary {
    ontocode_app_server_protocol::HookRunSummary {
        id: run_id.to_string(),
        event_name,
        handler_type: ontocode_app_server_protocol::HookHandlerType::Command,
        execution_mode: ontocode_app_server_protocol::HookExecutionMode::Sync,
        scope: ontocode_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: ontocode_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        completed_at: matches!(
            status,
            ontocode_app_server_protocol::HookRunStatus::Completed
                | ontocode_app_server_protocol::HookRunStatus::Failed
                | ontocode_app_server_protocol::HookRunStatus::Blocked
                | ontocode_app_server_protocol::HookRunStatus::Stopped
        )
        .then_some(11),
        duration_ms: matches!(
            status,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_started(
        ...
        hook_run(
            run_id,
            event_name,
            ontocode_app_server_protocol::HookRunStatus::Running,
            status_message,
            Vec::new(),
        ),
    );
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                ontocode_app_server_protocol::HookOutputEntry {
                    kind: ontocode_app_server_protocol::HookOutputEntryKind::Warning,
                    text: "Heads up from the hook".to_string(),
                },
                ontocode_app_server_protocol::HookOutputEntry {
                    kind: ontocode_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: ontocode_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        ontocode_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        ontocode_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        ontocode_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        ontocode_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        ontocode_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        ontocode_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        ontocode_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        ontocode_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        ontocode_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        ontocode_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
