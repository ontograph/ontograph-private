...
use ontocode_app_server_protocol::HookErrorInfo;
    ...
    chat.on_plugin_detail_loaded(
        ...
        Ok(PluginReadResponse {
            plugin: plugins_test_detail(
                ...
                &[
                    (ontocode_app_server_protocol::HookEventName::PreToolUse, 1),
                    (ontocode_app_server_protocol::HookEventName::Stop, 2),
                ],
...
#[tokio::test]
async fn model_picker_hides_gemini_models_for_openai_provider() {
    ...
    chat.open_model_popup_with_presets(vec![preset("gpt-5.4"), preset("gemini-2.5-pro")]);

    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!(
        "model_picker_filters_openai_unsupported_gemini_models",
        popup.clone()
    );
    assert!(
        popup.contains("gpt-5.4"),
        "expected OpenAI model to appear in picker:\n{popup}"
    );
    assert!(
        !popup.contains("gemini-2.5-pro"),
        "expected Gemini model to be excluded from OpenAI picker:\n{popup}"
    );
}

#[tokio::test]
async fn model_picker_groups_provider_catalogs_from_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.thread_id = Some(ThreadId::new());
    ...
    let active_model = preset("gpt-5.4", "OpenAI model");
    let claude_model = preset("claude-sonnet-4-5", "Claude model");
    chat.model_catalog =
        std::sync::Arc::new(crate::model_catalog::ModelCatalog::with_provider_groups(
            vec![active_model.clone()],
            vec![
                crate::model_catalog::ProviderModelGroup {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert_chatwidget_snapshot!("model_picker_groups_provider_catalogs", popup.clone());
    assert!(
        popup.contains("OpenAI"),
        "expected OpenAI group in popup:\n{popup}"
    );
    assert!(
        popup.contains("Claude"),
        "expected Claude group in popup:\n{popup}"
    );
    assert!(
        popup.contains("claude-sonnet-4-5"),
        "expected Claude model in popup:\n{popup}"
    );
    assert!(
        popup.contains("Gemini CLI") && popup.contains("runtime is not available yet"),
        "expected disabled Gemini CLI group in popup:\n{popup}"
    );
}

#[tokio::test]
async fn provider_reasoning_popup_persists_provider_model_and_effort() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    let preset = ModelPreset {
        id: "claude-sonnet-4-5".to_string(),
        model: "claude-sonnet-4-5".to_string(),
        display_name: "Claude Sonnet 4.5".to_string(),
        description: "Claude model".to_string(),
        default_reasoning_effort: ReasoningEffortConfig::Medium,
        supported_reasoning_efforts: vec![ReasoningEffortPreset {
            effort: ReasoningEffortConfig::Medium,
            description: "medium".to_string(),
    ...
    chat.open_reasoning_popup_for_provider(preset, Some("claude".to_string()));
    ...
    assert!(
        events.iter().any(|event| matches!(
            ...
            AppEvent::PersistModelSelection(selection)
                if selection.model == "claude-sonnet-4-5"
                    && selection.model_provider.as_deref() == Some("claude")
                    && selection.effort == Some(ReasoningEffortConfig::Medium)
        )),
        "expected provider-aware model persistence event; events: {events:?}"
    );
}

#[tokio::test]
async fn provider_reasoning_popup_infers_gemini_provider_from_catalog() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.config.model_provider_id = "openai".to_string();
    let gemini_preset = ModelPreset {
        id: "gemini-2.5-pro".to_string(),
        model: "gemini-2.5-pro".to_string(),
        display_name: "Gemini 2.5 Pro".to_string(),
        description: "Gemini model".to_string(),
        default_reasoning_effort: ReasoningEffortConfig::Medium,
        supported_reasoning_efforts: vec![ReasoningEffortPreset {
            effort: ReasoningEffortConfig::Medium,
            description: "medium".to_string(),
    ...
    chat.model_catalog =
        std::sync::Arc::new(crate::model_catalog::ModelCatalog::with_provider_groups(
            ...
            vec![crate::model_catalog::ProviderModelGroup {
                ...
                models: vec![gemini_preset.clone()],
                disabled_reason: None,
            }],
        ));

    chat.open_reasoning_popup_for_provider(gemini_preset, None);

    let events = std::iter::from_fn(|| rx.try_recv().ok()).collect::<Vec<_>>();
    assert!(
        !events.iter().any(
            |event| matches!(event, AppEvent::UpdateModel(model) if model == "gemini-2.5-pro")
        ),
        "did not expect a model-only update before provider persistence; events: {events:?}"
    );
    assert!(
        events.iter().any(|event| matches!(
            event,
            AppEvent::PersistModelSelection(selection)
                if selection.model == "gemini-2.5-pro"
                    && selection.model_provider.as_deref() == Some("gemini")
                    && selection.effort == Some(ReasoningEffortConfig::Medium)
        )),
        "expected inferred Gemini provider persistence event; events: {events:?}"
    );
}

#[tokio::test]
async fn provider_reasoning_popup_persists_gemini_provider_even_when_config_already_gemini() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gemini-2.5-pro")).await;
    ...
    assert!(
        ...
        "expected Gemini provider persistence event; events: {events:?}"
    );
}

#[tokio::test]
async fn provider_reasoning_popup_infers_openai_provider_when_switching_from_gemini() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gemini-2.5-pro")).await;
    chat.config.model_provider_id = "gemini".to_string();
    let openai_preset = ModelPreset {
    ...
    chat.model_catalog =
        std::sync::Arc::new(crate::model_catalog::ModelCatalog::with_provider_groups(
            ...
            vec![crate::model_catalog::ProviderModelGroup {
                provider_id: "openai".to_string(),
                display_name: "OpenAI".to_string(),
                models: vec![openai_preset.clone()],
                disabled_reason: None,
            }],
        ));

    chat.open_reasoning_popup_for_provider(openai_preset, None);
        ...
        events.iter().any(|event| matches!(
            ...
            AppEvent::PersistModelSelection(selection)
                if selection.model == "gpt-5.4"
                    && selection.model_provider.as_deref() == Some("openai")
                    && selection.effort == Some(ReasoningEffortConfig::Medium)
        )),
        "expected inferred OpenAI provider persistence event; events: {events:?}"
    );
}

#[tokio::test]
async fn provider_reasoning_popup_infers_openai_provider_when_switching_from_lmstudio() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-oss")).await;
    chat.config.model_provider_id = "lmstudio".to_string();
    let openai_preset = ModelPreset {
...
#[tokio::test]
async fn gemini_config_allows_gemini_model_submission() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gemini-2.5-pro")).await;
    chat.config.model_provider_id = "gemini".to_string();

    assert_eq!(
        chat.unsupported_model_provider_message("gemini-2.5-pro"),
        None
    );
}
...
async fn assert_reasoning_shortcuts_update_effort(
    ...
    for key_event in key_events {
        ...
        assert!(
            events
                .iter()
                .all(|event| !matches!(event, AppEvent::PersistModelSelection(_))),
...
async fn feedback_upload_consent_popup_snapshot() {
    ...
    chat.show_selection_view(crate::bottom_pane::feedback_upload_consent_params(
        ...
        &ontocode_feedback::FeedbackDiagnostics::new(vec![ontocode_feedback::FeedbackDiagnostic {
