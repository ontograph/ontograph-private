...
use ontocode_login::ProviderCredentialSourceKind;
use ontocode_login::ProviderOAuthCredential;
use ontocode_login::load_auth_dot_json;
use ontocode_login::upsert_provider_oauth_credential;
...
fn seed_provider_oauth_credential(chat: &mut ChatWidget, provider_id: &str, credential_id: &str) {
    let credential = ProviderOAuthCredential::new(
        provider_id,
    ...
    .expect("auth state seeded");
}

fn make_gemini_oauth_import_file(sample: &str) -> (tempfile::TempDir, std::path::PathBuf) {
    let tempdir = tempfile::tempdir().unwrap();
    let path = tempdir.path().join("gemini_oauth_import_sample.json");
    std::fs::write(&path, sample).expect("sample file should write");
    (tempdir, path)
}
...
async fn queued_bare_rename_drains_next_input_after_name_update() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            ontocode_app_server_protocol::ThreadNameUpdatedNotification {
...
#[tokio::test]
async fn slash_login_without_args_keeps_default_provider_and_shows_login_options() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.model_provider_id = OPENAI_PROVIDER_ID.to_string();
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Native login is Codex / OpenAI only."));
    assert!(rendered.contains("external OpenAI-compatible providers"));
    assert_eq!(chat.config.model_provider_id, OPENAI_PROVIDER_ID);
}

#[tokio::test]
async fn slash_logout_provider_args_show_read_only_message() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.dispatch_command_with_args(SlashCommand::Logout, "gemini".to_string(), Vec::new());
    ...
    assert!(
        !events.iter().any(|event| matches!(event, AppEvent::Logout)),
        "expected provider-scoped logout to stay read-only; events: {events:?}"
    );
    let rendered = events
        .iter()
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("read-only in this shell"));
    assert!(rendered.contains("Use '/auth gemini remove' to remove provider-scoped auth instead."));
}

#[tokio::test]
async fn slash_login_gemini_import_failure_keeps_default_provider() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.model_provider_id = OPENAI_PROVIDER_ID.to_string();
    chat.config.model_provider = ModelProviderInfo {
        name: "OpenAI".to_string(),
    ...
    let missing_import_path = chat.config.codex_home.join("missing-gemini-oauth.json");

    chat.dispatch_command_with_args(
        SlashCommand::Login,
        format!("gemini --import {}", missing_import_path.display()),
        Vec::new(),
    );
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Gemini OAuth import is no longer native."));
    assert_eq!(chat.config.model_provider_id, OPENAI_PROVIDER_ID);
}

#[tokio::test]
async fn slash_auth_provider_remove_removes_provider_credential() {
    ...
    chat.config.cli_auth_credentials_store_mode = AuthCredentialsStoreMode::File;
    seed_provider_oauth_credential(&mut chat, "provider-a", "provider-a");

    chat.dispatch_command_with_args(
        SlashCommand::Auth,
        "provider-a remove".to_string(),
        Vec::new(),
    );
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert!(
        rendered.contains("Removed OAuth credential for provider Provider A (provider-a)."),
        "unexpected removal output: {rendered}"
    );
    ...
    .expect("auth file should read")
    .expect("auth file should exist");
    assert!(auth.provider_oauth_credentials.is_empty());
}

#[tokio::test]
async fn slash_auth_provider_remove_removes_imported_gemini_credential() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.config.model_provider_id = "gemini".to_string();
    chat.config.model_provider = ModelProviderInfo::create_gemini_provider();
    chat.config.cli_auth_credentials_store_mode = AuthCredentialsStoreMode::File;
    seed_provider_oauth_credential(&mut chat, "gemini", "gemini:demo-project");

    chat.dispatch_command_with_args(SlashCommand::Auth, "gemini remove".to_string(), Vec::new());
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert!(
        rendered.contains("Removed OAuth credential for provider Gemini (gemini)."),
        "unexpected removal output: {rendered}"
    );
...
#[tokio::test]
async fn slash_auth_provider_remove_not_configured() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("No OAuth credential configured for \"provider-a\"."));
}

#[tokio::test]
async fn slash_auth_provider_remove_unknown_provider() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    chat.dispatch_command_with_args(
        SlashCommand::Auth,
        "missing-provider remove".to_string(),
        Vec::new(),
    );
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("No provider matches \"missing-provider\" for auth removal."));
}

#[tokio::test]
async fn slash_login_gemini_import_stores_gemini_oauth_credential() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.config.cli_auth_credentials_store_mode = AuthCredentialsStoreMode::File;
    let (_import_tempdir, import_path) = make_gemini_oauth_import_file(
        r#"{
            "client_id": "client-id",
    ...
    let import_path_display = import_path.to_string_lossy().to_string();

    chat.dispatch_command_with_args(
        SlashCommand::Login,
        format!("gemini --import {}", import_path.display()),
        Vec::new(),
    );
    ...
    assert!(!rendered.contains(&import_path_display));
...
#[tokio::test]
async fn slash_login_gemini_shows_native_login_options() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.dispatch_command_with_args(SlashCommand::Login, "gemini".to_string(), Vec::new());
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Gemini OAuth is not a native login mode."));
    assert!(rendered.contains("external OpenAI-compatible provider"));
    assert!(!rendered.contains("Usage: /login gemini"));
}

#[tokio::test]
async fn slash_login_antigravity_shows_import_options() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.dispatch_command_with_args(SlashCommand::Login, "antigravity".to_string(), Vec::new());
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Antigravity OAuth is not a native login mode."));
    assert!(rendered.contains("external OpenAI-compatible provider"));
}

#[tokio::test]
async fn slash_login_antigravity_import_stores_provider_oauth_credential() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    chat.dispatch_command_with_args(
        SlashCommand::Login,
        format!("antigravity --import {}", import_path.display()),
        Vec::new(),
    );
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Antigravity OAuth import is no longer native."));
    assert!(!rendered.contains("access-secret"));
    assert!(!rendered.contains("refresh-secret"));
    assert!(!rendered.contains("client-secret"));
    assert!(!rendered.contains(&import_path_display));
...
#[tokio::test]
async fn slash_login_antigravity_import_rejects_missing_refresh_token() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
...
#[tokio::test]
async fn slash_login_kimi_shows_import_options() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.dispatch_command_with_args(SlashCommand::Login, "kimi".to_string(), Vec::new());
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Kimi OAuth is not a native login mode."));
    assert!(rendered.contains("external OpenAI-compatible provider"));
}

#[tokio::test]
async fn slash_login_kimi_import_stores_kimi_oauth_credential() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    chat.dispatch_command_with_args(
        SlashCommand::Login,
        format!("kimi --import {}", import_path.display()),
        Vec::new(),
    );
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Kimi OAuth import is no longer native."));
    assert!(!rendered.contains("kimi-access-secret"));
    assert!(!rendered.contains("kimi-refresh-secret"));
    assert!(!rendered.contains(&import_path_display));
...
#[tokio::test]
async fn slash_auth_gemini_import_stores_gemini_oauth_credential() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
...
#[tokio::test]
async fn slash_auth_kimi_remove_removes_imported_kimi_credential() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    credential.refresh_token = Some("kimi-refresh-token-redacted".to_string());
    upsert_provider_oauth_credential(
        chat.config.codex_home.as_path(),
        chat.config.cli_auth_credentials_store_mode,
        credential,
    ...
    chat.dispatch_command_with_args(SlashCommand::Auth, "kimi remove".to_string(), Vec::new());
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Removed OAuth credential for provider Kimi (kimi)."));
...
#[tokio::test]
async fn slash_login_gemini_import_rejects_malformed_quotes() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.dispatch_command_with_args(
        SlashCommand::Login,
        "gemini --import \"unterminated".to_string(),
        Vec::new(),
    );
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Provider OAuth import is no longer native."));
}

#[tokio::test]
async fn slash_login_gemini_import_rejects_invalid_file() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
...
#[tokio::test]
async fn slash_login_gemini_import_rejects_missing_client_id() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
...
#[tokio::test]
async fn slash_auth_provider_remove_refuses_multiple_gemini_credentials() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    chat.config.cli_auth_credentials_store_mode = AuthCredentialsStoreMode::File;
    seed_provider_oauth_credential(&mut chat, "gemini", "gemini:primary");
    seed_provider_oauth_credential(&mut chat, "gemini", "gemini:secondary");
    ...
    assert!(rendered.contains(
        "Multiple OAuth credentials configured for gemini; account-specific removal is not implemented yet."
    ));
    let auth = load_auth_dot_json(
        chat.config.codex_home.as_path(),
        chat.config.cli_auth_credentials_store_mode,
    ...
    .expect("auth file should read")
    .expect("auth file should exist");
    assert_eq!(auth.provider_oauth_credentials.len(), 2);
}

#[tokio::test]
async fn slash_status_auth_shows_provider_auth_rows() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    chat.config.model_providers = providers;

    chat.dispatch_command_with_args(SlashCommand::Status, "auth".to_string(), Vec::new());
    ...
    let rendered = cells
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Provider auth status:"));
    assert!(rendered.contains("Default route: global model/provider selection"));
    assert!(rendered.contains("Provider A (provider-a)"));
    assert!(rendered.contains("Provider B (provider-b)"));
    assert!(rendered.contains("Antigravity (antigravity)"));
    assert!(rendered.contains("Antigravity runtime is not available yet."));
}

#[tokio::test]
async fn slash_status_auth_filters_provider_rows_by_id_or_name() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    let rendered = cells
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Provider auth status for \"provider-b\":"));
    assert!(!rendered.contains("Provider A (provider-a)"));
    assert!(rendered.contains("Provider B (provider-b)"));

    chat.dispatch_command_with_args(
        SlashCommand::Status,
        "auth Provider A".to_string(),
        Vec::new(),
    );
    let cells = drain_insert_history(&mut rx);
    let rendered = cells
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Provider auth status for \"Provider A\":"));
    assert!(rendered.contains("Provider A (provider-a)"));
    assert!(!rendered.contains("Provider B (provider-b)"));
}

#[tokio::test]
async fn slash_status_auth_shows_kimi_runtime_blocked() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.dispatch_command_with_args(SlashCommand::Status, "auth kimi".to_string(), Vec::new());
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Provider auth status for \"kimi\":"));
    assert!(rendered.contains("Kimi (kimi)"));
    assert!(rendered.contains("blocked: Kimi runtime is not available yet."));
}

#[tokio::test]
async fn slash_status_auth_shows_claude_runtime_blocked() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;

    chat.dispatch_command_with_args(SlashCommand::Status, "auth claude".to_string(), Vec::new());
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert!(rendered.contains("Provider auth status for \"claude\":"));
    assert!(rendered.contains("Claude (claude)"));
    assert!(rendered.contains("blocked: Claude runtime is not available yet."));
}

#[tokio::test]
async fn slash_status_auth_ignores_gemini_oauth_account_project_and_expiry() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    credential.account_id = Some("alice@example.com".to_string());
    credential.expires_at = Some(1_700_000_000_000);
    upsert_provider_oauth_credential(
        chat.config.codex_home.as_path(),
        chat.config.cli_auth_credentials_store_mode,
        credential,
    ...
    assert!(rendered.contains("Provider auth status:"));
    assert!(rendered.contains("Default route: global model/provider selection"));
    assert!(rendered.contains("Gemini (gemini)"));
    assert!(rendered.contains("Gemini (gemini)"));
    assert!(!rendered.contains("alice@example.com"));
    assert!(!rendered.contains("profile: demo-project"));
    assert!(!rendered.contains("expires: 2023-11-14T22:13:20Z"));
    assert!(!rendered.contains("mock-access-token"));
}

#[tokio::test]
async fn slash_status_auth_shows_gemini_api_key_route_without_oauth_profile() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    unsafe {
        env::set_var(key, "provider-a-test-key");
    }
    ...
    let rendered = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert!(
        rendered.contains("Provider auth status:"),
        "rendered:\n{rendered}"
    );
    assert!(
        rendered.contains("Default route: global model/provider selection"),
        "rendered:\n{rendered}"
    );
    assert!(
        rendered.contains("Provider A (provider-a)"),
        "rendered:\n{rendered}"
    );
    assert!(rendered.contains("api key active"), "rendered:\n{rendered}");
    assert!(rendered.contains("API key"), "rendered:\n{rendered}");
    assert!(!rendered.contains("oauth profile"), "rendered:\n{rendered}");
}

#[tokio::test]
async fn slash_status_auth_orders_openai_first() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    ...
    let lines = rendered
        ...
        .collect::<Vec<_>>();
    let openai_index = lines
        .iter()
        .position(|line| line.contains("OpenAI (openai)"))
        .expect("expected openai auth row");
    let provider_a_index = lines
        .iter()
        .position(|line| line.contains("Provider A (provider-a)"))
        .expect("expected active provider auth row");
    let provider_b_index = lines
        .iter()
        .position(|line| line.contains("Provider B (provider-b)"))
        .expect("expected provider-b auth row");
    let amazon_index = lines
        .iter()
        .position(|line| line.contains("Amazon (amazon)"))
        .expect("expected defaulted provider auth row");

    assert!(openai_index < amazon_index);
    assert!(openai_index < provider_a_index);
    assert!(openai_index < provider_b_index);
}
...
async fn copy_shortcut_can_be_remapped() {
    ...
    keymap_config.global.copy = Some(ontocode_config::types::KeybindingsSpec::One(
        ontocode_config::types::KeybindingSpec("ctrl-x".to_string()),
    ));
    let runtime_keymap =
...
async fn active_goal_without_follow_up_suppresses_agent_turn_complete_notification() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            ontocode_app_server_protocol::ThreadGoalUpdatedNotification {
                thread_id: "thread-1".to_string(),
                turn_id: None,
                goal: ontocode_app_server_protocol::ThreadGoal {
                    thread_id: "thread-1".to_string(),
                    objective: "finish the benchmark".to_string(),
                    status: ontocode_app_server_protocol::ThreadGoalStatus::Active,
                    token_budget: None,
                    tokens_used: 0,
                    time_used_seconds: 0,
                    created_at: 1,
