...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<ontocode_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<ontocode_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(ontocode_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`ontocode-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, ontocode_protocol::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        ...
        let auth_status = match status.auth_status {
            ontocode_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
            ontocode_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
            ontocode_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
