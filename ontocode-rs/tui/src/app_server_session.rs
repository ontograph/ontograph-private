...
use ontocode_app_server_client::AppServerClient;
use ontocode_app_server_client::AppServerEvent;
use ontocode_app_server_client::AppServerRequestHandle;
use ontocode_app_server_client::TypedRequestError;
...
use ontocode_app_server_protocol::ExternalAgentConfigDetectParams;
use ontocode_app_server_protocol::ExternalAgentConfigDetectResponse;
use ontocode_app_server_protocol::ExternalAgentConfigImportParams;
use ontocode_app_server_protocol::ExternalAgentConfigImportResponse;
use ontocode_app_server_protocol::ExternalAgentConfigMigrationItem;
...
use ontocode_app_server_protocol::ThreadMemoryMode;
...
use ontocode_app_server_protocol::ThreadRealtimeAudioChunk;
...
use ontocode_app_server_protocol::UserInput;
use ontocode_otel::TelemetryAuthMode;
use ontocode_protocol::ThreadId;
use ontocode_protocol::approvals::GuardianAssessmentEvent;
...
use ontocode_protocol::openai_models::ModelUpgrade;
use ontocode_protocol::openai_models::ReasoningEffortPreset;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<ontocode_protocol::account::PlanType>,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        ...
        approvals_reviewer: ontocode_protocol::config_types::ApprovalsReviewer,
        permissions_override: TurnPermissionsOverride,
        workspace_roots: &[AbsolutePathBuf],
        model: String,
        effort: Option<ontocode_protocol::openai_models::ReasoningEffort>,
        summary: Option<ontocode_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<String>>,
        collaboration_mode: Option<ontocode_protocol::config_types::CollaborationMode>,
        personality: Option<ontocode_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<ontocode_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<ontocode_app_server_protocol::ApprovalsReviewer> {
...
fn sandbox_mode_from_permission_profile(
    ...
    cwd: &std::path::Path,
) -> Option<ontocode_app_server_protocol::SandboxMode> {
    match permission_profile {
        PermissionProfile::Disabled => {
            Some(ontocode_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        ...
        PermissionProfile::Managed { .. } => {
            ...
            if file_system_policy.has_full_disk_write_access() {
                permission_profile
                    ...
                    .is_enabled()
                    .then_some(ontocode_app_server_protocol::SandboxMode::DangerFullAccess)
            } else if file_system_policy.can_write_path_with_cwd(cwd, cwd) {
                Some(ontocode_app_server_protocol::SandboxMode::WorkspaceWrite)
            } else {
                Some(ontocode_app_server_protocol::SandboxMode::ReadOnly)
            }
        }
    }
}
...
fn turn_permissions_overrides(
    ...
    Option<ontocode_app_server_protocol::SandboxPolicy>,
    Option<String>,
) {
...
fn display_permission_profile_from_thread_response(
    sandbox: &ontocode_app_server_protocol::SandboxPolicy,
    cwd: &std::path::Path,
    config: &Config,
    thread_params_mode: ThreadParamsMode,
) -> PermissionProfile {
...
async fn thread_session_state_from_thread_response(
    ...
    reasoning_effort: Option<ontocode_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
    ...
    let history_config =
        ontocode_message_history::HistoryConfig::new(config.codex_home.clone(), &config.history);
    let (log_id, entry_count) = ontocode_message_history::history_metadata(&history_config).await;
...
mod tests {
    ...
    use ontocode_protocol::config_types::Verbosity;
    use ontocode_protocol::config_types::WebSearchMode;
    ...
    fn rate_limit_snapshot(limit_id: &str) -> RateLimitSnapshot {
        RateLimitSnapshot {
            ...
            limit_name: None,
            primary: Some(ontocode_app_server_protocol::RateLimitWindow {
                used_percent: 0,
    ...
    fn legacy_turn_permissions_project_to_sandbox_when_explicitly_overridden() {
        ...
        assert_eq!(
            sandbox_policy,
            Some(ontocode_app_server_protocol::SandboxPolicy::ReadOnly {
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: ontocode_app_server_protocol::Thread {
                ...
                source: ontocode_app_server_protocol::SessionSource::Cli,
                thread_source: None,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                ...
                turns: vec![Turn {
                    ...
                    items: vec![
                        ontocode_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            client_id: None,
                            content: vec![ontocode_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        ontocode_app_server_protocol::ThreadItem::AgentMessage {
            ...
            approval_policy: ontocode_app_server_protocol::AskForApproval::Never,
            approvals_reviewer: ontocode_app_server_protocol::ApprovalsReviewer::User,
    ...
    async fn embedded_thread_response_uses_local_config_profile() {
        ...
        assert_eq!(
            display_permission_profile_from_thread_response(
                &ontocode_app_server_protocol::SandboxPolicy::DangerFullAccess,
                cwd.as_path(),
                &config,
                ThreadParamsMode::Embedded,
            ),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        let history_config = ontocode_message_history::HistoryConfig::new(
            config.codex_home.clone(),
            &config.history,
        );

        ontocode_message_history::append_entry("older", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ontocode_message_history::append_entry("newer", &thread_id, &history_config)
            .await
            .expect("history append should succeed");
        ...
        let session = thread_session_state_from_thread_response(
            ...
            ontocode_protocol::config_types::ApprovalsReviewer::User,
            PermissionProfile::read_only(),
            /*active_permission_profile*/ None,
            test_path_buf("/tmp/project").abs(),
            Vec::new(),
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(ontocode_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(ontocode_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
