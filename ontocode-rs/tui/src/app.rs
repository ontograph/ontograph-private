...
use ontocode_ansi_escape::ansi_escape_line;
...
use ontocode_app_server_protocol::CodexErrorInfo as AppServerCodexErrorInfo;
...
use ontocode_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use ontocode_app_server_protocol::SkillErrorInfo;
...
use ontocode_app_server_protocol::WriteStatus;
...
use ontocode_config::types::MemoriesToml;
use ontocode_config::types::ModelAvailabilityNuxConfig;
#[cfg(target_os = "windows")]
use ontocode_config::types::WindowsToml;
...
use ontocode_models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use ontocode_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
use ontocode_rollout::StateDbHandle;
use ontocode_terminal_detection::user_agent;
use ontocode_utils_absolute_path::AbsolutePathBuf;
use ontocode_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
...
const ANTIGRAVITY_PROVIDER_ID: &str = "antigravity";
const ANTIGRAVITY_RUNTIME_DISABLED_REASON: &str = "Antigravity runtime is not available yet.";
const CLAUDE_PROVIDER_ID: &str = "claude";
const CLAUDE_RUNTIME_DISABLED_REASON: &str = "Claude runtime is not available yet.";
const EXTERNAL_EDITOR_HINT: &str = "Save and close external editor to continue.";
const KIMI_PROVIDER_ID: &str = "kimi";
const KIMI_RUNTIME_DISABLED_REASON: &str = "Kimi runtime is not available yet.";
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        ontocode_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&ontocode_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&ontocode_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[ontocode_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&ontocode_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<ontocode_app_server_protocol::CommandExecutionApprovalDecision> {
    use ontocode_app_server_protocol::CommandExecutionApprovalDecision;
    use ontocode_app_server_protocol::NetworkPolicyRuleAction;
...
fn session_summary(
    ...
    let resume_hint = resumable_thread.as_ref().and_then(|thread| {
        ontocode_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
    });
...
async fn build_provider_model_groups(
    config: &Config,
    available_models: &[ModelPreset],
) -> Vec<ProviderModelGroup> {
    let mut groups = Vec::new();
    if config.model_provider_id == GEMINI_CLI_PROVIDER_ID {
        groups.push(crate::model_catalog::gemini_cli_disabled_provider_group());
    } else if let Some(disabled_reason) = provider_runtime_disable_reason(&config.model_provider_id)
    {
        groups.push(ProviderModelGroup {
            provider_id: config.model_provider_id.clone(),
            display_name: config.model_provider.name.clone(),
            models: Vec::new(),
    ...
    for provider_id in provider_ids {
        ...
        if let Some(disabled_reason) = provider_runtime_disable_reason(&provider_id) {
            groups.push(ProviderModelGroup {
        ...
        let provider = create_model_provider(provider_info.clone(), /*auth_manager*/ None);
        match provider.runtime_engine() {
            ...
            | ProviderRuntimeEngine::OpenAiResponses => {
                ...
                if !models.is_empty() {
                    groups.push(ProviderModelGroup {
                        provider_id,
                        display_name: provider_info.name.clone(),
                        models,
                        disabled_reason: None,
                    });
                }
    ...
    if !groups
        .iter()
        .any(|group| group.provider_id == GEMINI_PROVIDER_ID)
    {
        groups.push(crate::model_catalog::gemini_provider_group());
    }
    if !groups
        .iter()
        .any(|group| group.provider_id == GEMINI_CLI_PROVIDER_ID)
    {
        groups.push(crate::model_catalog::gemini_cli_disabled_provider_group());
    }
...
fn provider_runtime_disable_reason(provider_id: &str) -> Option<&'static str> {
    match provider_id {
        KIMI_PROVIDER_ID => Some(KIMI_RUNTIME_DISABLED_REASON),
        ANTIGRAVITY_PROVIDER_ID => Some(ANTIGRAVITY_RUNTIME_DISABLED_REASON),
        CLAUDE_PROVIDER_ID => Some(CLAUDE_RUNTIME_DISABLED_REASON),
        _ => None,
    }
}
...
pub(crate) struct App {
    ...
    pub(crate) feedback: ontocode_feedback::CodexFeedback,
...
impl App {
    ...
    pub async fn run(
        ...
        feedback: ontocode_feedback::CodexFeedback,
        is_first_run: bool,
        ...
        let provider_model_groups = build_provider_model_groups(&config, &available_models).await;
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            ontocode_login::default_client::originator().value,
