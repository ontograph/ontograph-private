...
use chrono::Utc;
use http::HeaderMap;
use ontocode_protocol::auth::PlanType;
use ontocode_protocol::error::CodexErr;
use ontocode_protocol::error::RetryLimitReachedError;
use ontocode_protocol::error::UnexpectedResponseError;
use ontocode_protocol::error::UsageLimitReachedError;
    ...
    match err {
        ...
        ApiError::Transport(transport) => match transport {
            ...
            TransportError::Network(msg) | TransportError::Build(msg) => {
                CodexErr::Stream(local_provider_hint(msg), None)
            }
        },
...
fn local_provider_hint(message: String) -> String {
    if message.contains("localhost") && message.contains("error sending request") {
        format!("{message}. Check that the local OpenAI-compatible server is running")
    } else {
        message
    }
}
