...
#[test]
fn map_api_error_adds_local_provider_hint_for_localhost_network_failure() {
    let err = map_api_error(ApiError::Transport(TransportError::Network(
        "error sending request for url (http://localhost:1234/v1/responses)".to_string(),
    )));
    ...
    assert_eq!(
        message,
        "error sending request for url (http://localhost:1234/v1/responses). Check that the local OpenAI-compatible server is running"
    );
}
