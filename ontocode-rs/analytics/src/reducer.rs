...
use ontocode_app_server_protocol::ClientResponse;
...
use ontocode_app_server_protocol::DynamicToolCallOutputContentItem;
...
use ontocode_app_server_protocol::WebSearchAction;
use ontocode_git_utils::collect_git_info;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn tool_item(
        notification: &'a ontocode_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
impl AnalyticsReducer {
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: ontocode_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
...
fn file_change_counts(
    changes: &[ontocode_app_server_protocol::FileUpdateChange],
) -> FileChangeCounts {
...
fn analytics_turn_status(status: ontocode_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        ontocode_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        ontocode_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        ontocode_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        ontocode_app_server_protocol::TurnStatus::InProgress => None,
    }
}
