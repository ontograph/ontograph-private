...
use ontocode_login::default_client::create_client;
...
impl AnalyticsEventsClient {
    ...
    pub fn track_subagent_thread_completed(&self, input: SubAgentThreadCompletedInput) {
        self.record_fact(AnalyticsFact::Custom(
            CustomAnalyticsFact::SubAgentThreadCompleted(Box::new(input)),
        ));
    }
...
async fn send_track_events_request(auth: &CodexAuth, url: &str, events: Vec<TrackEventRequest>) {
    ...
    let response = create_client()
        ...
        .headers(ontocode_model_provider::auth_provider_from_auth(auth).to_auth_headers())
