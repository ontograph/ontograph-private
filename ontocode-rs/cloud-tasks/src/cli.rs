...
pub enum Command {
    /// Submit a new Ontocode Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of an Ontocode Cloud task.
    Status(StatusCommand),
    /// List Ontocode Cloud tasks.
    List(ListCommand),
    /// Apply the diff for an Ontocode Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for an Ontocode Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Ontocode Cloud.
    ...
    /// Git branch to run in Ontocode Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Ontocode Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Ontocode Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Ontocode Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
