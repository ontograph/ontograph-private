...
use anyhow::anyhow;
use chrono::Utc;
use ontocode_cloud_tasks_client::TaskStatus;
use ontocode_git_utils::current_branch_name;
use ontocode_git_utils::default_branch_name;
use ontocode_login::default_client::get_codex_user_agent;
...
struct ApplyJob {
    task_id: ontocode_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn ontocode_cloud_tasks_client::CloudBackend>,
    base_url: String,
}
...
async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    ...
    if use_mock {
        return Ok(BackendContext {
            backend: Arc::new(ontocode_cloud_tasks_mock_client::MockClient),
            base_url,
        });
    }
    ...
    let mut http =
        ontocode_cloud_tasks_client::HttpClient::new(base_url.clone())?.with_user_agent(ua);
    ...
    let auth_provider = ontocode_model_provider::auth_provider_from_auth(&auth);
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let created = ontocode_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        &git_ref,
...
fn parse_task_id(raw: &str) -> anyhow::Result<ontocode_cloud_tasks_client::TaskId> {
    ...
    Ok(ontocode_cloud_tasks_client::TaskId(id.to_string()))
}
...
async fn collect_attempt_diffs(
    backend: &dyn ontocode_cloud_tasks_client::CloudBackend,
    task_id: &ontocode_cloud_tasks_client::TaskId,
) -> anyhow::Result<Vec<AttemptDiffData>> {
    let text =
        ontocode_cloud_tasks_client::CloudBackend::get_task_text(backend, task_id.clone()).await?;
    let mut attempts = Vec::new();
    if let Some(diff) =
        ontocode_cloud_tasks_client::CloudBackend::get_task_diff(backend, task_id.clone()).await?
    {
    ...
    if let Some(turn_id) = text.turn_id {
        let siblings = ontocode_cloud_tasks_client::CloudBackend::list_sibling_attempts(
            backend,
            task_id.clone(),
            turn_id,
        )
...
fn summary_line(summary: &ontocode_cloud_tasks_client::DiffSummary, colorize: bool) -> String {
...
fn format_task_status_lines(
    task: &ontocode_cloud_tasks_client::TaskSummary,
...
fn format_task_list_lines(
    tasks: &[ontocode_cloud_tasks_client::TaskSummary],
    base_url: &str,
    now: chrono::DateTime<Utc>,
    colorize: bool,
) -> Vec<String> {
...
async fn run_status_command(args: crate::cli::StatusCommand) -> anyhow::Result<()> {
    ...
    let summary =
        ontocode_cloud_tasks_client::CloudBackend::get_task_summary(&*ctx.backend, task_id).await?;
    let now = Utc::now();
...
async fn run_list_command(args: crate::cli::ListCommand) -> anyhow::Result<()> {
    ...
    let page = ontocode_cloud_tasks_client::CloudBackend::list_tasks(
        &*ctx.backend,
        env_filter.as_deref(),
        Some(args.limit),
...
async fn run_apply_command(args: crate::cli::ApplyCommand) -> anyhow::Result<()> {
    ...
    let attempts = collect_attempt_diffs(&*ctx.backend, &task_id).await?;
    let selected = select_attempt(&attempts, args.attempt)?;
    let outcome = ontocode_cloud_tasks_client::CloudBackend::apply_task(
        &*ctx.backend,
        task_id,
    ...
    if !matches!(
        outcome.status,
        ontocode_cloud_tasks_client::ApplyStatus::Success
    ) {
        std::process::exit(1);
    }
    Ok(())
...
fn level_from_status(status: ontocode_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        ontocode_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        ontocode_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        ontocode_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = ontocode_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = ontocode_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx
                            .send(Instant::now() + ontocode_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match ontocode_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, ontocode_cloud_tasks_client::ApplyStatus::Success) {
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    ...
                                    } else if let ontocode_tui::ComposerAction::Submitted(text) =
                                        page.composer.input(key)
                                    {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    ...
                                                    let result = ontocode_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, /*qa_mode*/ false, best_of_n).await;
                                    ...
                                    if page.composer.is_in_paste_burst() {
                                        let _ = frame_tx.send(
                                            Instant::now()
                                                + ontocode_tui::ComposerInput::recommended_flush_delay(),
                                        );
                                    }
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match ontocode_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match ontocode_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = ontocode_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match ontocode_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
mod tests {
    ...
    use crossterm::event::KeyModifiers;
    use ontocode_cloud_tasks_client::DiffSummary;
    use ontocode_cloud_tasks_client::TaskId;
    use ontocode_cloud_tasks_client::TaskStatus;
    use ontocode_cloud_tasks_client::TaskSummary;
    use ontocode_cloud_tasks_mock_client::MockClient;
    use ontocode_tui::ComposerAction;
    use ontocode_tui::ComposerInput;
