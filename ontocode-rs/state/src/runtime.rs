...
mod operational_evidence_import;
mod remote_control;
#[cfg(test)]
mod test_support;
mod threads;
...
const LEGACY_STATE_DB_LABEL: &str = "legacy CODEX_HOME state DB";
const LEGACY_LOGS_DB_LABEL: &str = "legacy CODEX_HOME log DB";
const LEGACY_GOALS_DB_LABEL: &str = "legacy CODEX_HOME goals DB";
const LEGACY_MEMORIES_DB_LABEL: &str = "legacy CODEX_HOME memories DB";
...
pub fn runtime_db_paths(codex_home: &Path) -> Vec<RuntimeDbPath> {
    ...
    if let Some(legacy_codex_home) = legacy_codex_state_home(codex_home) {
        paths.extend(RUNTIME_DBS.iter().map(|spec| RuntimeDbPath {
            label: legacy_runtime_db_label(spec),
            path: spec.path(legacy_codex_home.as_path()),
        }));
    }
...
fn legacy_codex_state_home(codex_home: &Path) -> Option<PathBuf> {
    if codex_home.file_name()? != ".ontocode" {
        return None;
    }

    codex_home.parent().map(|parent| parent.join(".codex"))
}

fn legacy_runtime_db_label(spec: &RuntimeDbSpec) -> &'static str {
    if spec.filename == STATE_DB_FILENAME {
        LEGACY_STATE_DB_LABEL
    } else if spec.filename == LOGS_DB_FILENAME {
        LEGACY_LOGS_DB_LABEL
    } else if spec.filename == GOALS_DB_FILENAME {
        LEGACY_GOALS_DB_LABEL
    } else if spec.filename == MEMORIES_DB_FILENAME {
        LEGACY_MEMORIES_DB_LABEL
    } else {
        spec.label
    }
}
...
mod tests {
    ...
    #[test]
    fn runtime_db_paths_include_legacy_codex_fallback_for_default_ontocode_home() {
        let base = unique_temp_dir();
        let ontocode_home = base.join(".ontocode");
        let codex_home = base.join(".codex");

        let paths = runtime_db_paths(ontocode_home.as_path());
        ...
        assert_eq!(
            ...
            vec![
                RuntimeDbPath {
                    label: "state DB",
                    path: ontocode_home.join(STATE_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "log DB",
                    path: ontocode_home.join(LOGS_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "goals DB",
                    path: ontocode_home.join(GOALS_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "memories DB",
                    path: ontocode_home.join(MEMORIES_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "legacy CODEX_HOME state DB",
                    path: codex_home.join(STATE_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "legacy CODEX_HOME log DB",
                    path: codex_home.join(LOGS_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "legacy CODEX_HOME goals DB",
                    path: codex_home.join(GOALS_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "legacy CODEX_HOME memories DB",
                    path: codex_home.join(MEMORIES_DB_FILENAME),
                },
            ]
        );
    ...
    #[test]
    fn runtime_db_paths_do_not_add_legacy_fallback_for_custom_home() {
        let home = unique_temp_dir().join("custom-home");

        let paths = runtime_db_paths(home.as_path());
        ...
        assert_eq!(
            ...
            vec![
                RuntimeDbPath {
                    label: "state DB",
                    path: home.join(STATE_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "log DB",
                    path: home.join(LOGS_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "goals DB",
                    path: home.join(GOALS_DB_FILENAME),
                },
                RuntimeDbPath {
                    label: "memories DB",
                    path: home.join(MEMORIES_DB_FILENAME),
                },
            ]
        );
