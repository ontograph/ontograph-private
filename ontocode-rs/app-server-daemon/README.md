# ontocode-app-server-daemon

> `ontocode-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`ontocode-app-server-daemon` backs the machine-readable `codex app-server`
