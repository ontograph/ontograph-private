...
//!
//! This crate wraps [`ontocode_app_server::in_process`] behind a single async API
...
//! [`InProcessClientHandle`](ontocode_app_server::in_process::InProcessClientHandle),
...
pub use ontocode_app_server::app_server_control_socket_path;
pub use ontocode_app_server::in_process::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
pub use ontocode_app_server::in_process::InProcessServerEvent;
use ontocode_app_server::in_process::InProcessStartArgs;
use ontocode_app_server::in_process::LogDbLayer;
pub use ontocode_app_server::in_process::StateDbHandle;
...
use ontocode_app_server_protocol::Result as JsonRpcResult;
...
use ontocode_config::NoopThreadConfigLoader;
use ontocode_config::RemoteThreadConfigLoader;
...
pub mod legacy_core {
    pub use ontocode_core::DEFAULT_AGENTS_MD_FILENAME;
    pub use ontocode_core::LOCAL_AGENTS_MD_FILENAME;
    pub use ontocode_core::McpManager;
    pub use ontocode_core::check_execpolicy_for_warnings;
    pub use ontocode_core::format_exec_policy_error_with_source;
    pub use ontocode_core::grant_read_root_non_elevated;
    pub use ontocode_core::web_search_detail;

    pub mod config {
        pub use ontocode_core::config::*;

        pub mod edit {
            pub use ontocode_core::config::edit::*;
        }
    }

    pub mod connectors {
        pub use ontocode_core::connectors::*;
    }

    pub mod otel_init {
        pub use ontocode_core::otel_init::*;
    }

    pub mod personality_migration {
        pub use ontocode_core::personality_migration::*;
    }

    pub mod review_format {
        pub use ontocode_core::review_format::*;
    }

    pub mod review_prompts {
        pub use ontocode_core::review_prompts::*;
    }

    pub mod test_support {
        pub use ontocode_core::test_support::*;
    }

    pub mod util {
        pub use ontocode_core::util::*;
    }

    pub mod windows_sandbox {
        pub use ontocode_core::windows_sandbox::*;
    }
}
...
/// - [`ontocode_app_server::in_process::InProcessClientHandle`], which speaks to
...
impl InProcessAppServerClient {
    ...
    pub async fn start(args: InProcessClientStartArgs) -> IoResult<Self> {
        ...
        let mut handle =
            ontocode_app_server::in_process::start(args.into_runtime_start_args()).await?;
...
mod tests {
    ...
    use ontocode_app_server_protocol::ConfigRequirementsReadResponse;
    ...
    use ontocode_app_server_protocol::ServerNotification;
    use ontocode_app_server_protocol::SessionSource as ApiSessionSource;
    ...
    use ontocode_core::init_state_db;
    use ontocode_uds::UnixListener;
    ...
    async fn start_test_client_with_capacity(
        ...
        let client = InProcessAppServerClient::start(InProcessClientStartArgs {
            ...
            client_name: "ontocode-app-server-client-test".to_string(),
    ...
    fn command_execution_output_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::CommandExecutionOutputDelta(
            ontocode_app_server_protocol::CommandExecutionOutputDeltaNotification {
    ...
    fn agent_message_delta_notification(delta: &str) -> ServerNotification {
        ServerNotification::AgentMessageDelta(
            ontocode_app_server_protocol::AgentMessageDeltaNotification {
                thread_id: "thread".to_string(),
                turn_id: "turn".to_string(),
                item_id: "item".to_string(),
                delta: delta.to_string(),
    ...
    fn item_completed_notification(text: &str) -> ServerNotification {
        ServerNotification::ItemCompleted(ontocode_app_server_protocol::ItemCompletedNotification {
            thread_id: "thread".to_string(),
            turn_id: "turn".to_string(),
            completed_at_ms: 0,
            item: ontocode_app_server_protocol::ThreadItem::AgentMessage {
                id: "item".to_string(),
                text: text.to_string(),
                phase: None,
    ...
    fn turn_completed_notification() -> ServerNotification {
        ServerNotification::TurnCompleted(ontocode_app_server_protocol::TurnCompletedNotification {
            thread_id: "thread".to_string(),
            turn: ontocode_app_server_protocol::Turn {
                id: "turn".to_string(),
                items_view: ontocode_app_server_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: ontocode_app_server_protocol::TurnStatus::Completed,
                error: None,
                started_at: None,
                completed_at: Some(0),
                duration_ms: Some(1),
    ...
    async fn typed_request_reports_json_rpc_errors() {
        ...
        let err = client
            .request_typed::<ConfigRequirementsReadResponse>(ClientRequest::ThreadRead {
                ...
                params: ontocode_app_server_protocol::ThreadReadParams {
    ...
    async fn threads_started_via_app_server_are_visible_through_typed_requests() {
        ...
        let response: ThreadStartResponse = client
            ...
            .expect("thread/start should succeed");
        let read = client
            .request_typed::<ontocode_app_server_protocol::ThreadReadResponse>(
    ...
    async fn forward_in_process_event_preserves_transcript_notifications_under_backpressure() {
        ...
        assert!(matches!(
            ...
            InProcessServerEvent::ServerNotification(ServerNotification::ItemCompleted(
                ...
                &notification.item,
                ontocode_app_server_protocol::ThreadItem::AgentMessage { text, .. } if text == "hello"
            )
        ));
        assert!(matches!(
            &events[4],
            InProcessServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                notification
            )) if notification.turn.status == ontocode_app_server_protocol::TurnStatus::Completed
        ));
    }
    ...
    async fn remote_typed_request_roundtrip_works() {
        ...
        let response: GetAccountResponse = client
            .request_typed(ClientRequest::GetAccount {
                request_id: RequestId::Integer(1),
                params: ontocode_app_server_protocol::GetAccountParams {
                    refresh_token: false,
                },
            })
            .await
        ...
        for event in &remaining_events {
            match event {
                ...
                AppServerEvent::ServerNotification(ServerNotification::TurnCompleted(
                    ...
                    == ontocode_app_server_protocol::TurnStatus::Completed =>
                {
    ...
    fn event_requires_delivery_marks_transcript_and_terminal_events() {
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ontocode_app_server_protocol::ServerNotification::TurnCompleted(
                    ontocode_app_server_protocol::TurnCompletedNotification {
                        thread_id: "thread".to_string(),
                        turn: ontocode_app_server_protocol::Turn {
                            id: "turn".to_string(),
                            items_view: ontocode_app_server_protocol::TurnItemsView::Full,
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ontocode_app_server_protocol::ServerNotification::AgentMessageDelta(
                    ontocode_app_server_protocol::AgentMessageDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
        ...
        assert!(event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ontocode_app_server_protocol::ServerNotification::ItemCompleted(
        ...
        assert!(!event_requires_delivery(
            &InProcessServerEvent::ServerNotification(
                ontocode_app_server_protocol::ServerNotification::CommandExecutionOutputDelta(
                    ontocode_app_server_protocol::CommandExecutionOutputDeltaNotification {
                        thread_id: "thread".to_string(),
                        turn_id: "turn".to_string(),
                        item_id: "item".to_string(),
    ...
    async fn runtime_start_args_use_remote_thread_config_loader_when_configured() {
        ...
        assert_eq!(
            err.code(),
            ontocode_config::ThreadConfigLoadErrorCode::RequestFailed
        );
    }
