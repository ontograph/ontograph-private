...
use ontocode_core::exec::process_exec_tool_call;
...
fn codex_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_ontocode-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> ontocode_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<ontocode_protocol::exec_output::ExecToolCallOutput> {
...
fn is_bwrap_unavailable_output(
    output: &ontocode_protocol::exec_output::ExecToolCallOutput,
) -> bool {
...
fn expect_denied(
    result: Result<ontocode_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> ontocode_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
