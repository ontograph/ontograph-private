...
use ontocode_api::RetryConfig as ApiRetryConfig;
use ontocode_api::is_azure_responses_provider;
...
use ontocode_protocol::error::EnvVarError;
...
impl ModelProviderInfo {
    ...
    pub fn create_gemini_provider() -> ModelProviderInfo {
        ModelProviderInfo {
            ...
            env_key_instructions: Some("Set GEMINI_API_KEY to a Google AI Studio API key.".into()),
            experimental_bearer_token: None,
            auth: None,
            aws: None,
            wire_api: WireApi::Responses,
    ...
    /// Canonical identity for the OAuth-backed Gemini CLI lane.
    ///
    /// Runtime execution stays disabled until the S4 adapter exists.
    pub fn create_gemini_cli_provider() -> ModelProviderInfo {
        ModelProviderInfo {
            name: GEMINI_CLI_PROVIDER_NAME.into(),
            base_url: Some(GEMINI_DEFAULT_BASE_URL.into()),
            env_key: None,
            env_key_instructions: None,
            experimental_bearer_token: None,
...
/// generally overridable. Gemini remains overridable for compatibility with
/// existing custom Gemini definitions, and Amazon Bedrock allows the user to
/// set `aws.profile` and `aws.region`.
