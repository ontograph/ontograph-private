...
#[test]
fn test_built_in_model_providers_include_gemini() {
    let providers = built_in_model_providers(/*openai_base_url*/ None);

    assert_eq!(
        providers.get(GEMINI_PROVIDER_ID),
        Some(&ModelProviderInfo::create_gemini_provider())
    );
}
...
#[test]
fn test_create_gemini_provider_uses_gemini_api_key() {
    assert_eq!(
        ModelProviderInfo::create_gemini_provider(),
        ModelProviderInfo {
            name: "Gemini".to_string(),
            base_url: Some(GEMINI_DEFAULT_BASE_URL.to_string()),
            env_key: Some("GEMINI_API_KEY".to_string()),
            env_key_instructions: Some(
                "Set GEMINI_API_KEY to a Google AI Studio API key.".to_string()
            ),
            experimental_bearer_token: None,
            auth: None,
            aws: None,
...
#[test]
fn test_create_gemini_cli_provider_uses_gemini_cli_identity() {
    assert_eq!(
...
#[test]
fn test_merge_configured_model_providers_keeps_builtin_gemini_when_auth_is_configured() {
    let base_dir = tempdir().unwrap();
    let configured_gemini = ModelProviderInfo {
        name: "Gemini".to_string(),
        base_url: Some("https://oauth.example.com/gemini/v1beta".to_string()),
        env_key: Some("CUSTOM_GEMINI_API_KEY".to_string()),
        auth: Some(ModelProviderAuthInfo {
            command: "./scripts/print-token".to_string(),
            args: vec!["--audience=gemini".to_string()],
            timeout_ms: NonZeroU64::new(5_000).unwrap(),
    ...
    let configured_model_providers =
        std::collections::HashMap::from([(GEMINI_PROVIDER_ID.to_string(), configured_gemini)]);
