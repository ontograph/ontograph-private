...
use ontocode_state::ThreadStateAuditRow;
...
async fn thread_inventory_check_for_roots(
    ...
    let state_db_path = ontocode_state::state_db_path(sqlite_home);
    ...
    let rows = match ontocode_state::read_thread_state_audit_rows(&state_db_path).await {
        Ok(rows) => rows,
        Err(err) => {
...
async fn thread_id_from_rollout(path: &Path) -> RolloutThreadId {
    ...
    ontocode_rollout::builder_from_items(items.as_slice(), path)
...
mod tests {
    ...
    impl Fixture {
        async fn new() -> Self {
            ...
            let _runtime = ontocode_state::StateRuntime::init(
        ...
        fn write_rollout(&self, archived: bool, timestamp: &str, thread_id: &str) -> PathBuf {
            ...
            let rollout_line = RolloutLine {
                ...
                item: RolloutItem::SessionMeta(ontocode_protocol::protocol::SessionMetaLine {
                    meta: ontocode_protocol::protocol::SessionMeta {
        ...
        async fn insert_thread_row(&self, id: &str, rollout_path: &Path, archived: bool) {
            let state_db_path = ontocode_state::state_db_path(self.sqlite_home.path());
