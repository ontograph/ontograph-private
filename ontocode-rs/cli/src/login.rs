...
use ontocode_login::ServerOptions;
use ontocode_login::login_with_access_token;
use ontocode_login::login_with_api_key;
use ontocode_login::logout_with_revoke;
use ontocode_login::run_device_code_login;
...
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match ontocode_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `ontocode login --device-auth` instead."
    );
}
