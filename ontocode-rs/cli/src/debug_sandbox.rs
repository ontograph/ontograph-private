...
#[cfg(target_os = "macos")]
use ontocode_core::spawn::CODEX_SANDBOX_ENV_VAR;
use ontocode_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use ontocode_protocol::config_types::SandboxMode;
use ontocode_protocol::permissions::NetworkSandboxPolicy;
use ontocode_sandboxing::landlock::allow_network_for_proxy;
use ontocode_sandboxing::landlock::create_linux_sandbox_command_args_for_permission_profile;
#[cfg(target_os = "macos")]
use ontocode_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use ontocode_sandboxing::seatbelt::create_seatbelt_command_args;
use ontocode_sandboxing::with_managed_mitm_ca_readable_root;
...
async fn run_command_under_sandbox(
    ...
    let network = network_proxy
        .as_ref()
        .map(ontocode_core::config::StartedNetworkProxy::proxy);
...
async fn run_command_under_windows_session(
    ...
    use ontocode_windows_sandbox::spawn_windows_sandbox_session_elevated_for_permission_profile;
    use ontocode_windows_sandbox::spawn_windows_sandbox_session_legacy;
...
mod tests {
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = ontocode_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
