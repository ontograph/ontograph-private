...
use ontocode_api::ResponsesWebsocketClient;
...
use ontocode_install_context::CodexPackageLayout;
use ontocode_install_context::InstallContext;
use ontocode_install_context::InstallMethod;
use ontocode_install_context::StandalonePlatform;
use ontocode_login::AuthDotJson;
use ontocode_login::AuthManager;
use ontocode_login::CODEX_ACCESS_TOKEN_ENV_VAR;
use ontocode_login::CODEX_API_KEY_ENV_VAR;
use ontocode_login::CodexAuth;
use ontocode_login::OPENAI_API_KEY_ENV_VAR;
use ontocode_login::default_client::build_reqwest_client;
use ontocode_login::default_client::default_headers;
use ontocode_login::load_auth_dot_json;
use ontocode_model_provider::ProviderCapabilities;
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.codex_home.join(ontocode_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    let provider_capabilities = active_provider_capabilities(config);
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    if let Some(check) = provider_specific_auth_check(
        provider_capabilities.requires_openai_auth,
...
fn stored_auth_mode(auth: &ontocode_login::AuthDotJson) -> &'static str {
    match stored_auth_mode_value(auth) {
        ontocode_app_server_protocol::AuthMode::ApiKey => "api_key",
        ontocode_app_server_protocol::AuthMode::Chatgpt => "chatgpt",
        ontocode_app_server_protocol::AuthMode::ChatgptAuthTokens => "chatgpt_auth_tokens",
        ontocode_app_server_protocol::AuthMode::AgentIdentity => "agent_identity",
    }
}

fn stored_auth_mode_value(auth: &AuthDotJson) -> ontocode_app_server_protocol::AuthMode {
    if let Some(mode) = auth.auth_mode {
        return mode;
    }
    if auth.openai_api_key.is_some() {
        ontocode_app_server_protocol::AuthMode::ApiKey
    } else {
        ontocode_app_server_protocol::AuthMode::Chatgpt
    }
}
...
fn stored_auth_issues(
    ...
    match stored_auth_mode_value(auth) {
        ...
        ontocode_app_server_protocol::AuthMode::ChatgptAuthTokens => {
            match auth.tokens.as_ref() {
                Some(tokens) => {
                    if tokens.access_token.trim().is_empty() {
        ...
        ontocode_app_server_protocol::AuthMode::AgentIdentity => {
            if auth
                .agent_identity
                .as_deref()
...
async fn state_check(config: &Config) -> DoctorCheck {
    ...
    for db in ontocode_state::runtime_db_paths(&config.sqlite_home) {
...
async fn sqlite_integrity_detail(
    ...
    match ontocode_state::sqlite_integrity_check(path).await {
...
async fn websocket_reachability_check(
    ...
    let capabilities = runtime_provider.capabilities();
    details.extend([
        format!("runtime engine: {}", runtime_provider.runtime_engine()),
        format!(
            "capability namespace tools: {}",
        ...
        format!("capability web search: {}", capabilities.web_search),
        format!(
            "capability requires OpenAI auth: {}",
            capabilities.requires_openai_auth
        ),
        format!(
            "capability models route probe: {}",
            capabilities.supports_models_route_probe
        ),
    ]);
    push_proxy_env_details(&mut details);
...
fn provider_reachability_plan(config: &Config) -> ReachabilityPlan {
    ...
    provider_reachability_plan_from_parts(
        ...
        provider_capabilities.supports_models_route_probe,
        &config.chatgpt_base_url,
    )
}

fn active_provider_capabilities(config: &Config) -> ProviderCapabilities {
...
fn provider_auth_reachability_mode_from_auth(
    ...
    match stored_auth.map(stored_auth_mode_value) {
        Some(ontocode_app_server_protocol::AuthMode::ApiKey) => {
...
fn provider_reachability_plan_from_parts(
    ...
    supports_models_route_probe: bool,
...
mod tests {
    ...
    use clap::Parser;
    use ontocode_core::config::LoaderOverrides;
    ...
    async fn config_with_model_provider(
        codex_home: &Path,
        provider_id: &str,
        provider_name: &str,
        base_url: &str,
...
"#
    ...
    fn assert_detail(check: &DoctorCheck, expected: &str) {
        assert!(
            check.details.iter().any(|detail| detail == expected),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            ...
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/ontocode-execve-wrapper")),
        };
    ...
    #[tokio::test]
    async fn websocket_reachability_details_include_native_provider_diagnostics() {
        let temp = tempfile::tempdir().expect("tempdir");
        let config = config_with_model_provider(
            temp.path(),
            "claude",
            "Anthropic",
        ...
        assert_detail(&check, "runtime engine: anthropic-messages");
        assert_detail(&check, "capability namespace tools: true");
        assert_detail(&check, "capability image generation: false");
        assert_detail(&check, "capability web search: false");
        assert_detail(&check, "capability requires OpenAI auth: false");
        assert_detail(&check, "capability models route probe: false");
    }

    #[tokio::test]
    async fn websocket_reachability_details_keep_openai_compatible_provider_behavior() {
        let temp = tempfile::tempdir().expect("tempdir");
        let config = config_with_model_provider(
            temp.path(),
            "openai-compatible",
        ...
        assert_detail(&check, "runtime engine: openai-responses");
        assert_detail(&check, "capability namespace tools: true");
        assert_detail(&check, "capability image generation: true");
        assert_detail(&check, "capability web search: true");
        assert_detail(&check, "capability requires OpenAI auth: true");
        assert_detail(&check, "capability models route probe: true");
    }

    #[test]
    fn stored_auth_validation_rejects_missing_api_key() {
        let auth = AuthDotJson {
            auth_mode: Some(ontocode_app_server_protocol::AuthMode::ApiKey),
            openai_api_key: None,
            tokens: None,
            last_refresh: None,
            agent_identity: None,
