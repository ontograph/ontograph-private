...
use ontocode_tui::LocalStateDbStartupError;
...
pub(crate) async fn repair_files(
    ...
    for path in ontocode_state::runtime_db_paths(sqlite_home)
        .into_iter()
...
mod tests {
    ...
    async fn repair_backs_up_owned_database_files() -> std::io::Result<()> {
        ...
        let state_path = ontocode_state::state_db_path(temp_dir.path());
        let logs_path = ontocode_state::logs_db_path(temp_dir.path());
        let goals_path = ontocode_state::goals_db_path(temp_dir.path());
    ...
    async fn repair_replaces_blocking_sqlite_home_file() -> std::io::Result<()> {
        ...
        let startup_error = LocalStateDbStartupError::new(
            ontocode_state::state_db_path(sqlite_home.as_path()),
