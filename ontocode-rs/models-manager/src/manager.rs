...
/// Model manager for native providers that have provider-specific discovery
/// endpoints but must retain a conservative static fallback.
#[derive(Debug)]
pub struct NativeModelsManager {
    remote_models: RwLock<Vec<ModelInfo>>,
...
impl NativeModelsManager {
    /// Construct a native provider model manager with a static fallback catalog.
    pub fn new(
        auth_manager: Option<Arc<AuthManager>>,
        ...
        Self {
            remote_models: RwLock::new(fallback_catalog.models.clone()),
            fallback_models: fallback_catalog.models,
            endpoint_client,
            auth_manager,
        }
    }
...
impl OpenAiModelsManager {
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            ontocode_otel::start_global_timer("codex.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::client_version_to_whole();
...
#[async_trait]
impl ModelsManager for NativeModelsManager {
    async fn raw_model_catalog(&self, refresh_strategy: RefreshStrategy) -> ModelsResponse {
        if !matches!(refresh_strategy, RefreshStrategy::Offline) {
            let client_version = crate::client_version_to_whole();
            match self.endpoint_client.list_models(&client_version).await {
                Ok((models, _etag)) if !models.is_empty() => {
                    *self.remote_models.write().await = models;
                }
                Ok((_models, _etag)) => {
                    error!(
                        "native models discovery returned an empty catalog; using static fallback"
                    );
                    *self.remote_models.write().await = self.fallback_models.clone();
                }
                Err(err) => {
                    error!("failed to refresh native models catalog: {err}");
                    *self.remote_models.write().await = self.fallback_models.clone();
                }
            }
        }
    ...
    async fn refresh_if_new_etag(&self, _etag: String) {
        let _ = self.raw_model_catalog(RefreshStrategy::Online).await;
    }
}
