...
use ontocode_login::ExternalAuth;
use ontocode_login::ExternalAuthRefreshContext;
use ontocode_login::ExternalAuthTokens;
use ontocode_login::TokenData;
...
fn native_manager_for_tests(
    fallback_catalog: ModelsResponse,
    endpoint_client: Arc<dyn ModelsEndpointClient>,
) -> NativeModelsManager {
    NativeModelsManager::new(
        /*auth_manager*/ None,
        fallback_catalog,
        endpoint_client,
    )
...
#[derive(Debug)]
struct TestFallibleModelsEndpoint {
    responses: Mutex<VecDeque<CoreResult<Vec<ModelInfo>>>>,
    fetch_count: AtomicUsize,
}

impl TestFallibleModelsEndpoint {
    fn new(responses: Vec<CoreResult<Vec<ModelInfo>>>) -> Arc<Self> {
        Arc::new(Self {
            responses: Mutex::new(responses.into()),
            fetch_count: AtomicUsize::new(0),
        })
...
#[async_trait]
impl ModelsEndpointClient for TestFallibleModelsEndpoint {
    fn has_command_auth(&self) -> bool {
        false
    }
...
async fn chatgpt_auth_tokens_for_tests(codex_home: &Path) -> CodexAuth {
    let auth_dot_json = ontocode_login::AuthDotJson { provider_oauth_credentials: Default::default(),
        ...
        tokens: Some(TokenData {
            id_token: ontocode_login::token_data::parse_chatgpt_jwt_claims(
...
#[tokio::test]
async fn native_models_manager_uses_dynamic_catalog_when_online() {
    let fallback = remote_model("static-native", "Static Native", /*priority*/ 0);
    let dynamic = remote_model("dynamic-native", "Dynamic Native", /*priority*/ 0);
    let endpoint = TestFallibleModelsEndpoint::new(vec![Ok(vec![dynamic.clone()])]);
    let manager = native_manager_for_tests(
        ModelsResponse {
            models: vec![fallback],
        },
        endpoint.clone(),
    ...
    assert_eq!(catalog.models, vec![dynamic]);
    assert_eq!(endpoint.fetch_count(), 1);
}

#[tokio::test]
async fn native_models_manager_keeps_static_catalog_offline() {
    let fallback = remote_model("static-native", "Static Native", /*priority*/ 0);
    let dynamic = remote_model("dynamic-native", "Dynamic Native", /*priority*/ 0);
    let endpoint = TestFallibleModelsEndpoint::new(vec![Ok(vec![dynamic])]);
    let manager = native_manager_for_tests(
        ModelsResponse {
            models: vec![fallback.clone()],
        },
        endpoint.clone(),
    );
    ...
    assert_eq!(catalog.models, vec![fallback]);
    assert_eq!(endpoint.fetch_count(), 0);
}

#[tokio::test]
async fn native_models_manager_falls_back_to_static_catalog_on_refresh_failure() {
    let fallback = remote_model("static-native", "Static Native", /*priority*/ 0);
    let endpoint = TestFallibleModelsEndpoint::new(vec![Err(
        ontocode_protocol::error::CodexErr::UnsupportedOperation(
