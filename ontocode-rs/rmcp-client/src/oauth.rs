...
use ontocode_keyring_store::DefaultKeyringStore;
use ontocode_keyring_store::KeyringStore;
...
impl StoredOAuthTokens {
    ...
    pub fn to_provider_credential_routing_view(&self) -> ProviderCredentialRoutingView {
        let mut view = ProviderCredentialRoutingView::new(
            ...
            ProviderCredentialSourceKind::McpOAuth,
            ProviderCredentialAuthKind::OAuthBearer,
            self.server_name.clone(),
        );
        view.endpoint = Some(self.url.clone());
        ...
        view.provenance = Some("mcp_oauth".to_string());
        view
    }
    ...
    pub fn to_provider_oauth_credential(&self) -> ProviderOAuthCredential {
        let mut credential = ProviderOAuthCredential::new(
            ...
            ProviderCredentialSourceKind::McpOAuth,
            self.server_name.clone(),
            self.token_response.0.access_token().secret().to_string(),
        );
        credential.endpoint = Some(self.url.clone());
        credential.client_id = Some(self.client_id.clone());
        credential.scopes = self
            .token_response
            .0
            .scopes()
            .map(|scopes| scopes.iter().map(|scope| scope.to_string()).collect())
        ...
        credential.provenance = Some("mcp_oauth".to_string());
...
impl OAuthPersistor {
    ...
    pub(crate) async fn current_tokens(&self) -> Option<StoredOAuthTokens> {
        self.inner.last_credentials.lock().await.clone()
    }
}
...
fn validate_stored_oauth_tokens(tokens: &StoredOAuthTokens) -> Result<()> {
    ...
    if let Some(scopes) = token_response.scopes() {
        for scope in scopes {
            ensure_non_empty_oauth_field("scope", scope.as_ref())?;
        }
    }
...
mod tests {
    ...
    #[test]
    fn stored_oauth_tokens_produce_redacted_provider_credential_routing_view() {
        let tokens = sample_tokens();

        let view = tokens.to_provider_credential_routing_view();

        assert_eq!(view.provider_name, "test-server");
        assert_eq!(view.credential_id, "test-server");
        assert_eq!(view.endpoint.as_deref(), Some("https://example.test"));
        assert_eq!(view.client_id.as_deref(), Some("client-id"));
        assert_eq!(
            view.scopes,
            vec!["scope-a".to_string(), "scope-b".to_string()]
        );
        assert_eq!(view.provenance.as_deref(), Some("mcp_oauth"));
    }

    #[test]
    fn stored_oauth_tokens_convert_to_provider_oauth_credential() {
        let credential = sample_tokens().to_provider_oauth_credential();

        assert_eq!(credential.provider_name, "test-server");
        assert_eq!(credential.credential_id, "test-server");
        assert_eq!(credential.endpoint.as_deref(), Some("https://example.test"));
        assert_eq!(credential.client_id.as_deref(), Some("client-id"));
        ...
        assert_eq!(credential.provenance.as_deref(), Some("mcp_oauth"));
    }

    #[tokio::test]
    async fn oauth_persistor_refresh_adapter_reports_healthy_tokens() {
        let authorization_manager = AuthorizationManager::new("https://example.test")
            .await
            .expect("authorization manager should initialize");
        let persistor = OAuthPersistor::new(
            "test-server".to_string(),
            "https://example.test".to_string(),
            std::sync::Arc::new(tokio::sync::Mutex::new(authorization_manager)),
            OAuthCredentialsStoreMode::File,
            Some(sample_tokens()),
        );
        let adapter = crate::refresh_adapter::OAuthPersistorRefreshAdapter::new(persistor);

        assert_eq!(
            adapter
                .current_refresh_descriptor()
                .await
                .expect("tokens should produce a descriptor")
                .state,
            ontocode_provider_auth::ProviderCredentialRefreshState::RefreshHealthy
        );
    }
