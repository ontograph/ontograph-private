...
`ontocode-otel` is the OpenTelemetry integration crate for Codex. It provides:

- Provider wiring for log/trace/metric exporters (`ontocode_otel::OtelProvider`
  and `ontocode_otel::provider`).
- Session-scoped business event emission via `ontocode_otel::SessionTelemetry`.
- Low-level metrics APIs via `ontocode_otel::metrics`.
- Trace-context helpers via `ontocode_otel::trace_context` and crate-root re-exports.
...
```rust
use ontocode_otel::config::OtelExporter;
use ontocode_otel::config::OtelHttpProtocol;
use ontocode_otel::config::OtelSettings;
...
`ontocode-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Codex-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use ontocode_otel::config::{OtelExporter, OtelHttpProtocol};
