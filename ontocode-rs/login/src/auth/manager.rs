...
use ontocode_agent_identity::decode_agent_identity_jwt;
use ontocode_agent_identity::fetch_agent_identity_jwks;
use ontocode_app_server_protocol::AuthMode;
use ontocode_app_server_protocol::AuthMode as ApiAuthMode;
...
use ontocode_client::CodexHttpClient;
...
use ontocode_protocol::auth::RefreshTokenFailedError;
use ontocode_protocol::auth::RefreshTokenFailedReason;
...
impl ExternalAuthTokens {
    ...
    pub fn to_provider_credential_routing_view(&self) -> ProviderCredentialRoutingView {
        let credential_id = self
            ...
            .unwrap_or_else(|| "openai-access-token".to_string());
        ...
        view.account_id = self
            .chatgpt_metadata
            .as_ref()
            .map(|metadata| metadata.account_id.clone());
        view.provenance = Some("login".to_string());
        view
    }
    ...
    pub fn to_provider_oauth_credential(&self) -> ProviderOAuthCredential {
        ...
        credential.account_id = self
            .chatgpt_metadata
            .as_ref()
            .map(|metadata| metadata.account_id.clone());
        credential.provenance = Some("login".to_string());
        credential
    }
...
pub fn load_provider_oauth_credential_by_provider_id(
    codex_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    provider_id: &str,
    credential_id: &str,
) -> std::io::Result<Option<ProviderOAuthCredential>> {
    Ok(load_auth_dot_json(codex_home, auth_credentials_store_mode)?
        .and_then(|auth| auth.provider_oauth_credential_by_provider_id(provider_id, credential_id)))
}

pub fn remove_provider_oauth_credential(
    codex_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    provider_id: &str,
    credential_id: &str,
) -> std::io::Result<bool> {
    let Some(mut auth) = load_auth_dot_json(codex_home, auth_credentials_store_mode)? else {
        return Ok(false);
    };
    let removed = auth.remove_provider_oauth_credential(provider_id, credential_id);
    if removed {
...
pub fn remove_provider_oauth_credential_by_provider_id(
    codex_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    provider_id: &str,
    credential_id: &str,
) -> std::io::Result<bool> {
    remove_provider_oauth_credential(
        codex_home,
        auth_credentials_store_mode,
        provider_id,
        credential_id,
...
impl AuthManager {
    /// Returns the current stored provider OAuth credential for a provider id, if any.
    pub fn provider_oauth_credential_by_provider_id(
        &self,
        provider_id: &str,
    ) -> std::io::Result<Option<ProviderOAuthCredential>> {
        Ok(
            load_auth_dot_json(&self.codex_home, self.auth_credentials_store_mode)?.and_then(
                |auth| {
                    auth.provider_oauth_credentials
                        .into_iter()
    ...
    /// Returns provider OAuth credentials for request auth, refreshing expired credentials first.
    pub async fn provider_oauth_credential_for_auth(
        &self,
        provider_id: &str,
    ) -> std::io::Result<Option<ProviderOAuthCredential>> {
        ...
        let mut matching_credentials = auth_dot_json
            .provider_oauth_credentials
            .into_iter()
            .filter(|credential| credential.provider_id() == provider_id)
            .map(ProviderOAuthCredential::from);

        let Some(credential) = matching_credentials.next() else {
            return Ok(None);
        };

        if matching_credentials.next().is_some() {
            return Err(std::io::Error::other(
                "ambiguous provider OAuth credentials for request auth",
            ));
        }

        if credential.refresh_state() != ProviderCredentialRefreshState::RefreshEligible {
            return Ok(Some(credential));
        }
        self.refresh_provider_oauth_credential(credential)
            .await
            .map(Some)
    }

    /// Returns the provider OAuth credential for request auth, optionally
    /// targeting a specific credential/profile id.
    pub async fn provider_oauth_credential_for_request_auth(
        &self,
        provider_id: &str,
        credential_id: Option<&str>,
    ) -> std::io::Result<Option<ProviderOAuthCredential>> {
        let Some(credential_id) = credential_id else {
            return self.provider_oauth_credential_for_auth(provider_id).await;
        };
        ...
        let Some(credential) =
            auth_dot_json.provider_oauth_credential_by_provider_id(provider_id, credential_id)
        else {
            return Ok(None);
        };
    ...
    async fn refresh_provider_oauth_credential(
        &self,
        mut credential: ProviderOAuthCredential,
    ) -> std::io::Result<ProviderOAuthCredential> {
        #[derive(Deserialize)]
        ...
        let refresh_token = credential.refresh_token.clone().ok_or_else(|| {
            std::io::Error::other("provider OAuth credential has no refresh token")
        })?;
        let client_id = credential
            .client_id
            .clone()
            .ok_or_else(|| std::io::Error::other("provider OAuth credential has no client id"))?;
        let token_endpoint = credential.token_endpoint.clone().ok_or_else(|| {
            std::io::Error::other("provider OAuth credential has no token endpoint")
        })?;
        ...
        if !status.is_success() {
            ...
            return Err(std::io::Error::other(format!(
                "provider OAuth refresh failed with status {status}{suffix}"
            )));
        }
        let tokens: TokenResponse = response.json().await.map_err(std::io::Error::other)?;
        ...
        credential.expires_at = tokens
            ...
            .map(|timestamp| timestamp.timestamp() as u64 * 1000);
...
pub fn upsert_provider_oauth_credential(
    codex_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
    credential: ProviderOAuthCredential,
) -> std::io::Result<()> {
    let mut auth = load_auth_dot_json(codex_home, auth_credentials_store_mode)?.unwrap_or_default();
    auth.upsert_provider_oauth_credential(credential);
    save_auth(codex_home, &auth, auth_credentials_store_mode)
}

fn is_provider_oauth_only_auth(auth_dot_json: &AuthDotJson) -> bool {
    auth_dot_json.auth_mode.is_none()
        ...
        && !auth_dot_json.provider_oauth_credentials.is_empty()
}
...
async fn load_auth(
    ...
    if let Some(auth_dot_json) = ephemeral_storage.load()? {
        if is_provider_oauth_only_auth(&auth_dot_json) {
            return Ok(None);
        }
...
/// `ontocode_core::config::Config`, but this trait keeps `codex-login` independent
/// from `codex-core`.
...
impl AuthManager {
    ...
    pub(crate) fn should_refresh_proactively(auth: &CodexAuth) -> bool {
