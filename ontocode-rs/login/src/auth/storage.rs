...
use ontocode_agent_identity::AgentIdentityJwtClaims;
...
use ontocode_provider_auth::ProviderOAuthCredentialRecord;
...
pub struct AuthDotJson {
    ...
    pub provider_oauth_credentials: Vec<ProviderOAuthCredentialRecord>,
}
...
impl AuthDotJson {
    pub(crate) fn remove_provider_oauth_credential(
        &mut self,
        provider_id: &str,
        credential_id: &str,
    ) -> bool {
        let original_len = self.provider_oauth_credentials.len();
        self.provider_oauth_credentials
            .retain(|credential| !credential.matches_provider_id(provider_id, credential_id));
        original_len != self.provider_oauth_credentials.len()
    }

    pub(crate) fn provider_oauth_credential_by_provider_id(
        &self,
        provider_id: &str,
        credential_id: &str,
        ...
        self.provider_oauth_credentials
            .iter()
            .find(|credential| credential.matches_provider_id(provider_id, credential_id))
            .cloned()
            .map(Into::into)
    }

    pub(crate) fn upsert_provider_oauth_credential(&mut self, credential: ProviderOAuthCredential) {
        let record = ProviderOAuthCredentialRecord::from(&credential);
        if let Some(existing) = self.provider_oauth_credentials.iter_mut().find(|existing| {
            existing.matches_provider_id(record.provider_id(), &record.credential_id)
        }) {
            *existing = record;
        } else {
            self.provider_oauth_credentials.push(record);
        }
    }
}
