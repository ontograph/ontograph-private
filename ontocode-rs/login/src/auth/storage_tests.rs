...
use crate::auth::load_provider_oauth_credential_by_provider_id;
use crate::auth::remove_provider_oauth_credential;
use crate::auth::save_auth;
use crate::auth::upsert_provider_oauth_credential;
use keyring::Error as KeyringError;
use ontocode_keyring_store::tests::MockKeyringStore;
...
#[tokio::test]
async fn file_storage_loads_legacy_auth_json_without_provider_oauth_credentials()
-> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let storage = FileAuthStorage::new(codex_home.path().to_path_buf());
    ...
    std::fs::write(
        &auth_file,
        r#"{"OPENAI_API_KEY":"test-key","tokens":null,"last_refresh":null}"#,
    )?;

    let loaded = storage.load().context("failed to load legacy auth file")?;
    let loaded = loaded.expect("auth file should load");
    assert!(loaded.provider_oauth_credentials.is_empty());
...
#[tokio::test]
async fn provider_oauth_credential_helpers_round_trip_through_auth_storage() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let credential = {
        ...
        credential.provenance = Some("donor-import".to_string());
    ...
    let loaded = load_provider_oauth_credential_by_provider_id(
        codex_home.path(),
        AuthCredentialsStoreMode::File,
        "gemini-cli",
        "gemini-account-1",
    ...
    assert_eq!(auth_dot_json.provider_oauth_credentials.len(), 1);
    assert!(!format!("{auth_dot_json:?}").contains("secret-access-token"));
    assert!(!format!("{auth_dot_json:?}").contains("secret-refresh-token"));
    Ok(())
}

#[tokio::test]
async fn kimi_imported_provider_oauth_credentials_round_trip_through_auth_storage()
-> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let mut oauth = ProviderOAuthCredential::new(
        "kimi",
        ProviderCredentialSourceKind::ExternalImport,
        "kimi:imported",
        "test-access-token",
    );
    oauth.scopes = vec!["chat:read".to_string(), "chat:write".to_string()];
    oauth.expires_at = Some(33_336_362_096_000);
    oauth.provenance = Some("cli-proxy-api".to_string());
    oauth.refresh_token = Some("test-refresh-token".to_string());
    ...
    .expect("kimi provider oauth credential should load");
    assert_eq!(loaded, oauth);

    let auth_dot_json = load_auth_dot_json(codex_home.path(), AuthCredentialsStoreMode::File)?
        .expect("auth payload should exist");
    assert_eq!(auth_dot_json.provider_oauth_credentials.len(), 1);
    assert_eq!(
        auth_dot_json.provider_oauth_credentials[0].provider_name,
        "kimi"
    );
    assert_eq!(
        auth_dot_json.provider_oauth_credentials[0].credential_id,
        "kimi:imported"
    );
    let auth_debug = format!("{auth_dot_json:?}");
    assert!(!auth_debug.contains("test-access-token"));
    assert!(!auth_debug.contains("test-refresh-token"));

    Ok(())
}

#[tokio::test]
async fn provider_oauth_credential_helpers_remove_by_provider_and_id_through_auth_storage()
-> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let primary_api_key = "sk-test-key";
    let credential_keep = {
        let mut credential = ProviderOAuthCredential::new(
            "gemini-cli",
            ProviderCredentialSourceKind::ExternalImport,
        ...
        credential.account_id = Some("keep-account".to_string());
        credential
    };
    let credential_remove = {
        ...
        credential.account_id = Some("remove-account".to_string());
        credential.provenance = Some("donor-import".to_string());
        credential.refresh_token = Some("secret-refresh-token".to_string());
        credential
    };
    ...
    let mut auth = AuthDotJson {
        auth_mode: None,
        openai_api_key: Some(primary_api_key.to_string()),
        tokens: None,
        last_refresh: Some(Utc::now()),
        agent_identity: None,
        provider_oauth_credentials: vec![],
    };
    auth.upsert_provider_oauth_credential(credential_keep);
    auth.upsert_provider_oauth_credential(credential_remove);
    save_auth(codex_home.path(), &auth, AuthCredentialsStoreMode::File)?;

    let removed = remove_provider_oauth_credential(
        codex_home.path(),
        AuthCredentialsStoreMode::File,
        "gemini-cli",
        "gemini-account-2",
    ...
    let auth = load_auth_dot_json(codex_home.path(), AuthCredentialsStoreMode::File)?
        .expect("auth payload should exist");
    assert_eq!(auth.openai_api_key.as_deref(), Some(primary_api_key));
    ...
    assert_eq!(
        auth.provider_oauth_credentials[0].account_id.as_deref(),
        Some("keep-account")
    );

    let removed_missing = remove_provider_oauth_credential(
        codex_home.path(),
        AuthCredentialsStoreMode::File,
        "gemini-cli",
        "does-not-exist",
