use super::*;
use crate::auth::AuthManagerRefreshAdapter;
use crate::auth::storage::FileAuthStorage;
use crate::auth::storage::get_auth_file;
use crate::token_data::IdTokenInfo;
use ontocode_app_server_protocol::AuthMode;
use ontocode_protocol::account::PlanType as AccountPlanType;
use ontocode_protocol::auth::KnownPlan as InternalKnownPlan;
use ontocode_protocol::auth::PlanType as InternalPlanType;
...
#[tokio::test]
async fn loads_provider_oauth_credentials_without_overriding_primary_auth() {
    let dir = tempdir().unwrap();
    let credential = {
        ...
        credential.account_id = Some("gemini-account-1".to_string());
        credential.refresh_token = Some("secret-refresh-token".to_string());
        credential
    };

    super::upsert_provider_oauth_credential(
        dir.path(),
        AuthCredentialsStoreMode::File,
        credential.clone(),
    )
    ...
    let loaded = super::load_provider_oauth_credential_by_provider_id(
        dir.path(),
        AuthCredentialsStoreMode::File,
        "gemini-cli",
        "gemini-account-1",
    ...
    .unwrap()
    .expect("provider oauth credential should load");
...
#[tokio::test]
async fn auth_manager_returns_current_provider_oauth_credential_by_provider_id() {
    let dir = tempdir().unwrap();
    let credential = {
        let mut credential = ProviderOAuthCredential::new(
            "gemini-cli",
    ...
    assert_eq!(
        manager
            .provider_oauth_credential_by_provider_id("gemini-cli")
            .expect("provider oauth credential lookup should succeed"),
        Some(credential)
    );
}

#[tokio::test]
async fn auth_manager_refuses_ambiguous_provider_oauth_credentials_for_auth() {
    let dir = tempdir().unwrap();
    ...
    let credential_two = ProviderOAuthCredential::new(
        "gemini-cli",
        ProviderCredentialSourceKind::ExternalImport,
        "gemini-cli:workspace-2",
        "secret-access-token-2",
    );
    ...
    let error = manager
        .provider_oauth_credential_for_auth("gemini-cli")
        .await
        .expect_err("provider oauth request-auth lookup should fail closed");
    assert_eq!(
        error.to_string(),
        "ambiguous provider OAuth credentials for request auth"
    );
}

#[tokio::test]
async fn auth_manager_refreshes_expired_provider_oauth_credential_for_auth() {
    let dir = tempdir().unwrap();
    let server = MockServer::start().await;
    ...
    credential.client_id = Some("gemini-client-id".to_string());
    credential.token_endpoint = Some(format!("{}/token", server.uri()));
    credential.expires_at = Some(1);
    credential.refresh_token = Some("secret-refresh-token".to_string());

    super::upsert_provider_oauth_credential(dir.path(), AuthCredentialsStoreMode::File, credential)
        .unwrap();
    ...
    let refreshed = manager
        .provider_oauth_credential_for_auth("gemini")
        .await
        .expect("provider oauth credential should refresh")
        .expect("provider oauth credential should exist");

    assert_eq!(refreshed.access_token, "fresh-access-token");
    assert_ne!(refreshed.expires_at, Some(1));
    assert_eq!(
        manager
            .provider_oauth_credential_by_provider_id("gemini")
            .expect("provider oauth credential lookup should succeed")
...
#[tokio::test]
async fn auth_manager_refreshes_expired_provider_oauth_credential_for_request_auth_by_credential_id()
 {
    let dir = tempdir().unwrap();
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/token"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "access_token": "fresh-selected-access-token",
            "expires_in": 3600
        })))
        .expect(1)
        .mount(&server)
    ...
    super::upsert_provider_oauth_credential(dir.path(), AuthCredentialsStoreMode::File, selected)
        .unwrap();
    super::upsert_provider_oauth_credential(
        dir.path(),
        AuthCredentialsStoreMode::File,
    ...
    let refreshed = manager
        .provider_oauth_credential_for_request_auth("gemini", Some("gemini:workspace-1"))
        .await
        .expect("selected provider oauth credential should refresh")
        .expect("selected provider oauth credential should exist");

    assert_eq!(refreshed.access_token, "fresh-selected-access-token");
    assert_ne!(refreshed.expires_at, Some(1));
    assert_eq!(
        super::load_provider_oauth_credential_by_provider_id(
            dir.path(),
            AuthCredentialsStoreMode::File,
            "gemini",
            "gemini:workspace-1",
...
#[tokio::test]
async fn auth_manager_remove_provider_oauth_credential_by_provider_id_preserves_primary_and_unrelated_credentials()
-> anyhow::Result<()> {
    let dir = tempdir().unwrap();
    let credential_keep = {
        let mut credential = ProviderOAuthCredential::new(
            ...
            ProviderCredentialSourceKind::ExternalImport,
            "workspace-1",
            "secret-access-token-keep",
        );
        credential.account_id = Some("account-keep".to_string());
        credential
    };
    let credential_remove = {
        let mut credential = ProviderOAuthCredential::new(
            "gemini-cli",
            ProviderCredentialSourceKind::ExternalImport,
            "workspace-2",
            "secret-access-token-remove",
        );
        credential.account_id = Some("account-remove".to_string());
        credential.refresh_token = Some("secret-refresh-token".to_string());
        credential
    };
    ...
    super::upsert_provider_oauth_credential(
        dir.path(),
        AuthCredentialsStoreMode::File,
        credential_keep.clone(),
    )
    .unwrap();
    super::upsert_provider_oauth_credential(
        dir.path(),
        AuthCredentialsStoreMode::File,
        credential_remove.clone(),
    )
    .unwrap();
    let mut auth = super::load_auth_dot_json(dir.path(), AuthCredentialsStoreMode::File)?
        .expect("auth payload should exist");
    auth.auth_mode = Some(AuthMode::ApiKey);
    auth.openai_api_key = Some("sk-test-key".to_string());
    ...
    assert!(
        manager
            .remove_provider_oauth_credential("gemini-cli", "workspace-2")
            .unwrap()
    );

    let remaining = super::load_provider_oauth_credential_by_provider_id(
        dir.path(),
        AuthCredentialsStoreMode::File,
        "gemini-cli",
        "workspace-1",
    ...
    .unwrap();
    assert_eq!(remaining, Some(credential_keep.clone()));

    let removed_missing = super::remove_provider_oauth_credential(
        dir.path(),
        AuthCredentialsStoreMode::File,
        "gemini-cli",
    ...
    let auth = super::load_auth_dot_json(dir.path(), AuthCredentialsStoreMode::File)?
        .expect("auth payload should exist");
    assert_eq!(auth.openai_api_key.as_deref(), Some("sk-test-key"));
    assert_eq!(auth.provider_oauth_credentials.len(), 1);
    assert_eq!(
        auth.provider_oauth_credentials[0].credential_id.as_str(),
        "workspace-1"
    );
    Ok(())
}
...
#[test]
fn external_auth_tokens_produce_redacted_provider_credential_routing_view() {
    let view =
        ExternalAuthTokens::chatgpt("test-access-token", "acct_123", Some("pro".to_string()))
            .to_provider_credential_routing_view();

    assert_eq!(view.provider_name, "OpenAI");
    assert_eq!(view.credential_id, "acct_123");
    assert_eq!(view.account_id.as_deref(), Some("acct_123"));
    assert_eq!(view.provenance.as_deref(), Some("login"));
    assert_eq!(view.scopes, Vec::<String>::new());
}

#[test]
fn external_auth_tokens_convert_to_provider_oauth_credential() {
    let credential =
        ExternalAuthTokens::chatgpt("test-access-token", "acct_123", Some("pro".to_string()))
            .to_provider_oauth_credential();

    assert_eq!(credential.provider_name, "OpenAI");
    assert_eq!(credential.credential_id, "acct_123");
    assert_eq!(credential.account_id.as_deref(), Some("acct_123"));
    assert_eq!(credential.provenance.as_deref(), Some("login"));
    assert_eq!(credential.access_token, "test-access-token");
    assert_eq!(credential.refresh_token, None);
}

#[tokio::test]
async fn auth_manager_refresh_adapter_reports_healthy_chatgpt_auth() {
    let codex_home = tempdir().unwrap();
    write_auth_file(
        AuthFileParams {
            openai_api_key: None,
    ...
    .await;
    let adapter = AuthManagerRefreshAdapter::new(Arc::clone(&manager));

    assert_eq!(
        adapter.current_refresh_descriptor().await,
        Some(ontocode_provider_auth::ProviderCredentialRefreshDescriptor {
            credential_key: "openai:chatgpt".to_string(),
            routing: ontocode_protocol::credential_routing::ProviderCredentialRoutingSummary {
                provider_name: "openai".to_string(),
                source_kind:
                    ontocode_protocol::credential_routing::ProviderCredentialSourceKind::FirstPartyLogin,
                auth_kind:
                    ontocode_protocol::credential_routing::ProviderCredentialAuthKind::OAuthBearer,
                has_account_id: false,
                has_endpoint: false,
                has_client_id: false,
                scope_count: 0,
                expires_at: None,
                provenance: Some("auth_manager".to_string()),
            },
...
#[tokio::test]
async fn auth_manager_refresh_adapter_reports_non_refreshable_api_key_auth() {
    let codex_home = tempdir().unwrap();
    save_auth(
        codex_home.path(),
        &AuthDotJson {
    ...
    .expect("failed to persist api key auth");
    ...
    assert_eq!(
        adapter
            ...
            .await
            .expect("api key auth should be described")
            .state,
...
fn agent_identity_record(account_id: &str) -> AgentIdentityAuthRecord {
    let key_material = ontocode_agent_identity::generate_agent_key_material()
