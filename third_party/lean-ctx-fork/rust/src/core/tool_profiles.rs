...
impl ToolProfile {
    pub fn parse(s: &str) -> Option<Self> {
        match s.to_lowercase().as_str() {
            ...
            "ontocode" | "ontocode-readonly" => Some(Self::Ontocode),
    ...
    pub fn description(&self) -> &str {
        match self {
            ...
            Self::Ontocode => "3 Ontocode read-only tools",
    ...
    pub fn is_tool_enabled(&self, tool_name: &str) -> bool {
        match self {
            ...
            Self::Ontocode => ONTOCODE_TOOLS.contains(&tool_name),
    ...
    pub fn tool_count(&self) -> usize {
        match self {
            ...
            Self::Ontocode => ONTOCODE_TOOLS.len(),
    ...
    pub fn tool_names(&self) -> Vec<&str> {
        match self {
            ...
            Self::Ontocode => ONTOCODE_TOOLS.to_vec(),
...
/// Ontocode-maintained fork surface: read-only carried v1 tools only.
const ONTOCODE_TOOLS: &[&str] = &["ctx_read", "ctx_search", "ctx_summary"];

/// Available built-in profile names.
pub const PROFILE_NAMES: &[&str] = &["minimal", "standard", "ontocode", "power"];
...
pub fn list_profiles() -> Vec<ProfileInfo> {
    vec![
        ...
        ProfileInfo {
            name: "ontocode",
            tool_count: "3",
            description: "Ontocode maintained-fork surface — ctx_read, ctx_search, ctx_summary",
        },
        ProfileInfo {
            name: "power",
            tool_count: "all",
...
mod tests {
    ...
    fn parse_known_profiles() {
        ...
        assert_eq!(ToolProfile::parse("ontocode"), Some(ToolProfile::Ontocode));
        assert_eq!(
            ToolProfile::parse("ontocode-readonly"),
            Some(ToolProfile::Ontocode)
        );
    ...
    #[test]
    fn ontocode_profile_exposes_carried_readonly_surface_only() {
        let profile = ToolProfile::Ontocode;
        assert!(profile.is_tool_enabled("ctx_read"));
        assert!(profile.is_tool_enabled("ctx_search"));
        assert!(profile.is_tool_enabled("ctx_summary"));
        assert!(!profile.is_tool_enabled("ctx_call"));
        assert!(!profile.is_tool_enabled("ctx_shell"));
        assert!(!profile.is_tool_enabled("ctx_edit"));
    ...
    fn profile_display_counts_match_tool_arrays() {
        ...
        assert_eq!(
            profiles[2].tool_count.parse::<usize>().unwrap(),
            ONTOCODE_TOOLS.len(),
            "ontocode count must match ONTOCODE_TOOLS length",
        );
        assert_eq!(profiles[3].tool_count, "all");
    }
    ...
    fn display_matches_as_str() {
        ...
        assert_eq!(format!("{}", ToolProfile::Ontocode), "ontocode");
    ...
    fn tool_count_matches_list_length() {
        ...
        assert_eq!(ToolProfile::Ontocode.tool_count(), ONTOCODE_TOOLS.len());
