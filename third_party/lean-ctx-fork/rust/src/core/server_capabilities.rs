...
pub fn capabilities_value() -> Value {
    ...
    let tool_names = crate::server::registry::build_registry_for_profile(&tool_profile)
        .names()
        .into_iter()
        .map(str::to_string)
        .collect::<Vec<_>>();
    let read_modes = manifest.get("read_modes").cloned().unwrap_or(Value::Null);
    let active_persona = crate::core::persona::Persona::resolve(&cfg);
    ...
    json!({
        ...
        "server": {
            ...
            "provenance": {
                ...
                "source_root": "third_party/lean-ctx-fork",
            },
        },
        "plane": "personal",
...
mod tests {
    ...
    #[test]
    fn ontocode_profile_reports_runtime_tools_and_provenance() {
        let _guard = crate::core::data_dir::isolated_data_dir();
        crate::test_env::set_var("LEAN_CTX_TOOL_PROFILE", "ontocode");

        let v = capabilities_value();
        assert_eq!(v["server"]["tool_profile"], json!("ontocode"));
        assert_eq!(v["server"]["provenance"]["kind"], json!("maintained-fork"));
        assert_eq!(
            v["server"]["provenance"]["source_root"],
            json!("third_party/lean-ctx-fork")
        );
        assert_eq!(v["tools"]["total"], json!(3));
        assert_eq!(
            v["tools"]["names"],
            json!(["ctx_read", "ctx_search", "ctx_summary"])
        );
