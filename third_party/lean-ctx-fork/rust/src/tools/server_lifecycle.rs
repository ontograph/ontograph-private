    ...
    pub(crate) fn new_with_startup(
        ...
        Self {
            ...
            registry: Some(std::sync::Arc::new(
                crate::server::registry::build_registry_for_profile(
                    &crate::core::config::Config::load().tool_profile_effective(),
                ),
            )),
...
mod tests {
    ...
    #[test]
    fn ontocode_profile_server_starts_with_carried_registry_only() {
        let _guard = crate::core::data_dir::isolated_data_dir();
        crate::test_env::set_var("LEAN_CTX_TOOL_PROFILE", "ontocode");
        let tmp = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(tmp.path().join(".git")).unwrap();

        let server = LeanCtxServer::new_with_project_root(Some(&tmp.path().to_string_lossy()));
        let registry = server
            .registry
            .as_ref()
            .expect("registry must be initialized");
