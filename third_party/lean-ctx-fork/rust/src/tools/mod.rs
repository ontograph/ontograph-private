...
mod resolve_path_tests {
    ...
    #[tokio::test]
    async fn ontocode_profile_blocks_noncarried_tool_dispatch() {
        let _iso = crate::core::data_dir::isolated_data_dir();
        crate::test_env::set_var("LEAN_CTX_TOOL_PROFILE", "ontocode");
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("root");
        let root_str = create_git_root(&root);
        let server = LeanCtxServer::new_with_project_root(Some(&root_str));

        let result = server
            .call_tool_guarded(rmcp::model::CallToolRequestParams::new(
                "ctx_shell".to_string(),
            ))
            .await
            .unwrap();
        ...
        assert_eq!(
            result
                ...
                .map(|t| t.text.as_str()),
            Some("[disabled] 'ctx_shell' is not part of the ontocode backend tool profile.")
        );
