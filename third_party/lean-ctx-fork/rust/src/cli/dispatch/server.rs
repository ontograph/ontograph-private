...
pub(super) fn run_mcp_server() -> Result<()> {
    ...
    let ontocode_backend = matches!(
        crate::core::config::Config::load().tool_profile_effective(),
        crate::core::tool_profiles::ToolProfile::Ontocode
    );
    if !ontocode_backend {
    ...
    rt.block_on(async {
        ...
        if !ontocode_backend {
            ...
            core::plugins::PluginManager::notify(
                core::plugins::executor::HookPoint::OnSessionStart,
            );
        }
        ...
        if !ontocode_backend && core::plugins::PluginManager::has_listener("on_session_end") {
