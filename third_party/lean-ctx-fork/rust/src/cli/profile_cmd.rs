...
fn cmd_tool_profile_show() {
    ...
    if !pinned {
        ...
        println!("\n  Switch with: lean-ctx tools <minimal|standard|ontocode|power>");
        return;
    }
    ...
    println!("\n  Switch with: lean-ctx tools <lean|minimal|standard|ontocode|power>");
    if matches!(profile, ToolProfile::Power) {
...
fn cmd_tool_profile_switch(name: &str) {
    ...
    let Some(profile) = ToolProfile::parse(name) else {
        ...
        eprintln!("Available: lean (default), minimal, standard, ontocode, power");
        std::process::exit(1);
    };
...
TOOL PROFILES — how many MCP tools your agent sees:
  ...
  lean-ctx tools ontocode       3 carried read-only tools
