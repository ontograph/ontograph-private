...
//!     tools hidden by lazy/profile filtering stay reachable, except for
//!     profiles whose contract is an authoritative backend allowlist.
...
pub fn advertised_tool_defs_default() -> Vec<rmcp::model::Tool> {
    ...
    if needs_invoker(full_mode, already, true, &disabled, &profile)
...
pub fn needs_invoker(
    ...
    !full_mode
        && !already_present
        && invoker_role_allowed
        && !disabled.iter().any(|d| d == INVOKER)
        && !matches!(profile, ToolProfile::Ontocode)
}

/// Whether the active backend profile allows this tool to dispatch at all.
///
/// Most lean-ctx profiles are advertisement profiles: hidden tools remain
/// reachable through direct calls or [`INVOKER`]. The Ontocode maintained-fork
/// profile is different; it is the backend-native allowlist for the carried
/// read-only surface, so direct calls must not bypass it.
#[must_use]
pub fn profile_allows_dispatch(name: &str, profile: &ToolProfile) -> bool {
    !matches!(profile, ToolProfile::Ontocode) || profile.is_tool_enabled(name)
}
...
mod tests {
    ...
    #[test]
    fn ontocode_profile_advertises_only_carried_surface() {
        let p = ToolProfile::Ontocode;
        for allowed in ["ctx_read", "ctx_search", "ctx_summary"] {
            assert!(is_tool_visible(allowed, &p, &[], false, true));
            assert!(profile_allows_dispatch(allowed, &p));
        }
        for blocked in [
            "ctx_call",
            "ctx_shell",
            ...
            assert!(!is_tool_visible(blocked, &p, &[], false, true));
            assert!(!profile_allows_dispatch(blocked, &p));
        }
    }
    ...
    fn invoker_added_when_missing_in_lazy_mode() {
        assert!(needs_invoker(false, false, true, &[], &ToolProfile::Power));
    }

    #[test]
    fn invoker_not_added_in_full_mode() {
        assert!(!needs_invoker(true, false, true, &[], &ToolProfile::Power));
    }

    #[test]
    fn invoker_not_duplicated_when_present() {
        assert!(!needs_invoker(false, true, true, &[], &ToolProfile::Power));
    }
    ...
    #[test]
    fn invoker_not_added_for_ontocode_profile() {
        assert!(!needs_invoker(
            false,
            false,
            true,
            &[],
            &ToolProfile::Ontocode
        ));
    }
