...
impl LeanCtxServer {
    pub(crate) async fn call_tool_guarded(
        ...
        let tool_profile = crate::core::config::Config::load().tool_profile_effective();
        if !tool_visibility::profile_allows_dispatch(name, &tool_profile) {
            return Ok(CallToolResult::success(vec![Content::text(format!(
                "[disabled] '{name}' is not part of the {} backend tool profile.",
                tool_profile.as_str()
            ))]));
        }
