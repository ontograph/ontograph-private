...
fn build_ontocode_registry() -> ToolRegistry {
...
pub fn build_registry_for_profile(
    ...
    if matches!(profile, crate::core::tool_profiles::ToolProfile::Ontocode) {
        return build_ontocode_registry();
    }
...
mod tests {
    ...
    #[test]
    fn build_registry_retains_noncarried_tools() {
        let registry = build_registry();

        assert!(registry.get("ctx_shell").is_some());
        assert!(registry.get("ctx_edit").is_some());
        assert!(registry.get("ctx_session").is_some());
    }

    #[test]
    fn ontocode_profile_exposes_carried_readonly_surface_only() {
        let registry =
            build_registry_for_profile(&crate::core::tool_profiles::ToolProfile::Ontocode);

        assert_eq!(
            registry.names(),
            vec!["ctx_read", "ctx_search", "ctx_summary"]
        );
