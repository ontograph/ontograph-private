...
| `server` | object | `{ name, version, persona, tool_profile, provenance }` — `version` is the running maintained backend release; `persona` is the active context persona (`persona-spec-v1`, EPIC 12.15); `tool_profile` is the effective runtime tool profile; `provenance` identifies the maintained source home, currently `{ kind, source_root }`. |
...
```json
{
  ...
  "server": {
    ...
    "version": "3.7.1",
    "persona": "coding",
    "tool_profile": "ontocode",
    "provenance": { "kind": "maintained-fork", "source_root": "third_party/lean-ctx-fork" }
  },
  "plane": "personal",
