...
| --- | --- | --- | --- | --- |
| Human-facing brand | Root docs and plans such as `README*`, release notes, screenshots, help copy, package descriptions; examples already visible in [sdk/python/README.md](/opt/demodb/_workfolder/ontocode/sdk/python/README.md:1), [sdk/typescript/README.md](/opt/demodb/_workfolder/ontocode/sdk/typescript/README.md:1), [codex-rs/exec-server/README.md](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/README.md:1) | rename now | Low | Best Stage 1 target. Keep technical identifiers unchanged in the same pass unless already aliased. |
| Primary CLI executable name | Rust bin name `codex` in [codex-rs/cli/Cargo.toml](/opt/demodb/_workfolder/ontocode/codex-rs/cli/Cargo.toml:7); docs and SDKs shell out to `codex` | alias | High | Add `ontocode` alongside `codex`; do not remove `codex` until a defined release after parity and rollback tests. |
| CLI subcommands and flags | Existing command surface rooted under the `codex` binary, including `codex exec-server`, `codex mcp`, `codex login`, `codex app-server`; examples in [codex-rs/exec-server/README.md](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/README.md:9) and CLI tests under [codex-rs/cli/tests](/opt/demodb/_workfolder/ontocode/codex-rs/cli/tests:1) | alias | Medium | Prefer command-name aliasing only. Avoid renaming subcommand nouns unless there is separate product value. |
| User home and config root | `CODEX_HOME`, `~/.codex`, `config.toml`, `auth.json`, plugin and session storage; surfaced in [sdk/python/src/openai_codex/client.py](/opt/demodb/_workfolder/ontocode/sdk/python/src/openai_codex/client.py:694) and many CLI tests such as [codex-rs/cli/tests/login.rs](/opt/demodb/_workfolder/ontocode/codex-rs/cli/tests/login.rs:23) | alias | Critical | Highest split-brain risk. Read both old and new locations, define write target explicitly, and test mixed-version behavior before any public cutover. |
...
| Auth and credential state | `auth.json` under `CODEX_HOME` in [codex-rs/cli/tests/login.rs](/opt/demodb/_workfolder/ontocode/codex-rs/cli/tests/login.rs:23) plus SDK/CLI login flows | alias | Critical | Credentials are persisted state, not branding. Favor read-both/write-one or copy-on-first-run; avoid duplicate auth stores. |
| Plugin metadata and plugin cache layout | `.codex-plugin` manifests and cached plugin paths in [codex-rs/cli/tests/plugin_cli.rs](/opt/demodb/_workfolder/ontocode/codex-rs/cli/tests/plugin_cli.rs:59) and [codex-rs/cli/tests/plugin_cli.rs](/opt/demodb/_workfolder/ontocode/codex-rs/cli/tests/plugin_cli.rs:783) | preserve | High | The dot-directory name is a local integration contract. Renaming it adds migration cost with little user-visible value. Consider supporting discovery from a future `.ontocode-plugin` only if there is strong need. |
| Environment variable family | `CODEX_HOME`, `CODEX_API_KEY`, `CODEX_EXEC_SERVER_URL`, `CODEX_LOG`, `CODEX_INTERNAL_ORIGINATOR_OVERRIDE`, SDK test envs in [sdk/typescript/src/exec.ts](/opt/demodb/_workfolder/ontocode/sdk/typescript/src/exec.ts:43) and exec-server envs in [codex-rs/exec-server/src/environment.rs](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/src/environment.rs:24) | alias | High | Add `ONTOCODE_*` as preferred inputs while continuing to honor `CODEX_*`. New names should win when both are set. |
| Rust workspace crate identities | Workspace dependency graph in [codex-rs/Cargo.toml](/opt/demodb/_workfolder/ontocode/codex-rs/Cargo.toml:102) includes `codex-core`, `codex-cli`, `codex-mcp`, `codex-app-server-protocol`, many more | defer | High | Internal churn is large and low-value relative to external rename. Revisit only after external surfaces are stable. |
| Rust library/package names shipped to users | Cargo package names such as `codex-cli`, `codex-exec-server`, `codex-windows-sandbox` in [codex-rs/cli/Cargo.toml](/opt/demodb/_workfolder/ontocode/codex-rs/cli/Cargo.toml:2), [codex-rs/exec-server/Cargo.toml](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/Cargo.toml:2), [codex-rs/windows-sandbox-rs/Cargo.toml](/opt/demodb/_workfolder/ontocode/codex-rs/windows-sandbox-rs/Cargo.toml:5) | defer | High | These are package identities and release automation inputs. Treat as a later package-migration program, not a branding pass. |
...
| Protocol and schema crate names | `codex-app-server-protocol`, related generated schemas, and protocol docs referenced by exec-server and SDKs | defer | High | Internal package names can stay `codex-*`; wire compatibility matters more than crate branding. |
| Protobuf package names and checked-in generated artifacts | `package codex.exec_server.relay.v1;` in [codex-rs/exec-server/src/proto/codex.exec_server.relay.v1.proto](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/src/proto/codex.exec_server.relay.v1.proto:3) and generated `.rs`/tests | version | Critical | This is a wire identifier. Change only with explicit versioning and dual-reader support, or preserve indefinitely. |
| MCP server, resource, and memo identifiers | Test-visible URIs and names such as `memo://codex/{slug}` and `codex-memo` in [codex-rs/rmcp-client/tests/resources.rs](/opt/demodb/_workfolder/ontocode/codex-rs/rmcp-client/tests/resources.rs:24) | version | High | If these patterns exist in real clients, rename via aliases or resource-versioning; do not silently switch canonical URIs. |
...
| OAuth labels, service names, analytics, and metrics | Metrics like `codex.memories.tool.call` in [codex-rs/ext/memories/src/metrics.rs](/opt/demodb/_workfolder/ontocode/codex-rs/ext/memories/src/metrics.rs:5), service/client names like `codex-core` and `codex-exec-server` in [codex-rs/exec-server/src/client.rs](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/src/client.rs:101) and [codex-rs/exec-server/src/remote.rs](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/src/remote.rs:120) | preserve | Medium | Low user-facing value, potentially high observability churn. Consider display-layer branding without renaming metric keys. |
| Helper executable and sandbox names | `codex-linux-sandbox`, `codex-windows-sandbox`, `codex-command-runner`, hidden fs helper args, and arg0 dispatch in [codex-rs/windows-sandbox-rs/Cargo.toml](/opt/demodb/_workfolder/ontocode/codex-rs/windows-sandbox-rs/Cargo.toml:5) and [codex-rs/exec-server/src/fs_helper.rs](/opt/demodb/_workfolder/ontocode/codex-rs/exec-server/src/fs_helper.rs:42) | defer | High | These participate in dispatch, packaging, and platform-specific install logic. Rename only if the top-level CLI aliasing proves insufficient. |
