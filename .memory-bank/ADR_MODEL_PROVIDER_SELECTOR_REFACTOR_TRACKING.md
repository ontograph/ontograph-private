...
| --- | --- | --- | --- | --- |
| P1 | Provider selector impl slice | Introduce internal provider-kind selector and route `create_model_provider` through it without behavior changes | done | Added private `ProviderKind` routing in `codex-rs/model-provider/src/provider.rs`; `just fmt`, `just test -p codex-model-provider`, and `just fix -p codex-model-provider` passed |
| P2 | External provider predicate audit | Classify provider/auth predicates outside `codex-rs/model-provider` | done | Audit captured in `MODEL_PROVIDER_SELECTOR_PREDICATE_AUDIT.md`; no must-migrate-now blockers found |
...
| P6 | Post-P5 selector verification | Re-audit predicate leaks, review P5 diff, and confirm validation after implementation landed | done | No direct CLI doctor auth/reachability predicate leaks remain; `codex-rs/cli/src/main.rs` has an unrelated CLI alias/help diff and should stay out of selector scope |
| P7 | App-server auth status provider capability migration | Move app-server auth status requirement logic off raw `ModelProviderInfo.requires_openai_auth` and onto provider-owned account/capability output | done | Auth status now uses provider capabilities; targeted app-server auth-status tests passed 8/8 and `just fix -p codex-app-server` passed |
...
| P9 | TUI auth/status provider capability migration | Move TUI login/status/rate-limit decisions off raw `requires_openai_auth` and onto provider-owned capability/account output | done | Runtime TUI predicate migration implemented; focused TUI login tests passed 7/7 and `just fix -p codex-tui` passed |
...
| 15 | Marked `P7` done | Re-ran `CARGO_BUILD_JOBS=1 just test -p codex-app-server get_auth_status` after build artifacts warmed; 8/8 passed, then `just fix -p codex-app-server` passed |
...
| 24 | Marked `P9` done | After Cargo lock cleared, focused TUI login tests passed 7/7 and `just fix -p codex-tui` passed |
| 25 | Marked `P10` done | Focused model-provider Azure probe test and CLI provider-reachability tests passed; `just fix -p codex-model-provider`, `just fix -p codex-cli`, and `just fix -p codex-tui` passed |
...
| 27 | Marked `P11` done | Registry implementation is deferred until a real third heterogeneous provider validates the abstraction; future provider-specific predicates must stay inside `codex-rs/model-provider` |
...
| 30 | Marked `P13`/`P14`/`P15` in progress | Dispatching one implementation worker for the shared `codex-rs/model-provider` write set |
