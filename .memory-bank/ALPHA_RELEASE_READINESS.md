...
- 2026-06-22 alpha staging checks
  - Clean release branch `alpha/0.1.0-alpha.1` bumps `ontocode-rs/Cargo.toml` to `0.1.0-alpha.1` so the `rust-v0.1.0-alpha.1` tag passes the `rust-release` tag check.
  - Clean release branch compile-fix slice includes the already-dirty core/protocol/state/tool API alignment required for `8b61fd0db` to build with the release version.
  - `CARGO_BUILD_JOBS=8 cargo build --release -p ontocode-cli --bin ontocode` passed on the clean release branch in `31m 07s`.
  - `/tmp/ontocode-alpha-release/ontocode-rs/target/release/ontocode --version` reports `Ontocode CLI 0.1.0-alpha.1`.
  - `CARGO_BUILD_JOBS=8 just test -p ontocode-api` passed with 127 tests.
  - `CARGO_BUILD_JOBS=8 just test -p ontocode-protocol` passed with 235 tests.
  - `CARGO_BUILD_JOBS=8 just test -p ontocode-state` passed with 168 tests after adding the missing `0037_agent_jobs_final_summary.sql` migration to match the state agent-job code already in the release branch.
